...
public subroutine set_count_connections (st_tab_sr_utenti kst_tab_sr_utenti) throws uo_exception
public function integer get_count_connections (ref st_tab_sr_utenti kst_tab_sr_utenti) throws uo_exception
public subroutine set_count_connections_add (st_tab_sr_utenti kst_tab_sr_utenti) throws uo_exception
...
//
boolean k_return = false
datetime k_dthr_last_access_prec
...
//--- 
//--- inp: st_tab_sr_utenti.id e count_connections
//--- 
//------------------------------------------------------------------
//
st_esito kst_esito
...
try
	...
	if kst_tab_sr_utenti.id > 0 then
		...
		kst_esito.esito = kkg_esito.no_esecuzione  
		kst_esito.SQLErrText = "Errore in aggiornamento contatore degli accessi dell'utente. Manca ID utente"
		kGuo_exception.set_esito( kst_esito )
		throw kGuo_exception
	end if

	update sr_utenti  
			set count_connections = :kst_tab_sr_utenti.count_connections
		where id = :kst_tab_sr_utenti.id
		using kguo_sqlca_db_magazzino;
	...
	if sqlca.sqlcode < 0 then
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.SQLErrText = "Errore in aggiornamento contatore degli accessi sull'utente " &
										+ string(kst_tab_sr_utenti.id) &
									   + ". " + trim(sqlca.SQLErrText)
		kGuo_exception.set_esito( kst_esito )
		throw kGuo_exception
...
try
	...
	if kst_tab_sr_utenti.id > 0 then
		...
		kst_esito.esito = kkg_esito.no_esecuzione  
		kst_esito.SQLErrText = "Errore in aggiunta contatore degli accessi dell'utente. Manca ID utente"
		kGuo_exception.set_esito( kst_esito )
		throw kGuo_exception
	end if

	get_count_connections(kst_tab_sr_utenti)

	kst_tab_sr_utenti.count_connections ++

	set_count_connections(kst_tab_sr_utenti)
