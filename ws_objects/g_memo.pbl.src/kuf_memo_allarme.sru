...
public function boolean if_open_window_allarmi () throws uo_exception
end prototypes
...
try
	...
	if isvalid(kds_memo_if_allarme_utente) then destroy kds_memo_if_allarme_utente
...
public function boolean if_open_window_allarmi () throws uo_exception;//------------------------------------------------------------------------------------------------------------------------
//--- Verifica se la finestra degli Allarmi è attivabile
//--- 
//--- Ret: true finestra allarme attivo
//------------------------------------------------------------------------------------------------------------------------
//
boolean k_return
string k_esito_base
...
try
	...
	k_esito_base = kuf1_base.prendi_dato_base( "memo_window_open")
	if left(k_esito_base,1) <> "0" then
	else
		if trim(mid(k_esito_base,2)) = "1" then
			k_return = true
		end if
	end if
