	...
	choose case ki_xpl_SCELTA_info 
		...
		case ki_xplistbar_riga_clienti
			...
			if kst_tab_clienti.codice > 0 then
				k_call=true
				kst_esito = kiuf_clienti.anteprima ( kdsi_elenco_output, kst_tab_clienti )
				destroy kuf1_armo
				if kst_esito.esito <> kkg_esito.ok then
					kuo_exception = create uo_exception
					kuo_exception.set_esito( kst_esito)
...
//	if ki_attiva_toolbar_periodo then
		if NOT m_main.m_strumenti.m_fin_gest_libero6.enabled then 
			m_main.m_strumenti.m_fin_gest_libero6.text = "Cambia il periodo di estrazione elenco "
			m_main.m_strumenti.m_fin_gest_libero6.microhelp =  "Cambia periodo di estrazione "
			...
			m_main.m_strumenti.m_fin_gest_libero6.toolbaritemName = "Custom015!"
...
public function boolean u_resize_predefinita ();int k_dist_bordi, k_spess_bordi_x, k_spess_bordi_y
...
type dw_print_0 from w_g_tab0`dw_print_0 within w_memo_mkt
end type
