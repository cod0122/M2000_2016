...
private boolean ki_flag_note_aco_si 
end variables
...
public subroutine u_set_note_aco ()
public subroutine u_set_ki_flag_note_aco ()
protected subroutine inizializza_5 () throws uo_exception
protected subroutine stampa_receiptgammarad_row ()
private subroutine crea_pklist_ext ()
private subroutine u_genera_lotto ()
...
if tab_1.tabpage_1.dw_1.rowcount( ) > 0 and (tab_1.tabpage_1.dw_1.getnextmodified(0, primary!) > 0 &
			or tab_1.tabpage_4.dw_4.getnextmodified(0, primary!) > 0 or tab_1.tabpage_4.dw_4.deletedcount() > 0) then
...
//--- Se tutto OK carico le righe di dettaglio 	
	if kst_esito.esito <> kkg_esito.db_ko then
		...
		if tab_1.tabpage_4.dw_4.getnextmodified(0, primary!) > 0 or tab_1.tabpage_4.dw_4.deletedcount() > 0 then
...
//---- azzera il flag delle modifiche
		...
		k_errore = string(kst_esito.sqlcode) + " " + trim(kst_esito.sqlerrtext) + "~n~rPrego, controllare i dati !! "
...
//=== Cancella la riga dal data windows di lista
	...
	//=== Richiesta di conferma della eliminazione del rek
				if messagebox("Elimina" + k_record, k_record_1, question!, yesno!, 2) = 1 then
					...
					kuf1_wm_pklist.tb_delete( kst_tab_wm_pklist )   // CANCELLA o 'ELIMINA' PKLIST
	...
	//				k_errore = kiuf_fatt.tb_delete_riga( kst_tab_arfa )   
		...
		kst_esito = kuo_exception.get_st_esito()
		messagebox("Cancellazione in Errore", kst_esito.sqlerrtext)
		kguo_sqlca_db_magazzino.db_rollback( )
		k_return = 1
...
//=== Controllo il primo tab
	...
	k_nr_errori = 0
	k_riga = 1
	this.set_st_tab_wm_pklist_da_dw1( k_riga, kst_tab_wm_pklist)
		...
		try 
			...
			do while k_riga > 0  and k_nr_errori < 10
				...
				if k_nr_errori < 9 then // per non uscire da check senza contr.eventuali eltri errori gravi 
					...
					this.set_st_tab_wm_pklist_righe_da_dw4(k_riga, kst_tab_wm_pklist_righe)
...
//--- solo se sono in caricamento posso importare nuove PKL grezze
	...
	if m_main.m_strumenti.m_fin_gest_libero1.enabled <> k_attiva then
		m_main.m_strumenti.m_fin_gest_libero1.text = "Crea nuovo Packing-list da file cliente importato"
		m_main.m_strumenti.m_fin_gest_libero1.microhelp = "Crea Packing-list già importato"
	
		m_main.m_strumenti.m_fin_gest_libero1.toolbaritemVisible = true
		m_main.m_strumenti.m_fin_gest_libero1.toolbaritemText = 	"Crea, Nuovo Packing-list da file importato"
		m_main.m_strumenti.m_fin_gest_libero1.toolbaritembarindex=2
	
		m_main.m_strumenti.m_fin_gest_libero1.enabled = true
		m_main.m_strumenti.m_fin_gest_libero1.toolbaritemname = "CreateTable5!"
...
//=== 
	if tab_1.tabpage_1.dw_1.rowcount( ) > 0 then	
		...
		if m_main.m_strumenti.m_fin_gest_libero2.enabled <> k_attiva then
			m_main.m_strumenti.m_fin_gest_libero2.text = "Genera nuovo Lotto di entrata "
			m_main.m_strumenti.m_fin_gest_libero2.microhelp = "Genera Lotto da questo Packing-list "
			m_main.m_strumenti.m_fin_gest_libero2.visible = true

			m_main.m_strumenti.m_fin_gest_libero2.toolbaritemText = 	"Genera, Carica Nuovo Lotto  "
			m_main.m_strumenti.m_fin_gest_libero2.toolbaritembarindex=2
		
			m_main.m_strumenti.m_fin_gest_libero2.enabled = k_attiva
			m_main.m_strumenti.m_fin_gest_libero2.toolbaritemname = "lotto16x16.gif" 
			m_main.m_strumenti.m_fin_gest_libero2.toolbaritemVisible = true
		end if
		
		m_main.m_finestra.m_gestione.m_fin_duplica.text = "Duplica Packing-list"
		m_main.m_finestra.m_gestione.m_fin_duplica.microhelp = "Crea una duplica da questo Packing-list"
		m_main.m_finestra.m_gestione.m_fin_duplica.toolbaritemText = "Duplica,Crea un nuovo Packing-list da questo "
		m_main.m_finestra.m_gestione.m_fin_duplica.toolbaritemVisible = ki_consenti_duplica
		...
		if m_main.m_strumenti.m_fin_gest_libero3.enabled <> k_attiva then
			m_main.m_strumenti.m_fin_gest_libero3.text = "Modifica codici barcode"
			m_main.m_strumenti.m_fin_gest_libero3.microhelp = "Modifica codici barcode del mittente"
			m_main.m_strumenti.m_fin_gest_libero3.visible = true

			m_main.m_strumenti.m_fin_gest_libero3.toolbaritemText = "Barcode,Modifica codici barcode"
			m_main.m_strumenti.m_fin_gest_libero3.toolbaritembarindex=2
		
			m_main.m_strumenti.m_fin_gest_libero3.enabled = k_attiva
			m_main.m_strumenti.m_fin_gest_libero3.toolbaritemname = "barcode.bmp" 
			m_main.m_strumenti.m_fin_gest_libero3.toolbaritemVisible = true
		end if

		m_main.m_strumenti.m_fin_gest_libero4.text = "Stampa righe pkl cliente"
		...
		m_main.m_strumenti.m_fin_gest_libero4.toolbaritemText = "St.Righe,"+ m_main.m_strumenti.m_fin_gest_libero4.text
		...
		if tab_1.selectedtab = 6 then
			...
			m_main.m_strumenti.m_fin_gest_libero4.enabled = false
		end if
	...
	if ki_flag_note_aco_si and k_attiva then
		tab_1.tabpage_1.dw_1.modify("b_import_note_aco.enabled = 1")
	else
		tab_1.tabpage_1.dw_1.modify("b_import_note_aco.enabled = 0")
	end if
...
choose case trim(left(k_par_in, 3))
	...
	case "l3"		//modifica barcode
		u_modifica_barcode( )
		
	case "l4"		//Stampa dettaglio righe RECEIPTGAMMARAD 
		stampa_receiptgammarad_row()
...
st_tab_wm_pklist_righe kst_tab_wm_pklist_righe
	...
	if k_riga > 0 then
		...
		set_st_tab_wm_pklist_righe_da_dw4(k_riga, 	kst_tab_wm_pklist_righe)	
...
try
	...
	kst_tab_wm_pklist.note_aco = tab_1.tabpage_1.dw_1.getitemstring(1, "note_aco")
	if trim(kst_tab_wm_pklist.note_aco) > " " then
		kguo_exception.inizializza( )
		kguo_exception.kist_esito.esito = kkg_esito.no_esecuzione
		kguo_exception.kist_esito.sqlerrtext = "Note già presenti, per reimportarle bisogna prima cancellarle!"
		throw kguo_exception
	else
		kst_tab_wm_pklist.idpkl = tab_1.tabpage_1.dw_1.getitemstring(1, "idpkl")
		if kst_tab_wm_pklist.idpkl > " " then
			kiuf_wm_pklist_testa.get_note_aco_build(kst_tab_wm_pklist)
			tab_1.tabpage_1.dw_1.setitem(1, "note_aco", kst_tab_wm_pklist.note_aco)
		end if
	end if
...
try
	if tab_1.tabpage_1.dw_1.rowcount() > 0 then
		...
		kst_tab_clienti_cntdep_cfg.id_cliente = tab_1.tabpage_1.dw_1.getitemnumber(1, "clie_3")
		if kst_tab_clienti_cntdep_cfg.id_cliente > 0 then
			if kuf1_clienti_cntdep.get_flag_note_aco(kst_tab_clienti_cntdep_cfg) = kuf1_clienti_cntdep.kki_flag_note_aco_si then
				ki_flag_note_aco_si = true 
			end if
		end if
	else
		ki_flag_note_aco_si = false
	end if
...
if kst_tab_wm_pklist.id_wm_pklist > 0 then //or trim(kst_tab_wm_pklist.packinglistcode) > " " then
	...
	if tab_1.tabpage_6.dw_6.rowcount() < 1 then
		...
		//tab_1.tabpage_6.dw_6.retrieve( kst_tab_wm_pklist.packinglistcode, kst_tab_wm_pklist.id_wm_pklist ) 
		tab_1.tabpage_6.dw_6.retrieve( '', kst_tab_wm_pklist.id_wm_pklist ) 
...
//--- stampa righe dettaglio del RECEIPTGAMMARAD
if tab_1.selectedtab = 6 then
	if tab_1.tabpage_1.dw_1.rowcount( ) > 0 then
		kst_tab_wm_pklist.id_wm_pklist  = tab_1.tabpage_1.dw_1.getitemnumber(1, "id_wm_pklist") 
		if kst_tab_wm_pklist.id_wm_pklist > 0 then 

			dw_print_0.dataobject = "d_wm_receiptgammarad_dett"
			dw_print_0.settransobject(kguo_sqlca_db_magazzino)
			k_rc = dw_print_0.retrieve(kst_tab_wm_pklist.id_wm_pklist)
			if k_rc <= 0 then
				messagebox("Richiesta Stampa", "Stampa non eseguita, nessuna riga da stmpare (" + string(k_rc) +")")
			else
				kidw_selezionata = dw_print_0
			
				kst_tab_wm_pklist.packinglistcode  = tab_1.tabpage_1.dw_1.getitemstring(1, "packinglistcode")  
				kist_stampe.titolo = "Scheda Pkl_" + trim(kst_tab_wm_pklist.packinglistcode)
...
try 
	...
	if tab_1.tabpage_1.dw_1.rowcount( ) > 0 then	
		...
		if kst_tab_wm_pklist.id_wm_pklist > 0 then
			...
			if messagebox("Genera Lotto", & 
				"Generare il nuovo Lotto da questa Packing-List '" + kst_tab_wm_pklist.idpkl + "'?~n~r" &
				+ "L'operazione aggiorna anche i dati del Packing-List in archivio." &
				, question!, yesno!, 2) = 1 then
						
				try
					
					if ki_flag_note_aco_si then
						try
							if trim(tab_1.tabpage_1.dw_1.getitemstring(1, "note_aco")) > " " then
							else	
								u_set_note_aco()   // Importa se non già fatto le NOTE x ACO
							end if
					...
					kst_esito = u_genera_lotto_check_dati()  //  Controlla dati della PKL x Importazione
					
					if kst_esito.esito = kkg_esito.dati_wrn then
						if messagebox("Genera Lotto dal Packing-List", & 
											"Attenzione ai seguenti errore NON bloccanti" &
											+ kkg.acapo + trim(kst_esito.sqlerrtext) &
											+ "Proseguire comunque l'operazione?" &
											, question!, yesno!, 2) = 2 then
							kst_esito.esito = kkg_esito.no_esecuzione
							kst_esito.sqlerrtext = "Operazione interrotta dall'utente per questo motivo:" + kkg.acapo + trim(kst_esito.sqlerrtext) 
							kguo_exception.set_esito(kst_esito)
							throw kguo_exception
						end if
					end if
...
//								"Vuoi aprire il nuovo Lotto prodotto da questa Packing-List (id lotto = " + string(kst_tab_meca.id) + ") ?", question!, yesno!, 2) = 1 then
...
//--- se tutto OK chiude la FUNZIONE
					cb_ritorna.post triggerevent(clicked!)
...
private subroutine set_st_tab_wm_pklist_da_dw1 (long k_riga, ref st_tab_wm_pklist kst_tab_wm_pklist);//
		...
		kst_tab_wm_pklist.mc_co = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "mc_co"))
		kst_tab_wm_pklist.sc_cf = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "sc_cf"))
		kst_tab_wm_pklist.eliminato = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "eliminato"))
		
		kst_tab_wm_pklist.packinglistcode = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "packinglistcode"))
		...
		kst_tab_wm_pklist.x_utente = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "x_utente"))
		kst_tab_wm_pklist.x_datins_elim = tab_1.tabpage_1.dw_1.getitemdatetime(k_riga, "x_datins_elim")
		kst_tab_wm_pklist.x_utente_elim = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "x_utente_elim"))
...
Controllo formale e logico dei dati inseriti
Lancia Exception
Ret: st_esito: Torna se necessario un messaggio da visualizzare come avvertimento
======================================================================
*/
long k_nr_righe
int k_riga, k_ctr
...
//=== Controllo il primo tab
	...
	set_st_tab_wm_pklist_da_dw1( k_riga, kst_tab_wm_pklist)  //--- riempie l'area di lavoro

	if kst_tab_wm_pklist.clie_1 = 0 then
		kst_esito.sqlerrtext = tab_1.tabpage_1.text + ": Manca il codice Mandante, operazione interrotta!" 
		kst_esito.esito = kkg_esito.no_esecuzione
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
//--- controllo se PK-LIST già caricato
		...
		if kst_tab_wm_pklist_altro.id_wm_pklist > 0 then
			...
			kuf1_armo.get_id_da_id_wm_pklist(kst_tab_meca)
			if kst_tab_meca.id > 0 then
				kuf1_armo.get_dati_rid(kst_tab_meca)    // get id lotto
			end if
		end if
			
		if kiuf_wm_pklist_testa.if_esiste( kst_tab_wm_pklist ) then
			kst_esito.sqlerrtext = tab_1.tabpage_1.text + ": Packing-list '" + trim(kst_tab_wm_pklist.idpkl) + "' già caricato con lo stesso Codice, vedi id " &
												+ string(kst_tab_wm_pklist_altro.id_wm_pklist) 
				
			if kst_tab_meca.id > 0 then
				kst_esito.sqlerrtext += kkg.acapo + "Lotto n. " + string(kst_tab_meca.num_int) + " del " + string(kst_tab_meca.data_int, "dd mmm yyyy") 
				if date(kst_tab_meca.data_ent) > kkg.data_zero then
					kst_esito.sqlerrtext += " entrato il " + string(kst_tab_meca.data_ent, "dd mmm yy")
				else
					kst_esito.sqlerrtext +=  " non ancora entrato. "
				end if
				if kuf1_armo.if_lotto_chiuso(kst_tab_meca) then
					kst_esito.sqlerrtext += kkg.acapo + "Lotto CHIUSO o ANNULLATO, per proseguire Eliminare il Packing-list indicato." 
				else
					kst_esito.sqlerrtext += kkg.acapo + "Per proseguire ANNULLARE il Lotto ed Eliminare il Packing-list indicato."
				end if
			else
				kst_esito.sqlerrtext += kkg.acapo + "Nessun Lotto associato è stato trovato, per proseguire Eliminare prima il Packing-list indicato." 
			end if
			kst_esito.esito = kkg_esito.no_esecuzione
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		...
		if kst_tab_wm_pklist_altro.id_wm_pklist > 0 and kst_tab_meca.id > 0 then
			kst_esito.sqlerrtext += "ATTENZIONE Lotto n. " + string(kst_tab_meca.num_int) + " del " + string(kst_tab_meca.data_int, "dd mmm yyyy") 
			if date(kst_tab_meca.data_ent) > kkg.data_zero then
				kst_esito.sqlerrtext += " entrato il " + string(kst_tab_meca.data_ent, "dd mmm yy")
			else
				kst_esito.sqlerrtext +=  " non ancora entrato. "
			...
			if kuf1_armo.if_lotto_chiuso(kst_tab_meca) then
				kst_esito.sqlerrtext += kkg.acapo + "Lotto CHIUSO o ANNULLATO." 
				kst_esito.esito = kkg_esito.dati_wrn
				kguo_exception.set_esito(kst_esito)
				kguo_exception.scrivi_log( )
			else
				kst_esito.sqlerrtext += kkg.acapo + "Per proseguire ANNULLARE il Lotto."
				kst_esito.esito = kkg_esito.no_esecuzione
				kguo_exception.set_esito(kst_esito)
				throw kguo_exception
			end if
...
//--- controllo CO 
	if trim(kst_tab_wm_pklist.mc_co) > " " then
		kst_tab_contratti.mc_co = trim(kst_tab_wm_pklist.mc_co)
		if not kiuf_contratti.if_esiste_mc_co(kst_tab_contratti) then
			kst_esito.sqlerrtext = tab_1.tabpage_1.text + " - " + trim(tab_1.tabpage_1.dw_1.object.mc_co_t.text) + " " + trim(kst_tab_contratti.mc_co) + " non Trovato."
			kst_esito.esito = kkg_esito.no_esecuzione
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		end if
...
//--- controllo CF
	if trim(kst_tab_wm_pklist.sc_cf) > " " then
		kst_tab_contratti.sc_cf = trim(kst_tab_wm_pklist.sc_cf)
		if not kiuf_contratti.if_esiste_sc_cf(kst_tab_contratti) then
			kst_esito.sqlerrtext = tab_1.tabpage_1.text + " - " + trim(tab_1.tabpage_1.dw_1.object.sc_cf_t.text) + " " + trim(kst_tab_contratti.sc_cf) + " Capitolato " + trim(kst_tab_contratti.sc_cf) + " non Trovato."
			kst_esito.esito = kkg_esito.no_esecuzione
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		else
			k_data_scad_sf_cf = kiuf_contratti.get_data_scad_sc_cf(kst_tab_contratti)
			if k_data_scad_sf_cf < k_data_pkl then
				kst_esito.sqlerrtext = tab_1.tabpage_1.text + " - " + trim(tab_1.tabpage_1.dw_1.object.sc_cf_t.text) + " " + trim(kst_tab_contratti.sc_cf) + " Capitolato " + trim(kst_tab_contratti.sc_cf) &
									+ " Scaduto il " + string(k_data_scad_sf_cf) + ". Data di confronto " + string(k_data_pkl) + "."
				kst_esito.esito = kkg_esito.no_esecuzione
				kguo_exception.set_esito(kst_esito)
				throw kguo_exception
			end if
...
//--- Controllo Contratto periodo	
	if trim(kst_tab_wm_pklist.mc_co) > " " then
		...
		if kst_esito.esito <> kkg_esito.ok then
			if kst_esito.esito = kkg_esito.err_logico then
				kst_esito.sqlerrtext = tab_1.tabpage_1.text + " - " + trim(tab_1.tabpage_1.dw_1.object.sc_cf_t.text) + " " + trim(kst_tab_contratti.sc_cf) + " Più di un Contratto " + trim(kst_tab_contratti.mc_co) &
								+ " + cf = '" + trim(kst_tab_contratti.sc_cf) + "' presente in archivio." + kkg.acapo + kst_esito.sqlerrtext
			else
				kst_esito.sqlerrtext = tab_1.tabpage_1.text + " - " + trim(tab_1.tabpage_1.dw_1.object.sc_cf_t.text) + " " + trim(kst_tab_contratti.sc_cf) + " Contratto " + trim(kst_tab_contratti.mc_co) &
								+ " + cf = '" + trim(kst_tab_contratti.sc_cf) + "', non Trovato in archivio." 
			end if
			kst_esito.esito = kkg_esito.no_esecuzione
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
			...
			kiuf_contratti.get_data_scad(kst_tab_contratti)
			if k_data_pkl > kst_tab_contratti.data_scad or k_data_pkl < kst_tab_contratti.data then
				kst_esito.sqlerrtext = tab_1.tabpage_1.text + " - " + trim(tab_1.tabpage_1.dw_1.object.sc_cf_t.text) + " " + trim(kst_tab_contratti.sc_cf) + " Contratto " + trim(kst_tab_contratti.sc_cf) &
									+ " + cf = '" + trim(kst_tab_contratti.sc_cf) + "', non valido~n~r" &
									+ "data del pk.list " + string(k_data_pkl) + " oltre il periodo " + string(kst_tab_contratti.data) + "-" + string(kst_tab_contratti.data_scad) + "." 
				kst_esito.esito = kkg_esito.no_esecuzione
				kguo_exception.set_esito(kst_esito)
				throw kguo_exception
			end if
...
//--- controllo se è presente il Contratto-E1 su Listino 
	if kst_tab_wm_pklist.clie_3  > 0 and kst_tab_contratti.codice > 0 then
		...
		kst_tab_listino[1].dt_end = today()
		...
		if trim(kst_tab_listino[1].e1litm) > " " then // verifica se c'è il Contratto-E1 su Listino
			...
			if isnull(kst_tab_listino[1].id) then kst_tab_listino[1].id = 0
			kst_esito.sqlerrtext = tab_1.tabpage_1.text + ": Attenzione listino '" + string(kst_tab_listino[1].id) + "' (id Contratto '" + string(kst_tab_contratti.codice) &
					+ "') senza Contratto-E1, il documento ASN su E1 non può essere generato!!" 
			kst_esito.esito = kkg_esito.no_esecuzione
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		end if
...
//--- verifica se ci sono righe di dettaglio		
	...
	if k_nr_righe = 0 then
		if kst_tab_wm_pklist.id_wm_pklist > 0 then
			 k_nr_righe = tab_1.tabpage_4.dw_4.retrieve(kst_tab_wm_pklist.id_wm_pklist)
		end if
	end if
	if k_nr_righe = 0 then
		kst_esito.sqlerrtext = tab_1.tabpage_1.text + ": Non ci sono righe caricate x questo documento."
		kst_esito.esito = kkg_esito.no_esecuzione
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
	...
	k_nr_errori = 0
	kst_esito.sqlerrtext = tab_1.tabpage_4.text
	k_riga = tab_1.tabpage_4.dw_4.getnextmodified(0, primary!)
	do while k_riga > 0  and k_nr_errori < 10  // per non uscire da check senza contr.eventuali eltri errori gravi 

		this.set_st_tab_wm_pklist_righe_da_dw4(k_riga, kst_tab_wm_pklist_righe)
			
//--- Controllo Numero COLLI			
		if kst_tab_wm_pklist_righe.colli > 0 then 
		else
			k_nr_errori++
			kst_esito.sqlerrtext = trim(kst_esito.sqlerrtext) + ": Manca il numero Colli alla riga "+ string(k_riga, "#####") + "." + kkg.acapo
		end if
...
//--- contollo numero colli messo in testata con quello nelle righe
	if tab_1.tabpage_4.dw_4.rowcount( ) > 0 then
		...
		if kst_tab_wm_pklist.collipkl > 0 then
			if kst_tab_wm_pklist.colliddt <> kst_tab_wm_pklist_righe.colli then
				kst_esito.sqlerrtext = tab_1.tabpage_4.text + ": Totale Colli righe ( "+ string(kst_tab_wm_pklist_righe.colli) &
										 + " ) diverso dal n.Colli bolla (" + string(kst_tab_wm_pklist.colliddt ) + ") indicato; " +  " ~n~r" 
				kst_esito.esito = kkg_esito.dati_wrn
				kguo_exception.set_esito(kst_esito)
				kguo_exception.scrivi_log( )
...
type dw_print_0 from w_g_tab_3`dw_print_0 within w_wm_pklist
end type
...
boolean enabled = true
string text = "barcode"
string picturename = "barcode16.png"
...
integer height = 4872
boolean enabled = true
string text = "dati file"
string picturename = "Custom007_2!"
string powertiptext = "dati origine packing list caricati dal file cliente"
end type
...
integer height = 1280
boolean enabled = true
string dataobject = "d_wm_receiptgammarad"
boolean hsplitscroll = false
end type
