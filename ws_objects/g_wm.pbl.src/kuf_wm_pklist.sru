...
//constant string kki_STATO_DES_nuovo = "1" // nuovo PKL
//constant string kki_STATO_DES_non_importare = "2" // se non e' da Importare come Riferimento
//constant string kki_STATO_DES_importato = "4" // importato nel Riferimento
//constant string kki_STATO_DES_chiuso = "9" // PKL chiusa
...
kuf_wm_pklist_righe kuf1_wm_pklist_righe
	...
	try
		...
		if kuf1_wm_pklist_testa.tb_add(kst_wm_pklist.st_tab_wm_pklist) then
			...
			kuf1_wm_pklist_testa.u_add_note_aco(kst_wm_pklist.st_tab_wm_pklist)
...
public function boolean tb_delete (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception;//
//-------------------------------------------------------------------------
//--- Setta il flag a ELIMINATO o DELETE delle righe nelle tabelle
//---          wm_pklist (Testa e Righe e ReceiptGammarad) 
//--- 
//--- Inp: kst_tab_wm_pklist.id_wm_pklist
//--- Ritorna TRUE - ok eliminato
//---           	
//-------------------------------------------------------------------------
//
boolean k_return
...
//--- se è già stato collegato a un RIFERIMENTO non posso cancellare		
		kuf1_armo = create kuf_armo
		kst_tab_meca.id_wm_pklist = kst_tab_wm_pklist.id_wm_pklist
		if kuf1_armo.get_id_da_id_wm_pklist(kst_tab_meca) > 0 then   // recupera ID lotto

//--- se è collegato a un RIFERIMENTO non Annullato non posso neanche settare il flag a eliminato
			if not kuf1_armo.if_lotto_annullato(kst_tab_meca) then
				
				kst_esito.SQLErrText = "Lotto id ASN " + string(kst_tab_meca.id) +  " è in essere ed è associato a questo Packing list " &
														+ string(kst_tab_wm_pklist.id_wm_pklist) + ". " &
														+ "~n~rCancellazione del Packing list non consentita (prima Annullare o Cancellare il Riferimento)" 
...
//--- faccio il set al flag a eliminato	
			kst_tab_wm_pklist_righe.id_wm_pklist = kst_tab_wm_pklist.id_wm_pklist 
			kuf1_wm_pklist_righe.set_eliminato(kst_tab_wm_pklist_righe)   // elimina le righe
			kuf1_wm_pklist_testa.set_eliminato(kst_tab_wm_pklist)  // elimina la testata
...
//--- faccio DELETE
			kst_tab_wm_pklist_righe.id_wm_pklist = kst_tab_wm_pklist.id_wm_pklist 
			kuf1_wm_pklist_righe.tb_delete(kst_tab_wm_pklist_righe)   // DELETE le righe
			kuf1_wm_pklist_testa.tb_delete(kst_tab_wm_pklist)			// DELETE la testata

		end if
			
//--- DELETE su ReceiptGammarad 
