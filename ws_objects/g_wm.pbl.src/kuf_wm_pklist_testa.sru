...
public function string get_note_aco_build (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function boolean set_note_aco (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function string get_note_aco (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function string u_add_note_aco (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function string get_nrord (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function string get_note_lotto (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function st_esito set_add_note (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
public function boolean set_eliminato (ref st_tab_wm_pklist kst_tab_wm_pklist)
public function boolean tb_delete (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
end prototypes
...
if isnull(kst_tab_wm_pklist.note_aco  ) then kst_tab_wm_pklist.note_aco = ""
...
try
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in lettura Codice Packing List dalla tabella, id cercato: " + string(kst_tab_wm_pklist.id_wm_pklist) + " ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
...
try
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in lettura Codice Lotto caricato in Packing List dal cliente, id pkl: " + string(kst_tab_wm_pklist.id_wm_pklist) + " ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.inizializza( )
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
...
public function string get_note_aco_build (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception;//
//------------------------------------------------------------------
//--- Ritorna composizione NOTE ACO 
//--- inp: st_tab_wm_pklist.idpkl 
//--- rit: stringa con le NOTE ACO
//--- uo_exception: ST_ESITO
//---           	
//------------------------------------------------------------------
//
...
try
	...
	kds_1.dataobject = "ds_wm_pklist_note_aco_build"
	kds_1.settransobject(kguo_sqlca_db_magazzino)
	k_righe = kds_1.retrieve(kst_tab_wm_pklist.idpkl)
		
	if k_righe < 0 then
		kst_esito.sqlcode = k_righe
		kst_esito.SQLErrText = "Errore in Packing List in composizione Note ACO, id: " + string(kst_tab_wm_pklist.idpkl) + "(" + kds_1.dataobject + ")" //" ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.inizializza( )
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	...
	if k_righe > 0 then
		kst_tab_wm_pklist.note_aco = trim(kds_1.getitemstring(1, "note_aco"))
		if kst_tab_wm_pklist.note_aco > " " then
			k_return = kst_tab_wm_pklist.note_aco
		end if
	end if
...
public function boolean set_note_aco (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception;//
//====================================================================
//=== Aggiorna il campo NOTE ACO del Packing List 
//=== 
//=== Input: st_tab_wm_pklist.id_wm_pklist / NOTE_ACO
//=== Ritorna: 
//=== 
//====================================================================
//
...
try
	...
	if kst_tab_wm_pklist.id_wm_pklist > 0 then
		...
		update wm_pklist
				set note_aco = :kst_tab_wm_pklist.note_aco
					,x_datins = :kst_tab_wm_pklist.x_datins
					,x_utente = :kst_tab_wm_pklist.x_utente
				WHERE id_wm_pklist = :kst_tab_wm_pklist.id_wm_pklist
				using kguo_sqlca_db_magazzino;
		...
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = "Errore in aggiornamento Note ACO del Packing-List id = " + string(kst_tab_wm_pklist.id_wm_pklist, "####0") + " " &
	...
	//---- COMMIT....	
		...
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.SQLErrText = "Aggiornamento Note ACO del Packing-List non eseguito, manca il codice id" 
		kst_esito.esito = kkg_esito.no_esecuzione
		kguo_exception.inizializza( )
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
...
try
	...
	select note_aco
		into :kst_tab_wm_pklist.note_aco
		from wm_pklist 
		where id_wm_pklist = :kst_tab_wm_pklist.id_wm_pklist
		using kguo_sqlca_db_magazzino ;
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in lettura Note ACO del 'Packing List' (id " + string(kst_tab_wm_pklist.id_wm_pklist) + ") :" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
//------------------------------------------------------------------------------------------------------------------------------------
//---
//--- Aggiunge le NOTE ACO in tabella wm_pklist
//---
//---	inp: kst_tab_wm_pklist.idpkl + id_wm_pklist
//---	out: 
//---	rit: TRUE=tutto ok
...
try
	...
	if kst_tab_wm_pklist.idpkl > " " and kst_tab_wm_pklist.id_wm_pklist > 0 then
	
		kst_tab_wm_pklist.note_aco = get_note_aco_build(kst_tab_wm_pklist)
		set_note_aco(kst_tab_wm_pklist)
		...
		kguo_exception.inizializza( )
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.sqlerrtext = "Note ACO non aggiunte al Packing-list, codici assenti!"
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
public function string get_nrord (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception;//
//------------------------------------------------------------------
//--- Rstituisce il Numero Ordine
//--- 
//--- Inp: st_tab_wm_pklist.id_wm_pklist
//--- Out: st_tab_wm_pklist.nrord
//--- Ret: stringa Numero Ordine
//---           	
//------------------------------------------------------------------
//
string k_return
...
try
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in lettura Numero Ordine nel 'Packing List' (id " + string(kst_tab_wm_pklist.id_wm_pklist) + ") :" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
	...
	if sqlca.sqlcode = 0 then
		if kst_tab_wm_pklist.nrord > " " then
			k_return = trim(kst_tab_wm_pklist.nrord)
		end if
	end if
...
try
	...
	if kst_tab_wm_pklist.id_wm_pklist > 0 then
		...
		if sqlca.sqlcode <> 0 then
			...
			if sqlca.sqlcode < 0 then
				kst_esito.sqlcode = sqlca.sqlcode
				kst_esito.SQLErrText = "Errore in lettura Note Cliente del 'Packing List' id: " &
										+ string(kst_tab_wm_pklist.id_wm_pklist) + " (Tab. wm_pklist). " + trim(sqlca.SQLErrText)
				kst_esito.esito = kkg_esito.db_ko
				kguo_exception.set_esito(kst_esito)
				throw kguo_exception
			end if
		...
		if isnull(kst_tab_wm_pklist.note_lotto) then 
			kst_tab_wm_pklist.note_lotto = ""
		end if
...
return trim(kst_tab_wm_pklist.note_lotto)
...
st_esito kst_esito
	...
	if kst_tab_wm_pklist.id_wm_pklist > 0 then
		...
		select note 
		     into :kst_tab_wm_pklist_appo.note
		       from wm_pklist
				WHERE id_wm_pklist = :kst_tab_wm_pklist.id_wm_pklist
				using sqlca;
		...
		if trim(kst_tab_wm_pklist_appo.note) > " " then 
			kst_tab_wm_pklist_appo.note += "; " + trim(kst_tab_wm_pklist.note)
		else
			kst_tab_wm_pklist_appo.note = trim(kst_tab_wm_pklist.note)
		end if

		k_lencolmax = kguo_sqlca_db_magazzino.u_get_col_len( "wm_pklist", "note") //get max size colonna su db
		if k_lencolmax > 0 and len(trim(kst_tab_wm_pklist_appo.note)) > k_lencolmax then
			kst_tab_wm_pklist_appo.note = right(kst_tab_wm_pklist_appo.note, k_lencolmax)
		end if
...
public function boolean set_eliminato (ref st_tab_wm_pklist kst_tab_wm_pklist);//
//====================================================================
//=== Imposta e ELIMINATO il rek dalla tabella wm_pklist 
//=== 
//=== Ritorna TRUE = campo eliminato settato
//===           	
//====================================================================
//
boolean k_return
...
try
	...
	if kst_tab_wm_pklist.id_wm_pklist > 0 then
		...
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in Cancellazione elettronica (eliminato) del Packing List (wm_pklist): " &
...
try
	...
	if kst_tab_wm_pklist.id_wm_pklist > 0 then
		...
		delete from wm_pklist 
			where id_wm_pklist = :kst_tab_wm_pklist.id_wm_pklist
			using kguo_sqlca_db_magazzino ;
		
		if kguo_sqlca_db_magazzino.sqlcode <> 0 then
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in Cancellazione del Packing List (wm_pklist), id " &
										+ string(kst_tab_wm_pklist.id_wm_pklist) &
										+ ". Errore: " + trim(kguo_sqlca_db_magazzino.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
		end if
