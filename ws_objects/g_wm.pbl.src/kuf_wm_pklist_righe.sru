...
public function boolean set_eliminato (ref st_tab_wm_pklist_righe kst_tab_wm_pklist_righe) throws uo_exception
public function boolean tb_delete (ref st_tab_wm_pklist_righe kst_tab_wm_pklist_righe) throws uo_exception
end prototypes
...
if not k_return then
	...
	"Errore durante la Modifica Stato  a 'Importato' di tutte le righe del Packing-List entrato ~n~r" &
...
public function boolean set_stato_importato_x_id_wm_pklist (ref st_tab_wm_pklist_righe kst_tab_wm_pklist_righe) throws uo_exception;//
//====================================================================
//=== Cambia lo STATO di tutte le righe del Packing List 
//=== 
//=== Input: st_tab_wm_pklist_righe.id_wm_pklist_riga 
//=== Ritorna: TRUE = ok stato modificato
//=== 
//====================================================================
//
boolean k_return=false
st_esito kst_esito
	...
	if kst_tab_wm_pklist_righe.id_wm_pklist > 0 then
		...
		if sqlca.sqlcode < 0 then
			...
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = &
	"Errore durante la Modifica Stato  a 'Importato' di tutte le Riga di Packing-List entrato " &
					+ "id=" + string(kst_tab_wm_pklist_righe.id_wm_pklist, "####0") + " " &
					+ "~n~rErrore-tab.'wm_pklist':"	+ trim(sqlca.SQLErrText)
			if kst_tab_wm_pklist_righe.st_tab_g_0.esegui_commit <> "N" or isnull(kst_tab_wm_pklist_righe.st_tab_g_0.esegui_commit) then
				kguo_sqlca_db_magazzino.db_rollback( )
...
try
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in lettura Riga del Lotto (id) dalle righe del 'Packing List' cliente (wm_pklist_righe): " + "~n~Id lotto: " + string(kst_tab_wm_pklist_righe.id_meca) + ", barcode cliente: "  + trim(kst_tab_wm_pklist_righe.wm_barcode) &
...
public function boolean set_eliminato (ref st_tab_wm_pklist_righe kst_tab_wm_pklist_righe) throws uo_exception;//
//====================================================================
//=== Imposta ELIMINATO a tutte le righe del Packing List 
//=== 
//=== Input: st_tab_wm_pklist_righe.id_wm_pklist_riga 
//=== Ritorna: TRUE = ok modifica effettuata
//=== 
//====================================================================
//
boolean k_return
...
try
	...
	if kst_tab_wm_pklist_righe.id_wm_pklist > 0 then
		...
		update wm_pklist_righe
				set eliminato = :kki_Eliminato_si
					,x_datins_elim = :kst_tab_wm_pklist_righe.x_datins
					,x_utente_elim = :kst_tab_wm_pklist_righe.x_utente
					,x_datins = :kst_tab_wm_pklist_righe.x_datins
					,x_utente = :kst_tab_wm_pklist_righe.x_utente
				WHERE id_wm_pklist = :kst_tab_wm_pklist_righe.id_wm_pklist
				using kguo_sqlca_db_magazzino ;
		...
		if sqlca.sqlcode < 0 then
			...
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = &
	"Errore durante la cancellazione elettronica (riga a ELIMINATO) di tutte le righe del Packing-List entrato " &
					+ "id=" + string(kst_tab_wm_pklist_righe.id_wm_pklist, "####0") + " " &
					+ "~n~rErrore-tab.'wm_pklist_righe':"	+ trim(sqlca.SQLErrText)
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
...
public function boolean tb_delete (ref st_tab_wm_pklist_righe kst_tab_wm_pklist_righe) throws uo_exception;//
//====================================================================
//=== Cancella le righe in tabella wm_pklist_righe
//=== 
//=== Ritorna ST_ESITO
//===           	
//====================================================================
	...
	if kst_tab_wm_pklist_righe.id_wm_pklist > 0 then
		...
		if kguo_sqlca_db_magazzino.sqlcode <> 0 then
			...
			kst_esito.SQLErrText = "Errore in Cancellazione del Packing List (wm_pklist), id " &
										+ string(kst_tab_wm_pklist_righe.id_wm_pklist) &
