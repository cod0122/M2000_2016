...
public function integer set_barcode_in_wm_receiptgammarad (ref st_tab_wm_pklist_righe ast_tab_wm_pklist_righe_arg) throws uo_exception
end prototypes
...
//--- Note Lotto compilate dal cliente
			if len(trim(kst_tab_wm_receiptgammarad[k_ind].note_1)) > 0 then
				kst_wm_pklist.st_tab_wm_pklist.note_lotto = trim(kst_tab_wm_receiptgammarad[k_ind].note_1) + trim(kst_tab_wm_receiptgammarad[k_ind].note_2) + trim(kst_tab_wm_receiptgammarad[k_ind].note_3)
				kst_wm_pklist.st_tab_wm_pklist.note_lotto = kuf1_utility.u_stringa_spezza(kst_wm_pklist.st_tab_wm_pklist.note_lotto)  // spezza le parole troppo lunghe per permettere il a capo
			end if
...
try
	...
	if kst_tab_wm_pklist.idpkl > " " and kst_tab_wm_pklist.id_wm_pklist > 0 then
		...
		kst_tab_wm_pklist.note_aco = kuf1_wm_pklist_testa.get_note_aco_build(kst_tab_wm_pklist)
		kuf1_wm_pklist_testa.set_note_aco(kst_tab_wm_pklist)
...
//---	out: 
//---	rit: nr di barcode aggiornati
//---   Lancia EXCEPTION se errore
//------------------------------------------------------------------------------------------------------------------------------------
//
int k_return
long  k_ind, k_nr_barcode, k_nr_barcode_pkl
int k_rcn
st_esito kst_esito
...
//--- Piglio il ID_WM_PKLIST
	kst_tab_wm_pklist_righe.id_meca = ast_tab_wm_pklist_righe_arg.id_meca
...
//--- Piglio elenco Barcode da ID_MECA
	...
	//--- Piglio elenco ID da PKLIST grezzo
			...
			k_nr_barcode_pkl = upperbound(kst_tab_wm_receiptgammarad[])
			if k_nr_barcode_pkl > 0 then
				if k_nr_barcode < k_nr_barcode_pkl then
					kst_esito.esito = kkg_esito.err_logico
					kst_esito.sqlerrtext = "Numero Barcode prodotti (" + string(k_nr_barcode) & 
								+ ") inferiore al nr. Colli in Packing List ~n~rAggiornamento di " + string(k_nr_barcode_pkl) + " colli eseguito comunque! "
				else			
					if k_nr_barcode > k_nr_barcode_pkl then
						kst_esito.esito = kkg_esito.err_logico
						kst_esito.sqlerrtext = "Numero Barcode prodotti (" + string(k_nr_barcode) &
								+ ") superiore al nr. Colli in Packing List ~n~rAggiornamento di " + string(k_nr_barcode_pkl) + " colli eseguito comunque! "
					end if
				end if
			end if
			if k_nr_barcode_pkl = k_nr_barcode then
				
				for k_ind = 1 to k_nr_barcode_pkl
...
//	end if
	...
	if isvalid(kuf1_wm_receiptgammarad) then destroy kuf1_wm_receiptgammarad
	if isvalid(kuf_wm_pklist_righe) then destroy kuf1_wm_pklist_righe	
	if isvalid(kuf_barcode) then destroy kuf1_barcode	
