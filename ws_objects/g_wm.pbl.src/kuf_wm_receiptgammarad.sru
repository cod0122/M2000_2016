...
//--- datastore che serve SOLO per IMPORTARE le note_1/3 
	...
	if kds_wm_receiptgammarad.rowcount( ) > 0 then
		...
		for k_ind = 1 to  kds_wm_receiptgammarad.rowcount( )
			...
			kst_tab_wm_receiptgammarad[k_ind].barcodewo = kds_wm_receiptgammarad.getitemstring( k_ind, "barcodewo") 
			...
			if kds_wm_receiptgammarad_note_lotto.rowcount( ) = 0 then
				...
				kst_tab_wm_receiptgammarad[k_ind].note_1 = kds_wm_receiptgammarad_note_lotto.getitemstring( 1, "note_1")  // Note del Lotto 1/2/3 trimmate
			end if
...
if isnull(kst_tab_wm_receiptgammarad.barcodewo ) then kst_tab_wm_receiptgammarad.barcodewo = ""
...
//--- Cancellazione
		case kkg_flag_modalita.cancellazione
			...
			for k_index = 1 to upperbound(kst_tab_wm_receiptgammarad)   	
				...
				//28062021if len(trim(kst_tab_wm_receiptgammarad[k_index].packinglistcode)) > 0 then
				if kst_tab_wm_receiptgammarad[k_index].idwmpklist > 0 then
					//this.tb_delete(kst_tab_wm_receiptgammarad[k_index])
					try
						this.tb_delete_x_idwmpklist(kst_tab_wm_receiptgammarad[k_index])
					catch (uo_exception kuo_exception) 
						kuo_exception.messaggio_utente( )
					end try
				end if
...
//------------------------------------------------------------------------------------------------------------------------------------
//---
//---	Torna ID del IDWMPKLIST (ID del PKL caricato su db) indicato 
...
try
	...
	tb_delete_x_packinglistcode(kst_tab_wm_receiptgammarad)
...
//--- da qui purtroppo il TXT può avere un formato diverso, quindi vado per campi separati da '|' il pipe				
				...
				kst_tab_wm_receiptgammarad[k_righe].barcodeWO = ""
...
//				kds_1.setitem(k_riga_ds, "area_mag_trim", kst_tab_wm_receiptgammarad[k_riga].area_mag_trim) 
			...
			//	kds_1.setitem(k_riga_ds, "registered", kst_tab_wm_receiptgammarad[k_riga].registered) 
				...
				kds_1.setitem(k_riga_ds, "barcodewo", kst_tab_wm_receiptgammarad[k_riga].barcodeWO)
