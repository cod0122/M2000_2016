...
public function boolean get_nome_oggetto_da_window_oggetti_old (ref st_tab_menu_window_oggetti kst_tab_menu_window_oggetti)
end prototypes
...
integer k_ctr=0, k_ctr_prec=0, k_rows, k_row, k_rc, k_max_tab_menu_window
string k_utente
long k_id_utente, k_righe_settore, k_riga_find
st_esito kst_esito
st_tab_menu_window kst_tab_menu_window[300]
st_tab_menu_window_oggetti kst_tab_menu_window_oggetti
datastore kds_get_sr_settore, kds_menu_window
...
try 
	...
	kds_menu_window = create datastore
	choose case k_utente
			
		case "*VUOTO*"   // solo x l'utente che accede inizialmente: AGGIUNGERE A MANO LE FUNZIONI NECESSARIE 
			kds_menu_window.dataobject = "ds_menu_window_x_id_vuoto"  
			kds_menu_window.settransobject( kguo_sqlca_db_magazzino )
			k_rows = kds_menu_window.retrieve( )
	//		k_cursore = &
	//			" select distinct " + &
	//			"   trim(menu_window.id), " + &
	...
	//			" order by 1  " 
		...
		case "MASTER"  // è il mio utente per avere tutto Autorizzzato
			kds_menu_window.dataobject = "ds_menu_window_all"  
			kds_menu_window.settransobject( kguo_sqlca_db_magazzino )
			k_rows = kds_menu_window.retrieve( )
	//		k_cursore = &
	//			" select distinct " + &
	...
	//			" order by 1  " 
		...
		case else
			kds_menu_window.dataobject = "ds_menu_window_x_cod_utente"  
			kds_menu_window.settransobject( kguo_sqlca_db_magazzino )
			k_rows = kds_menu_window.retrieve(k_utente)
	//		k_cursore = &
	//			" select distinct " + &
	//			"   trim(menu_window.id), " + &
	//			"	 trim(menu_window.window), " + &
	...
	if k_rows > 0 then
		...
		if k_rows < 0 then
			...
			kst_esito.sqlerrtext = "Errore durante lettura delle Funzioni disponibili in '" + string(kds_menu_window.dataobject) + "' " &
						      	+ "Utente: " + k_utente + " Esito: " + string(k_rows) &
									+ " Parametri db: " + trim(sqlca.dbparm) + "; "  
		else
			kst_esito.esito = kkg_esito.not_fnd
			kst_esito.sqlerrtext = "Nessuna Funzione disponibile in '" + string(kds_menu_window.dataobject) + "' " &
						      	+ "Utente: " + k_utente + " Esito: " + string(k_rows) &
								   + " Parametri db: " + trim(sqlca.dbparm) + "; " 
		end if
	...
	k_max_tab_menu_window = UpperBound(kGst_tab_menu_window)
	k_ctr_prec = 0
	for k_row = 1 to k_rows 

		if k_ctr < k_max_tab_menu_window then
		else
			kst_esito.esito = kkg_esito.ko
			kst_esito.sqlerrtext = "Troppe Funzioni definite " + string(k_rows)+ "!! Allargare la tabella. " 
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		end if
	...
	//	fetch	kc_menu_window
		k_ctr = k_ctr_prec + 1
		kGst_tab_menu_window[k_ctr].id = kds_menu_window.getitemstring( k_row, "id")
		kGst_tab_menu_window[k_ctr].window = kds_menu_window.getitemstring( k_row, "window")
		kGst_tab_menu_window[k_ctr].istanze = kds_menu_window.getitemnumber( k_row, "istanze")
		kGst_tab_menu_window[k_ctr].salva_size = kds_menu_window.getitemstring( k_row, "salva_size")
		kGst_tab_menu_window[k_ctr].salva_controlli = kds_menu_window.getitemstring( k_row, "salva_controlli")
		kGst_tab_menu_window[k_ctr].min_w_prec = kds_menu_window.getitemstring( k_row, "min_w_prec")
		kGst_tab_menu_window[k_ctr].titolo = kds_menu_window.getitemstring( k_row, "titolo")
		kGst_tab_menu_window[k_ctr].funzioni = kds_menu_window.getitemstring( k_row, "funzioni")
		kGst_tab_menu_window[k_ctr].batch = kds_menu_window.getitemstring( k_row, "batch")
		kGst_tab_menu_window[k_ctr].msg_se_gia_open = kds_menu_window.getitemstring( k_row, "msg_se_gia_open")
		kGst_tab_menu_window[k_ctr].primopiano = kds_menu_window.getitemstring( k_row, "primopiano")
		kGst_tab_menu_window[k_ctr].nome_id_tabella = kds_menu_window.getitemstring( k_row, "nome_id_tabella")
		kGst_tab_menu_window[k_ctr].nome_oggetto = kds_menu_window.getitemstring( k_row, "nome_oggetto") // get del nome Oggetto (kuf_....)
...
//--- Carica la tabella  "Anteprime+id campo tabella"  x chiamare le funzioni dallo zoom
	...
	kuo_exception.scrivi_log(kuo_exception.get_st_esito())
	
finally
	if isvalid(kds_get_sr_settore) then destroy kds_get_sr_settore
	if isvalid(kds_menu_window) then destroy kds_menu_window
...
//--- Se trovo i caratteri 'ivmc' s'intendono le funzioni di +in+vi+mo+ca posso aggiungere anche 'es' ovvero +el+st e anche 'a' ovvero +an
				choose case trim(k_funzione)
					...
					case "dp"   // solo Duplica (è come inserimento)
						k_funzione_composta = "+in"
					...
					case "vesac"   // CONSULTAZIONE + cancellazione
						k_funzione_composta = "+el+st+an+vi+qy+ca"
					case "mb"   // solo Modifica e batch
						k_funzione_composta = "+mo+bt"
