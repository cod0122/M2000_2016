...
private subroutine set_window_chiamante (ref st_open_w kst_open_w)
private subroutine u_suona_motivo (st_open_w kst_open_w)
private function string u_open_window (integer k_index)
public function string get_descr (string a_menu_id)
end prototypes
...
string k_funzione
 	...
 	kst_tab_menu_window_oggetti.nome_oggetto = trim(kst_tab_menu_window_oggetti.nome_oggetto)

	if kst_tab_menu_window_oggetti.nome_oggetto > " " then
		
		k_funzione = kiuf_sicurezza.get_funzione_master(trim(kst_tab_menu_window_oggetti.funzione))
		...
		do while upperbound(kguo_g.kGst_tab_menu_window_oggetti[]) >= k_ctr and k_pos = 0 
			...
			if kguo_g.kGst_tab_menu_window_oggetti[k_ctr].id > 0 then
				if trim(kguo_g.kGst_tab_menu_window_oggetti[k_ctr].nome_oggetto) = trim(kst_tab_menu_window_oggetti.nome_oggetto) then
					...
					k_pos = pos(trim(kguo_g.kGst_tab_menu_window_oggetti[k_ctr].funzione), k_funzione, 1) 
...
//--- recupera il riferimento alla windows chiamante
			set_window_chiamante(kst_open_w)
...
//--- Per window gia' aperta controllo SE posso aprire più istanze e faccio la richiesta su farlo o tornare alla window gia' aperta
							if upper(kGst_tab_menu_window[k_ctr].msg_se_gia_open) = kki_msg_se_gia_open_esponi then
								k_rc = messagebox( "Funzione già aperta", "Premere 'SI' per aprire una nuova finestra, per rimanere sulla precedente premere 'No' ", question!, yesno!, 2 )
								if k_rc = 2 then
									k_open_window = false
								end if
							end if
...
private subroutine set_window_chiamante (ref st_open_w kst_open_w);//
//--- imposta i campi: kst_open_w.key10_window_chiamante
//---                e kst_open_w.id_programma_chiamante (se manca)
//
w_super kw_window_attiva
...
private function string u_open_window (integer k_index);//
string k_rc_open
w_super kw_prima_window_aperta
w_super kw_window_open


	setpointer(hourglass!)

	choose case kGst_tab_menu_window[k_index].primopiano
...
//--- A = APRO WINDOW PARTICOLARE TIPO CAERCA/STAMPA/....k_ctr
		case "A"
			kGst_tab_menu_window[k_index].primopiano = "A" 
			kw_prima_window_aperta = kguo_g.window_aperta_get_x_primopiano(kGst_tab_menu_window[k_index].primopiano)
			if isnull(kw_prima_window_aperta) then
...
//								k_rc_open =  string(OpenSheetWithParmAsDocument(kw_window_open, kguo_g.kGst_open_w, kguo_g.kGst_open_w.window, w_main, k_w_name, true ))
				...
				k_rc_open =  string(OpenSheetWithParmInTabGroup(kw_window_open, kguo_g.kGst_open_w, kguo_g.kGst_open_w.window, kw_prima_window_aperta, kguo_g.kGst_open_w.window ))
...
//--- M = APRO WINDOW MENU PER I FATTI SUOI (es. toolbar di menu)
		case "M"
			kGst_tab_menu_window[k_index].primopiano = "M" 
			kw_prima_window_aperta = kguo_g.window_aperta_get_x_primopiano(kGst_tab_menu_window[k_index].primopiano)
			if isnull(kw_prima_window_aperta) then
...
//--- APRO TUTTE LE WINDOW NORMALI
		case else
			kGst_tab_menu_window[k_index].primopiano = "N" 
			kw_prima_window_aperta = kguo_g.window_aperta_get_x_primopiano(kGst_tab_menu_window[k_index].primopiano)
...
//								if k_nr_opened = 0 then // prima window da aprire
				...
				k_rc_open =  string(OpenSheetWithParmAsDocument(kw_window_open, kguo_g.kGst_open_w, kguo_g.kGst_open_w.window, kw_prima_window_aperta, kguo_g.kGst_open_w.window ))
			end if
...
long k_ctr, k_ctr_idx
	...
	do while k_ctr <= k_ctr_idx 
		if a_menu_id = (trim(kGst_tab_menu_window[k_ctr].id)) then
			k_return = trim(kGst_tab_menu_window[k_ctr].descr)
			exit
		end if
		k_ctr++
	loop
