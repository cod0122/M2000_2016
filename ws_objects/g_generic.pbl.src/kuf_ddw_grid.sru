...
long		ll_findrow
//long k_ufo
int k_len_ls_foundtext, k_limit
...
If lb_matchfound Then
	...
	// limita le lunghezze al max
	k_limit = integer(idw.Describe(ls_dddw_displaycol + ".Edit.Limit"))
	k_len_ls_foundtext = len(ls_foundtext)
	if k_limit > 0 and k_limit < k_len_ls_foundtext then
		k_len_ls_foundtext = k_limit - 1
	// Set the text.
		idw.SetText (left(ls_foundtext, k_len_ls_foundtext))
	else
	// Set the text.
		idw.SetText (ls_foundtext)
	end if
	if k_limit > 0 and k_limit < li_searchtextlen then
		li_searchtextlen = k_limit - 1
	end if

	// Determine what to highlight or where to move the cursor..
	if li_searchtextlen = k_len_ls_foundtext THEN
		// Move the cursor to the end
		idw.SelectText (k_len_ls_foundtext+1, 0)
	else
		// Hightlight the portion the user has not actually typed.
		idw.SelectText (li_searchtextlen + 1, k_len_ls_foundtext)
	end if
end if
