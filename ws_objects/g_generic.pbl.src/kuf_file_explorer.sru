...
if trim(a_src) = trim(a_dst) then
		kguo_exception.kist_esito.sqlerrtext = "Il nome del file sorgente uguale a quello di destinazione '" &
					+ trim(a_src) + "' operazione non necessaria!"
else

	if not a_replace then
		if FileExists(a_dst) then
			kguo_exception.kist_esito.sqlerrtext = "Il file di destinazione '" &
						+ trim(a_dst) + "' esiste già!"
			k_return = -2
		end if
	end if

	if k_return = 0 then
		if not FileExists(a_src) then
...
public function boolean u_directory_create (string k_path);//---
//--- Creazione del path indicato (crea anche l'intero percorso) se non esiste
//---
...
// verifica come è stato passato il path con o senza barra
if right(trim(a_path), 1) = kkg.path_sep then
	path = a_path + a_search
else
...
public function integer u_get_n_list_file (string k_path, string k_file_search) throws uo_exception;//
//--- Ricava il numero di file della tipologia indicata
//--- Inp: k_path, k_search   es: c:\pippo *.pdf 
//--- Ret: n. file trovati
//
int k_return
datastore kds_elenco_file
...
try
	...
	if k_path > " " then
		...
		kds_elenco_file = dirlist_path_search(k_path, k_file_search)

		k_return = kds_elenco_file.rowcount( )
	end if
	...
	if isvalid(kds_elenco_file) then destroy kds_elenco_file	
...
public function longlong u_get_filesize (string a_file);//
//--- Torna la dimensione del file
//
longlong k_return
st_file_explorer_win32_find_data kst_file_explorer_win32_find_data


 //es. ("C:\Program Files\Sybase\PowerBuilder 9.0\pb90.exe", kst_file_explorer_win32_find_data)
if FindFirstFile(a_file, kst_file_explorer_win32_find_data) > 0 then
	
	k_return = kst_file_explorer_win32_find_data.filesizelow
...
lnvos_shellexecuteinfo.fMask = SEE_MASK_CLASSNAME  // Use classname
lnvos_shellexecuteinfo.hwnd = 0
lnvos_shellexecuteinfo.lpVerb = "edit"
lnvos_shellexecuteinfo.lpfile = as_file
lnvos_shellexecuteinfo.lpClass = ls_class
lnvos_shellexecuteinfo.nShow = SW_NORMAL
