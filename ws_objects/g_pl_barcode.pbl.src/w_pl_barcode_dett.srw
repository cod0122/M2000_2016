...
private constant int kki_impianto = kiuf_impianto.kki_impiantog2 //G2
...
private function long u_dw_barcode_retrieve (readonly long a_riga_meca)
...
choose case k_nome_controllo
	...
	case dw_meca.dataobject //"d_pl_barcode_meca_no_lav_l"
...
try
	...
	kst_tab_pl_barcode.impianto = kuf1_impianto.kki_impiantog2
...
//	else
		...
		dw_meca.retrieve(ki_data_ini, k_pl_barcode)  //--- leggo su DB
...
private function st_esito retrieve_padri ();//
//---- Rilegge tutti i padri da db contenuti nel dw_lista_0
//
long k_riga
st_tab_barcode kst_tab_barcode
...
//--- Controllo programmazione
	kiuf_pl_barcode.if_pianificazione_g2_ok(kds_pl_barcode_dett)
...
//--- controlla pianificazione figli
	kiuf_pl_barcode.if_pianificazione_figli_ok(kds_pl_barcode_dett, kds_pl_barcode_dett_grp, "inserimento")
...
if kds_1.rowcount() > 0 then
//--- se non sono trascorsi più di 30 minuti (1800 sec)
	if date(k_data_backup) = today() and  SecondsAfter ( time(k_data_backup), now() ) < 1800 then
		if messagebox("La Programmazione si è chiusa in modo inaspettato", "Trovato un Piano di Lavorazione da Ripristinare delle ore " &
...
public function long togli_barcode_figlio (boolean k_esponi_msg);/*
 ...
 inp: esporre messaggio di avviso
 out: id_meca ultimo tolto
*/
...
try
	...
	do while k_riga_drag > 0
		...
		if dw_groupage.getitemnumber(k_riga_drag, "barcode_fila_1") <> kst_tab_barcode.fila_1 &
				or dw_groupage.getitemnumber(k_riga_drag, "barcode_fila_2") <> kst_tab_barcode.fila_2 &
			   or dw_groupage.getitemnumber(k_riga_drag, "barcode_fila_1p") <> kst_tab_barcode.fila_1p &
				or dw_groupage.getitemnumber(k_riga_drag, "barcode_fila_2p") <> kst_tab_barcode.fila_2p &
				or dw_groupage.getitemstring(k_riga_drag, "barcode_tipo_cicli") <> kst_tab_barcode.tipo_cicli &
				then
			...
			if kst_esito.esito <> "0" then
				...
				dw_groupage.setitem(k_riga_drag, "barcode_tipo_cicli", kst_tab_barcode.tipo_cicli)			
				dw_groupage.setitem(k_riga_drag, "barcode_fila_1", kst_tab_barcode.fila_1)			
				dw_groupage.setitem(k_riga_drag, "barcode_fila_2", kst_tab_barcode.fila_2)			
				dw_groupage.setitem(k_riga_drag, "barcode_fila_1p", kst_tab_barcode.fila_1p)			
				dw_groupage.setitem(k_riga_drag, "barcode_fila_2p", kst_tab_barcode.fila_2p)	
...
//--- verifica se il Padre ha ancora figli se non e' cosi' lo resetta
		...
		k_id_meca = dw_groupage.getitemnumber(k_riga_drag, "id_meca")
...
//--- copia la dw1 in barcode, il formato e' la solito dettaglio			
		...
		k_riga_drag = dw_groupage.getselectedrow( k_riga_drag )
...
//--- conteggio righe da ripristinare anche i giri
	...
	do while k_riga_drag > 0
		...
		kst_esito = kiuf_barcode.tb_prendi_campo( kst_tab_barcode, "fila_1_e_fila_2" ) 
		if kst_esito.esito = kkg_esito.ok then
			if dw_lista_0.getitemnumber(k_riga_drag, "barcode_fila_1") <> kst_tab_barcode.fila_1 &
					or dw_lista_0.getitemnumber(k_riga_drag, "barcode_fila_2") <> kst_tab_barcode.fila_2 &
					or dw_lista_0.getitemnumber(k_riga_drag, "barcode_fila_1p") <> kst_tab_barcode.fila_1p &
					or dw_lista_0.getitemnumber(k_riga_drag, "barcode_fila_2p") <> kst_tab_barcode.fila_2p &
					or dw_lista_0.getitemstring(k_riga_drag, "barcode_tipo_cicli") <> kst_tab_barcode.tipo_cicli &
				then
				k_nrows_to_restore ++
			end if
		else
		...
		k_riga_drag = dw_lista_0.getselectedrow( k_riga_drag )
	loop
	...
	do while k_riga_drag > 0
		...
		if k_nrows_to_restore > 1 then
			...
			dw_lista_0.setitem(k_riga_drag, "barcode_tipo_cicli", kst_tab_barcode.tipo_cicli)			
			dw_lista_0.setitem(k_riga_drag, "barcode_fila_1", kst_tab_barcode.fila_1)			
			dw_lista_0.setitem(k_riga_drag, "barcode_fila_2", kst_tab_barcode.fila_2)			
			dw_lista_0.setitem(k_riga_drag, "barcode_fila_1p", kst_tab_barcode.fila_1p)			
			dw_lista_0.setitem(k_riga_drag, "barcode_fila_2p", kst_tab_barcode.fila_2p)	
...
public subroutine togli_barcode_figlio_post (long k_id_meca);//
//=== Da chiamare dopo il: Toglie i BARCODE FIGLI selezionati della lista dei Pianificati
//===  
//
int k_rc
long k_riga_meca
...
this.Control[iCurrent+7]=this.dw_prev
this.Control[iCurrent+8]=this.dw_pilota_inlav
this.Control[iCurrent+9]=this.dw_meca
end on
...
string dataobject = "d_pl_barcode_barcode_no_pl_l"
boolean minbox = true
boolean maxbox = true
boolean resizable = true
boolean hsplitscroll = false
...
//   	kdw_1 = source
		...
		elseif kdw_source.DataObject = "d_pl_barcode_meca_no_lav_l" then
...
event u_selectrow_false_all ( string a_num_fila )
...
string dataobject = "d_pl_barcode_meca_no_lav_l"
boolean minbox = true
boolean maxbox = true
boolean resizable = true
...
int k_nrowsel
	...
	k_nrowsel = this.getItemnumber ( 1 , "knrow_choosed_" + a_num_fila )
	if k_nrowsel > 0 then
	
		k_rows = this.rowcount()
		for k_row = 1 to k_rows
			if this.getitemnumber(k_row, "k_choose_" + a_num_fila) = 1 then
				k_ctr ++
				this.event u_selectrow_false(k_row, a_num_fila)
...
if left(trim(dwo.name), 15) = "k_choose_clear_" then
	event u_selectrow_false_all(trim(right(trim(dwo.name), len(trim(dwo.name)) - 15)))
end if
...
//		if ki_dragdrop then
			...
			CHOOSE CASE kdw_source.DataObject 
				case "d_pl_barcode_dett_1"
					togli_barcode_padre()
				case "d_pl_barcode_dett_grp_all"
					k_id_meca = togli_barcode_figlio(true)
					togli_barcode_figlio_post(k_id_meca)
				case "d_barcode_l"
					togli_dettaglio()
//					
//				case else
