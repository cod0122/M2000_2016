...
private subroutine import_end_work_pilota_g3_e1 (long a_riga_pallet, ref ds_pilota_pallet_out_g3 ads_pilota_pallet_out_g3, ref st_tab_barcode kst_tab_barcode, ref kuf_e1_wo_f5548014 kuf1_e1_wo_f5548014, ref kuf_barcode kuf1_barcode, ref st_tab_meca ast_tab_meca) throws uo_exception
...
public function integer import_start_work_pilota_g3 (integer a_gg_less, long a_work_order) throws uo_exception
private function integer import_start_work_pilota_g3_1 (ref ds_pilota_pallet_in_g3 ads_pilota_pallet_in_g3) throws uo_exception
public function integer import_end_work_pilota_g3 (integer a_gg_less, long a_work_order) throws uo_exception
private function integer import_end_work_pilota_g3_1 (ds_pilota_pallet_out_g3 ads_pilota_pallet_out_g3, kuf_barcode auf_barcode) throws uo_exception
end prototypes
...
try 
	...
	k_ctr = this.import_start_work_pilota_g3(30, 0) // tutto per gli utlimi 30 giorni
	k_ctr1 = this.import_end_work_pilota_g3(0, 0) // per data di default 
...
public function integer import_start_work_pilota_g3 (integer a_gg_less, long a_work_order) throws uo_exception;/*
 ...
 Inp: num.giorni da prendere in considerazione se zero = 30  oppure il Worn Order di un preciso Lotto
 rit: Numero di righe aggiornate
 */
long k_return, k_ctr=0, k_rows=0
date k_date_START
...
try
	...
	if a_work_order > 0 then
	else
		a_work_order = 0 
		choose case true
			case a_gg_less > 0
				a_gg_less = -1 * a_gg_less
			case isnull(a_gg_less)
				a_gg_less = -30
		end choose
		k_date_START = relativedate(kguo_g.get_dataoggi( ), a_gg_less)
	end if
		
	k_rows = kds_pilota_pallet_in_g3.u_retrieve(k_date_START, a_work_order)  // leggi dal PILOTA i Barcode da importare

	if k_rows = 0 then 
		if a_work_order > 0 then
			kguo_exception.kist_esito.SQLErrText = "Nessun Lotto da Importare dall'Impianto G3 per il WO n. " + string(a_work_order)
		else
			kguo_exception.kist_esito.SQLErrText = "Nessun Lotto da Importare dall'Impianto G3, dal " + string(k_date_START, "dd mmm yy") 
		end if
		throw kguo_exception
	end if

// IMPORTA DATI PILOTA
	k_return = import_start_work_pilota_g3_1(kds_pilota_pallet_in_g3)
...
private function integer import_start_work_pilota_g3_1 (ref ds_pilota_pallet_in_g3 ads_pilota_pallet_in_g3) throws uo_exception;/*
 ...
 Inp: ds_pilota_pallet_in_g3  	// Connessione al DB PILOTA IMPIANTO G3
 Out: Numero di righe aggiornate
 */
long k_return, k_ctr=0, k_rows, k_row_pallet, k_rc
date k_datainizioanno
int k_giorniafter, k_anno, k_anno_rid
...
try
	...
	for k_row_pallet = 1 to k_rows
		
//--- popola struttura da datastore elenco PALLET con data FINE LAVORAZIONE
//		kst_tab_pilota_pallet.Data_Entrata = ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "Data_Entrata")   
//		kst_tab_pilota_pallet.data_scarico = ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "data_scarico")
//		kst_tab_pilota_pallet.Pallet_Code = ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "barcode") 
//		kst_tab_pilota_pallet. = ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "F1AVP")
//		kst_tab_pilota_pallet.F2AVP = ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "F2AVP")
//		kst_tab_pilota_pallet.F1APP = ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "F1APP")
//		kst_tab_pilota_pallet.F2APP = ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "F2APP")
//		kst_tab_pilota_pallet.Posizione = ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "Posizione")
//		kst_tab_pilota_pallet.Bilancella = ads_pilota_pallet_in_g3.getitemnumber(k_row_pallet, "Bilancella")
...
//--- piccolo lasso di tempo a favore di altre cose usato x  tasto 'interrompi'
		...
		kst_tab_barcode.barcode = trim(ads_pilota_pallet_in_g3.getitemstring(k_row_pallet, "barcode"))
...
//--- se già messo in lavorazione evito di ri-aggiornare
	...
	//--- verifica se è il primo/ultimo barcode che finisce il trattamento e lo salva in e1_wo_f5548014 per poi comunicarlo a E1
					...
					if k_e1_enabled then
						...
						if kst_tab_e1_wo_f5548014.wo_osdoco > 0 then
							kst_tab_e1_wo_f5548014.data_osa801 = string(ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "data_carico"), "dd/mm/yy")
							k_anno = integer(string(ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "data_carico"), "yyyy"))
							k_anno_rid = integer(string(ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "data_carico"), "yy"))
							k_datainizioanno = date(k_anno,01,01)
							k_giorniafter = DaysAfter(k_datainizioanno, date(ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "data_carico"))) + 1
							kst_tab_e1_wo_f5548014.data_osdee = 100000 + k_anno_rid * 1000 + k_giorniafter
							kst_tab_e1_wo_f5548014.ora_oswwaet = long(kGuf_data_base.get_e1_timeformat(ads_pilota_pallet_in_g3.getitemdatetime(k_row_pallet, "data_carico")))
							if kuf1_barcode.get_nr_barcode_lav_ini(kst_tab_barcode) = 0 then
								kst_tab_e1_wo_f5548014.flag_osev01 = kuf1_e1_wo_f5548014.kki_stato_ev01_firstload
								kst_tab_e1_wo_f5548014.tcicli_osmmcu = ""
				...
				//--- popola area TAB_BARCODE padre	
					u_set_tab_barcode(k_row_pallet, ads_pilota_pallet_in_g3, kst_tab_barcode)
...
//--- imposta i dati di trattamento uguali a quelli del PADRE		
						u_set_tab_barcode(k_row_pallet, ads_pilota_pallet_in_g3, kst_tab_barcode_figlio)
...
//--- COMMIT consolido su DB di Magazzino x E1
...
*/
long k_return=0, k_righe_pallet_tot
...
try
	...
	if a_work_order > 0 then
		...
		choose case true
			case a_gg_less > 0
				k_data_da = relativedate(kguo_g.get_dataoggi( ), (1* a_gg_less))
			case a_gg_less < 0
				k_data_da = relativedate(kguo_g.get_dataoggi( ), a_gg_less)
			case else
	...
	k_righe_pallet_tot = kds_pilota_pallet_out_g3.u_retrieve(k_data_da, a_work_order) // LETTURA PALLET CON DATA FINE LAV IMPOSTATA

	if k_righe_pallet_tot > 0 then
		
//--- Importa TRATTATI
		k_return = import_end_work_pilota_g3_1(kds_pilota_pallet_out_g3, kuf1_barcode)
...
private function integer import_end_work_pilota_g3_1 (ds_pilota_pallet_out_g3 ads_pilota_pallet_out_g3, kuf_barcode auf_barcode) throws uo_exception;/*
 Importa dati LAVORAZIONE dal Pilota e aggiorna le tabelle
    Inp: ds_pilota_pallet_out_g3   dati barcode da trattare
	      kuf_barcode 
	 chiamato dal import_end_work_pilota_g3
*/
long k_return=0, k_ctr, k_righe_pallet_tot=0, k_riga_pallet
...
//--- legge Barcode x prendere id armo e vedere se gia' lavorato
			auf_barcode.select_barcode(kst_tab_barcode)
...
//--- Carica i dati nella tabella di appoggio 'e1_wo_f5548014' per scambio con E1
				...
				if k_e1_enabled then
					...
					//--- carica dati in E1
					import_end_work_pilota_g3_e1(k_riga_pallet, ads_pilota_pallet_out_g3, kst_tab_barcode, kuf1_e1_wo_f5548014, auf_barcode, kst_tab_meca)
		...
		//--- verifica la lavorazione se ci sono anomalie
				auf_barcode.check_anomalie_lavorazione_g3(kst_tab_barcode)

		//--- get Tempi Impianto del BARCODE PADRE
				auf_barcode.get_imptime_second_g3(kst_tab_barcode)
			
		//--- Aggiorna gli archivi con i dati di Lavorazione ------------------------------------------------------
				kst_tab_barcode.st_tab_g_0.esegui_commit = "N" 
				auf_barcode.u_update_campo(kst_tab_barcode, "data_lav_ini_fin") //AGGIORNA TAB
			
		//--- se Anomalia....
				if kst_tab_barcode.err_lav_fin = auf_barcode.ki_err_lav_fin_ko then
					kst_tab_meca.id = kst_tab_barcode.id_meca 
	...
	//--- Piglia gli eventuali Figli
				kds_1 = auf_barcode.get_figli_barcode(kst_tab_barcode)

				for k_ctr = 1 to kds_1.rowcount( ) 
								
					kst_tab_barcode_figlio.barcode = kds_1.object.barcode[k_ctr]
//--- legge Barcode figlio
					auf_barcode.select_barcode(kst_tab_barcode_figlio)

//--- imposta i dati di trattamento uguali a quelli del PADRE
					u_set_tab_barcode(k_riga_pallet, ads_pilota_pallet_out_g3, kst_tab_barcode_figlio)

//--- verifica la lavorazione se ci sono anomalie
					auf_barcode.check_anomalie_lavorazione_g3(kst_tab_barcode_figlio)
...
//--- Carica i dati nella tabella di scambio per E1
					...
					if k_e1_enabled then
						...
						import_end_work_pilota_g3_e1(k_riga_pallet, ads_pilota_pallet_out_g3, kst_tab_barcode_figlio, kuf1_e1_wo_f5548014, auf_barcode, kst_tab_meca)

					end if
					
//--- get Tempi Impianto del BARCODE FIGLIO
					auf_barcode.get_imptime_second_g3(kst_tab_barcode_figlio)
