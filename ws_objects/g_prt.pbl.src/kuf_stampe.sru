...
private string ki_tab_nome_oggetto[100,7]
private constant int ki_tab_nome_oggetto_COLHEADER=1				// colonna di testata (..._t)
private constant int ki_tab_nome_oggetto_COLVISIBLE=2			// proprietà visible
private constant int ki_tab_nome_oggetto_COLNAMEDB=3			// nome colonna ripulita dal suffiso '_t' (dovrebbe essere del db)
private constant int ki_tab_nome_oggetto_COLNAMENODB=4	  	// nome colonna da compute (k_...) (non da db)
private constant int ki_tab_nome_oggetto_COLHEADERBAND=5		// banda del report dove si trova la colonna di testata
private constant int ki_tab_nome_oggetto_COLDBBAND=6			// banda del report dove si trova la colonna di db
private constant int ki_tab_nome_oggetto_COLNODBBAND=7		// banda del report dove si trova la colonna di NO db
...
public subroutine dw_print_testata_add ()
public function boolean dw_print_restore_orig ()
end prototypes
...
string k_section, k_rcx, k_visible, k_num 
...
//--- trova la larghezza orizzontale del datawindow
	...
	do while len(trim(ki_tab_nome_oggetto[k_riga, 1])) > 0  
		...
		if trim(kist_stampe_orig.dw_print.describe(k_nome_testata + ".visible")) = "!" then
			k_nome_testata = ""
		end if
		k_nome = trim(ki_tab_nome_oggetto[k_riga, 3])
		if trim(kist_stampe_orig.dw_print.describe(k_nome + ".visible")) = "!" then
			k_nome = ""
		end if
		k_knome = trim(ki_tab_nome_oggetto[k_riga, 4])
		if trim(kist_stampe_orig.dw_print.describe(k_knome + ".visible")) = "!" then
			k_knome = ""
		end if
...
//--- ripristina Proprieta' VISIBLE nel campo testo e colonna 
			...
			if kst_profilestring_ini.esito = "0" &
					and (trim(kst_profilestring_ini.valore) = "0" &
							or trim(kst_profilestring_ini.valore) = "1") then
				k_visible = trim(kst_profilestring_ini.valore)
			else
				k_visible = kist_stampe_orig.dw_print.describe(k_nome + ".visible") 
			end if
			k_str_modify += k_nome + ".visible = '" + k_visible + "' "
			
			if k_nome_testata > " " then
				k_str_modify += k_nome_testata + ".visible = '" + k_visible + "' "
			end if
...
//--- ripristina Proprieta' Width
			...
			k_rcx = trim(kGuf_data_base.profilestring_ini(kst_profilestring_ini))
			if kst_profilestring_ini.esito = "0" &
						and isnumber(trim(kst_profilestring_ini.valore)) then
				k_num = trim(kst_profilestring_ini.valore)
			else
				k_num = kist_stampe_orig.dw_print.describe(k_nome + ".Width") 
			end if
			k_str_modify += k_nome + ".Width = " + k_num + " "
...
//--- ripristina Proprieta' Height
			...
			if kst_profilestring_ini.esito = "0" &
				...
				k_num = kist_stampe_orig.dw_print.describe(k_nome + ".Height") 
			end if
			k_str_modify += k_nome + ".Height = " + k_num + " "
...
//--- ripristina Proprieta' Width
			...
			k_str_modify += k_knome + ".Width = " + k_num + " " 
	...
	//--- ripristina Proprieta' Height
			...
			k_str_modify += k_knome + ".Height = " + k_num + " " 
...
private function string u_get_dw_nomi_col (string a_datawindow_syntax);//---
//--- Estrae da dw i nomi dei text con indicazione di 'visible' popola la array: ki_tab_nome_oggetto
//--- input: describe("DataWindow.Syntax")       impostare in kist_stampe_add_testata il Titolo e dw_print
...
string k_rc, k_str, k_dw, k_nome_visible, k_modify, k_nome_campo_visible, k_nome_col
...
//--- nome colonna TESTATA				
				ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER] = mid(k_dw, k_pos_field, k_ctr2)
				k_nome_campo_visible = ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER] + ".visible"
...
//--- La colonna deve esistere e NON deve essere "fzoom" poichè servono solo come FUNZIONE di ZOOM a VIDEO
				if k_str <> "!" and pos(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER], "fzoom", 1) = 0 then

					k_len = len(trim(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER])) 
					k_nome_col = left(trim(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER]), k_len - 2)

//--- nome colonna DETTAGLIO (tolgo '_t' dalla fine del nome)	
					if right(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER], 2) = "_t"  then
						k_str = kdw_1.Describe(k_nome_col + ".visible")
						if k_str = "!" then
							k_nome_visible = ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER]
						else
							ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNAMEDB] = k_nome_col
							k_nome_visible = ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNAMEDB]
						end if
					else 
						k_nome_visible = ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER]
					end if
...
//--- Proprietà VISIBLE				
					...
					if k_str = "!" then
						...
						ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLVISIBLE] = k_str   // Proprietà VISIBLE 
					end if

//--- nome colonna K_				
					k_str = kdw_1.Describe("k_" + k_nome_col + ".visible")
					if k_str <> "!" then
						ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNAMENODB] = "k_" + k_nome_col
					end if
					if k_str = "!" then
						k_str = kdw_1.Describe("k" + k_nome_col + ".visible")
					end if
					if k_str <> "!" then
						ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNAMENODB] = "k" + k_nome_col
					end if

//--- banda dove sono i campi header
					k_str = kdw_1.Describe(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADER] + ".band")
					if k_str <> "!" then
						ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLHEADERBAND] = k_str
					end if
//--- banda dove sono i campi db
					k_str = kdw_1.Describe(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNAMEDB] + ".band")
					if k_str <> "!" then
						ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLDBBAND] = k_str
					end if
//--- banda dove sono i campi no db
					k_str = kdw_1.Describe(ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNAMENODB] + ".band")
					if k_str <> "!" then
						ki_tab_nome_oggetto[k_tab_nome_oggetto_ind, ki_tab_nome_oggetto_COLNODBBAND] = k_str
					end if
...
//--- input: impostare in kist_stampe_add_testata il Titolo e dw_print
//---
long k_colcount=0, k_riga, k_pos_x, k_pos_x_max=0, k_width=0, k_pos_y=0, k_width_invisibile=0, k_y_testata=0, k_footer_height
...
string k_visible_linea, k_visible_titolo, k_visible_titolo_2, k_visible_logo, k_visible_data_stampa, k_visible_pagina, k_visible_nome_tabulato
//boolean k_testata_gia_presente = true
long kk_testata_altezza = 460 //520 //400 //--- in unita' PB
double k_conv_unita_pb=1, k_coeff_unita_pb_y
double k_logo_dim[4] // x,y,height,width


//--- distruggo gli oggetti da mettere in testata (utile se sono al 'secondo giro')	
//	k_testata_gia_presente = dw_print_testata_destroy( )
...
//--- chech unità di misura 0=pb, 3=1/1000 di CM, ecc... SPERIAMO SIA SOLO UNO DEI DUE...
	...
	k_coeff_unita_pb_y = 10 * k_conv_unita_pb
...
//--- trova la larghezza orizzontale del datawindow
	...
	do while trim(ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER]) > " "  
//--- se oggetto invisibile lo devo poi sottrarre
		if ki_tab_nome_oggetto[k_ind1, 2] <> '1' then
			k_str = ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER] + ".width"
			k_width_invisibile = k_width_invisibile + integer(trim(kist_stampe_add_testata.dw_print.Describe(k_str)))
		end if
			
//--- posizione colonna
		k_str = ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER] + ".x"
...
//--- modifica dw aggiungendo la coda
		k_footer_height = integer(kist_stampe_add_testata.dw_print.describe("DataWindow.footer.Height"))
		if k_footer_height > 0 then
			k_footer_height = k_footer_height + 80
			k_str_modify += "DataWindow.footer.Height='" + string(k_ctr)+ "' "
		end if

//--- porta giu' di k_dimensione_testata gli oggetti gia' presenti nella header		
		k_ind1 = 1 
		do while len(trim(ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER])) > 0  
//--- se oggetto e' nella header
//			if trim(kist_stampe_add_testata.dw_print.Describe(ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER] + ".band")) = "header" then
			k_str = trim(ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER]) + ".y"
...
////--- aggiunge in coda il nome del tabulato e data e ora e utente
//	k_str_modify += &
//		"create bitmap(band=footer name=footer_box_sottofondo filename='" + trim(k_path_risorse_box_bianco) + "' x='" +string(0.1)+ "' y='" +string(k_footer_height)+ "' height='" +string(k_dimensione_testata)+ "' width='" +string(k_width+50*k_conv_unita_pb)+ "' border='4'  visible='" + k_visible_linea + "' ) " 
//	k_str_modify += &
//		"create text(band=footer name=footer_nome_tabulato visible='" + k_visible_nome_tabulato + "' alignment='0' text='nome tabulato' border='0' color='0' x='" +string(1) + "' y='" +string(k_footer_height + 1)+ "' height='" +string(60 * k_conv_unita_pb)+ "' width='" +string(1400 * k_conv_unita_pb)+ "' html.valueishtml='0' slideup=directlyabove  font.face='Arial' font.height='-10' font.weight='400'  font.family='2' font.pitch='2' font.charset='0' background.mode='1' background.color='" + string(kkg_colore.bianco) + "' ) "
//	k_str_modify += &
//		"create text(band=footer name=footer_data_stampa visible='" + k_visible_nome_tabulato + "' alignment='1' text='nome tabulato' border='0' color='0' x='" +string(k_width - (k_width - (k_ctr * k_conv_unita_pb) - (150* k_conv_unita_pb))) + "' y='" +string(k_footer_height + 1)+ "' height='" +string(60 * k_conv_unita_pb)+ "' width='" +string(k_width - (k_ctr * k_conv_unita_pb) - (150* k_conv_unita_pb)) + "' html.valueishtml='0' slideup=directlyabove  font.face='Arial' font.height='-10' font.weight='400'  font.family='2' font.pitch='2' font.charset='0' background.mode='1' background.color='" + string(kkg_colore.bianco) + "' ) "
//	k_str_modify += "footer_nome_tabulato.text=' tabulato:" + kist_stampe_add_testata.dataobject + "'" + " "
//	k_str_modify += "footer_data_stampa.text=' Prodotto il " + string(kguo_g.get_datetime_current(), "dd mmm yyyy hh:mm")  + " - Utente: " + lower(trim (kguo_utente.get_codice()))+ "'" + " "
//	k_xx = kist_stampe_add_testata.dw_print.Modify(k_str_modify)
//	k_str_modify = "" 
...
public function boolean dw_print_restore_orig ();//---
//--- Rimuove la testata (logo+titolo+data) alla dw di stampa
//--- input: impostare in kist_stampe_add_testata il Titolo e dw_print
//---
boolean k_return
string k_str_modify
...
datastore kdw_orig
	...
	kdw_orig = create datastore
	kdw_orig.dataobject = kist_stampe_add_testata.dw_print.dataobject

	k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_titolo.text" )
	if k_xx = "!" or k_xx = "?" then 
		k_return = false	
	else
		k_return = true	
	
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_titolo.visible" )
		if k_xx = "!" or k_xx = "?" then 
		else
			k_str_modify += "destroy testata1_titolo "
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_linea.visible" )
		if k_xx = "!" or k_xx = "?" then 
		else
			k_str_modify += "destroy testata1_linea "
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_titolo_2.visible" )
		if k_xx = "!" or k_xx = "?" then 
		else
			k_str_modify += "destroy testata1_titolo_2 "
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_logo.visible" )
		if k_xx = "!" or k_xx = "?" then 
		else
			k_str_modify += "destroy testata1_logo "
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_data_stampa.visible" )
		if k_xx = "!" or k_xx = "?" then 
	//		k_visible_data_stampa = '1'
		else
			k_str_modify += "destroy testata1_data_stampa "
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_pagina.visible" )
		if k_xx = "!" or k_xx = "?" then 
	//		k_visible_pagina = '1'
		else
			k_str_modify += "destroy testata1_pagina " 
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata1_nome_tabulato.visible" )
		if k_xx = "!" or k_xx = "?" then 
	//		k_visible_nome_tabulato = '1'
		else
			k_str_modify += "destroy testata1_nome_tabulato "
		end if
		k_xx = kist_stampe_add_testata.dw_print.describe( "testata_box_sottofondo.visible" )
		if k_xx = "!" or k_xx = "?" then 
		else
...
//--- ridimensione la testata
		k_xx = kist_stampe_add_testata.dw_print.describe("DataWindow.syntax")
		k_xx = kdw_orig.describe("DataWindow.syntax")
		k_str_modify += "DataWindow.Header.Height='" + kdw_orig.describe("DataWindow.Header.Height") + "' "
		
//--- estrazione dei nomi dei txt
		u_get_dw_nomi_col(kist_stampe_add_testata.dw_print.describe("DataWindow.Syntax"))
		
//--- porta su gli oggetti originale gia' presenti nella header		
		k_ind1 = 1 
		do while len(trim(ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER])) > 0  
//--- se oggetto e' nella header
			if ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADERBAND] = "header" then
				k_str = trim(ki_tab_nome_oggetto[k_ind1, ki_tab_nome_oggetto_COLHEADER]) + ".y"
				k_xx = trim(kdw_orig.Describe(k_str))
				if isnumber(k_xx) then
					k_str_modify += k_str + "='" + string(k_xx)+ "' "
						//	if integer(k_rc) <= k_dimensione_testata then
						//	k_pos_y = integer(k_rc) + k_dimensione_testata
				end if
			end if
		...
		kist_stampe_add_testata.dw_print.Modify(k_str_modify)  //applica le modifiche
	end if
