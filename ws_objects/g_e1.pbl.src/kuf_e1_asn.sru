...
public constant string kki_status_just_created_descr = "Generato ASN (trasmesso a E1)"		// generato ASN e WO
public constant string kki_status_ready_label = "08"			// LOTTO ricevuto e accettato etichette pronte x M2000
public constant string kki_status_ready_label_descr = "Lotto Ricevuto in magazzino"			// LOTTO ricevuto e accettato etichette pronte x M2000
public constant string kki_status_ready_toschedule = "20"  	// etichette associate (label matched) pronte per la pianificazione
public constant string kki_status_ready_toschedule_descr = "Barcode Accociati pronti alla Pianificazione"  	// etichette associate (label matched) pronte per la pianificazione
public constant string kki_status_working = "45"  	// in lavorazione....
public constant string kki_status_working_descr = "Barcode in Lavorazione..."  	// in lavorazione....
public constant string kki_status_released = "95"  	// ultimo stato del ASN praticamente chiuso
public constant string kki_status_released_descr = "Lotto Chiuso"  	// ultimo stato del ASN praticamente chiuso
public constant string kki_status_canceled = "99"  	// ASN annullato
public constant string kki_status_canceled_descr = "Lotto Annullato"  	// ASN annullato
public constant string kki_status_NC = "NC"  	// Non rilevato
public constant string kki_status_NC_descr = "Stato non rilevato"  	// Non rilevato
...
public function string u_get_stato_descr (string k_status)
end prototypes
...
//--- get dati del pkl
	...
	if kst_tab_meca.id_wm_pklist > 0  then
		...
		kguo_exception.inizializza()
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.SQLErrText = "Lotto senza Packing List, non è possibile generare i dati ASN per E-ONE"
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
//--- Passa a E1 il nr DDT o il N. Ordine
	kst_tab_wm_pklist.nrord = ""
	kst_tab_clienti_altro.id_cliente = kst_tab_meca.clie_3
	if kst_tab_clienti_altro.id_cliente > 0 then  
		kuf1_clienti_altro = create kuf_clienti_altro
		if kuf1_clienti_altro.if_e1_asn_ehvr01_nrord_si(kst_tab_clienti_altro) then   // bisogna passare il NUMERO ORDINE?
			auf_armo.get_id_wm_pklist(kst_tab_meca)
			if kst_tab_meca.id_wm_pklist > 0 then
				kuf1_wm_pklist_testa = create kuf_wm_pklist_testa
				kst_tab_wm_pklist.id_wm_pklist = kst_tab_meca.id_wm_pklist
				kuf1_wm_pklist_testa.get_nrord(kst_tab_wm_pklist)
			end if
		end if
	end if
	if trim(kst_tab_wm_pklist.nrord) > " " then
		kds1_e1_asn_header.setitem( k_riga_insert, "EHVR01", trim(kst_tab_wm_pklist.nrord)) 
	else
		kds1_e1_asn_header.setitem( k_riga_insert, "EHVR01", trim(kst_tab_meca.num_bolla_in)) 
	end if
	
	//forse 21GIUGNO2022 su indicazione di PIETRO metto solo DATAOGGI
	kds1_e1_asn_header.setitem( k_riga_insert, "EHA801", kGuf_data_base.get_e1_dateformat(RelativeDate(kguo_g.get_dataoggi( ), 1)))
	//kds1_e1_asn_header.setitem( k_riga_insert, "EHA801", kGuf_data_base.get_e1_dateformat(kguo_g.get_dataoggi( )) )
...
public function string u_get_stato_descr (string k_status);//-------------------------------------------------------------------------------
//--- Torna la descrizione delo stato
//--- Inpu: codice status
//--- Out: 
//--- Rit: descrizione
//--- lancia exception x errore grave
...
string k_return 
	...
	choose case k_status
		case kki_status_just_created
			k_return = kki_status_just_created_descr
		case kki_status_ready_label
			k_return = kki_status_ready_label_descr 
		case kki_status_ready_toschedule 
			k_return = kki_status_ready_toschedule_descr
		case kki_status_working 
			k_return = kki_status_working_descr 
		case kki_status_released
			k_return = kki_status_released_descr
		case kki_status_canceled
			k_return = kki_status_canceled_descr
		case kki_status_NC
			k_return = kki_status_NC_descr

		case else
			k_return = "Stato " + k_status + " non riconsciuto "
	end choose
