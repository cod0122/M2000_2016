...
type dw_print_0 from w_g_tab_3`dw_print_0 within w_e1_cfg_proprieta
end type
...
try
	choose case  dwo.name 
		...
		case "b_dbparm"
			if not isvalid(kguo_sqlca_db_e1) then kguo_sqlca_db_e1 = create kuo_sqlca_db_e1
			kguo_exception.inizializza( )
			kguo_exception.set_tipo(kguo_exception.KK_st_uo_exception_tipo_OK)
			if messagebox("Test Connessione", "Salvare prima i dati della Connessione qui indicati. ~n~rAttenzione: il test di Connessione è fatto con i parametri già SALVATI su DB! ~n~rProseguire?", Question!, yesno!, 1) = 1 then
				try
					kguo_sqlca_db_e1.ki_conn_blk_msg_done = false  // reset flag display messagge
...
type st_duplica from w_g_tab_3`st_duplica within w_e1_cfg_proprieta
end type
