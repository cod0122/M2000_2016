...
try
	...
	k_return = kuf1_docpath.get_docpath(kuf1_doctipo.kki_tipo_report_pilota)
		...
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			...
			if kguo_sqlca_db_magazzino.sqlcode = 0 then
				if trim(ast_tab_meca_reportpilota.pathreport) > " " then
					...
					kst_esito.esito = kkg_esito.no_esecuzione
					kst_esito.SQLErrText = "Manca in archivio il percorso cartella interna del Report Pilota, Id Lotto= '" &
		                   + string(ast_tab_meca_reportpilota.id_meca) + "' tabella: 'meca_reportpilota'"
					kguo_exception.set_esito(kst_esito)
					throw kguo_exception
				end if
			end if
...
try
	...
	if ast_tab_meca_reportpilota.pathreport > " " then
		...
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.SQLErrText = "Manca il path relativo per completare il nome della cartella dei Report Pilota, operazione non eseguita!"
		kguo_exception.inizializza( )
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
//									+ kkg.path_sep 
	...
	if trim(ast_tab_meca_reportpilota.pathreport) > " " then
		...
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.SQLErrText = "Composizione percorso cartella interna del Report Pilota non rilevata, dati non corretti: " &
		                   + "data='" + string(k_data) + "' cliente: '" + string(kst_tab_meca.clie_3) + "' sep.= '" + kkg.path_sep + "'"
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
