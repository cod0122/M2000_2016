...
string k_title, k_num_int_x, k_rcx
st_tab_armo kst_tab_armo
...
try 
	...
	if k_row > 0 then
		...
		kguo_exception.kist_esito.sqlerrtext = "ZOOM: nessuna riga selezionata, operazione interrotta"
		throw kguo_exception
	end if
	
//--- prova a fabbricare una parte del titolo
	k_title = "Lotto"
	k_rcx = trim(kdw_1.describe("num_int.x"))
	if k_rcx <> "!" then
		k_num_int_x = string(kdw_1.object.num_int[k_row])
		k_title += " n. " + k_num_int_x
		if trim(kdw_1.describe("data_int.x")) <> "!" then
			kst_tab_meca.data_int = kdw_1.getitemdate(k_row, "data_int")
			k_title += "/" + string(kst_tab_meca.data_int, "yyyy")
		end if
	end if
	...
	choose case k_campo_link
		...
		case "id_meca_righe"
			...
			kst_tab_armo.id_meca = kdw_1.getitemnumber(k_row, "id_meca")
			if kst_tab_armo.id_meca > 0 then
				k_title = "Dettaglio Righe " + k_title + " (id=" + string(kst_tab_armo.id_meca) + ") " 
			else
				k_return = false
			end if
		...
		case "b_armo", "id_armo"
			...
			if kst_tab_armo.id_armo > 0 then
				k_title = "Dettaglio Riga " + k_title + " (id riga=" + string(kst_tab_armo.id_armo) + ") " 
			else
				k_return = false
			end if
...
//	
//	else
		if_sicurezza(kkg_flag_modalita.elenco)
...
//	end if
		...
		if kdsi_elenco_output.rowcount() > 0 then
			...
			kuf1_elenco.u_open_zoom(k_title, k_campo_link, kdsi_elenco_output) //CAMPO DI LINK + DATASTORE CON I DATI
