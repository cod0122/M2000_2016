...
private constant string kki_sped_lotto_no_completo_SI = "S" // consente spedizione parziale
private constant string kki_sped_lotto_no_completo_NO = "S" // NON consente spedizione parziale
...
//--- flag di spedizione parzialem mette il default
		if ast_tab_meca_qtna.sped_lotto_no_completo > " " then
		else
			ast_tab_meca_qtna.sped_lotto_no_completo = get_sped_lotto_no_completo_default( )
		end if
	...
	//--- toglie valori NULL
	  ...
	  VALUES (   
				  ...
				  :ast_tab_meca_qtna.sped_lotto_no_completo,
	...
	//--- toglie valori NULL
		...
		UPDATE meca_qtna  
				 SET x_data_inizio = :ast_tab_meca_qtna.x_utente_inizio,   
					  ...
					  x_datins = :ast_tab_meca_qtna.x_datins,   
					  x_utente = :ast_tab_meca_qtna.x_utente,
					  sped_lotto_no_completo = :ast_tab_meca_qtna.sped_lotto_no_completo
			 WHERE id_meca_qtna = :ast_tab_meca_qtna.id_meca_qtna
		      USING kguo_sqlca_db_magazzino;
...
public function string get_sped_lotto_no_completo_default ();//
string k_return = kki_sped_lotto_no_completo_NO
kuf_base kuf1_base
string k_esito
...
//--- flag di spedizione parzialem mette il default
	...
	if left(k_esito,1) = "0" then
		if mid(k_esito,2,1) = "1" then
			k_return = kki_sped_lotto_no_completo_SI
		end if
	end if
