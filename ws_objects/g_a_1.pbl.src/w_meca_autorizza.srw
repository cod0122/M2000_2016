...
st_tab_meca kst_tab_meca
	...
	dw_dett_0.object.b_meca_cert_aliment_st_ok.enabled = 0
	...
	dw_dett_0.object.b_meca_cert_aliment_st_ok.enabled = 1
...
st_tab_meca kst_tab_meca
	...
	dw_dett_0.object.b_meca_cert_forza_stampa.enabled = 0
	...
	dw_dett_0.object.b_meca_cert_forza_stampa.enabled = 1
...
//!!!--- non si capisce per quale ragione ma no passa per la RETRIEVEEND allora forza la call !!!
		dw_dett_0.event retrieveend(k_rc)
		
		choose case k_rc

			case is < 0				
				kguo_exception.setmessage("Operazione fallita" &
			...
			case 0
				...
				if k_scelta <> kkg_flag_modalita.inserimento then
					kguo_exception.setmessage("Ricerca fallita" &
...
//--- controlla se lotto è già in quarantena
	...
	dw_dett_0.post setrow(1)
	dw_dett_0.post setfocus()
...
//--- aggiunge PT x fare email (06.05.2021)		
				try
					kiuf_meca_dosim.add_avviso_pronto_merce(kst_tab_meca)  
...
//--- legge lo stato del Riferimento
		...
		dw_dett_0.object.b_meca_stato.text = get_blk_descr(kst_tab_meca.stato)	// descrizione dello stato
...
private subroutine forza_convalida ();//
	...
	dw_dett_0.object.b_convalida.enabled = 0
	...
	dw_dett_0.object.b_convalida.enabled = 1

end subroutine

protected subroutine attiva_menu ();//
super::attiva_menu() // attiva_tasti_0()
...
st_tab_meca kst_tab_meca
	...
	try 
		dw_dett_0.object.b_meca_stato.enabled = 0
...
//--- aggiorna lo stato del Riferimento
			...
			messagebox("Operazione Conclusa", "Lo Stato del Lotto è stato impostato a "  &
							...
							//+ " (" + string(kst_tab_meca.stato) + ")" )
			inizializza_lista()
		else
			dw_dett_0.object.b_meca_stato.enabled = 1
		end if
...
choose case k_stato
	case kiuf_armo.ki_meca_stato_blk
		k_return = "Sblocca Lotto Incompleto"
	case kiuf_armo.ki_meca_stato_blk_controllato
		k_return = "Sblocca Lotto (già verificato)"
	case kiuf_armo.ki_meca_stato_blk_da_controllare
		k_return = "Sblocca Lotto (da verificare)"
	case kiuf_armo.ki_meca_stato_sblk
		k_return = "Blocca Lotto come Incompleto"
	case kiuf_armo.ki_meca_stato_OK
		k_return = "Blocca Lotto"
	case else
		k_return = "Sblocca/Blocca Riferimento"
end choose
...
type st_duplica from w_g_tab0`st_duplica within w_meca_autorizza
end type
