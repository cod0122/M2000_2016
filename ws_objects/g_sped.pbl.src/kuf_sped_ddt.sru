...
private function long produci_ddt_riga_add () throws uo_exception
public function string get_form_di_stampa_libero_current () throws uo_exception
...
private function string get_form_di_stampa_current () throws uo_exception
private function boolean produci_ddt_1 (st_ddt_stampa ast_ddt_stampa, ref uo_ds_std_1 ads_ddt_stampa) throws uo_exception
private function boolean produci_ddt_1_libero (st_ddt_stampa ast_ddt_stampa, ref uo_ds_std_1 ads_ddt_stampa) throws uo_exception
private function boolean produci_ddt_piede (ref uo_ds_std_1 ads_ddt_stampa, long a_riga_dw) throws uo_exception
public function boolean produci_ddt_popola (ref uo_ds_std_1 ads_ddt_stampa) throws uo_exception
private function boolean oldproduci_ddt_riga_pagamento (ref uo_ds_std_1 ads_ddt_stampa, long a_riga_ds) throws uo_exception
private function boolean produci_ddt_riga (ref uo_ds_std_1 ads_ddt_stampa, long a_riga_ds) throws uo_exception
...
//---- inizializza x stampa 
	kids_stampa_ddt.dataobject = trim(get_form_di_stampa_current( )) 
...
//---- COMMIT....	
			...
			kguo_exception.set_esito(kst_esito)  //--- scrive il LOG
...
try
	...
	kds_ddt_stampa = create uo_ds_std_1
...
//--- legge UN DDT intero
			...
			if k_row < 0 then
				kguo_exception.inizializza(this.classname())
				kguo_exception.set_st_esito_err_ds(kds_ddt_stampa, "Errore in lettura DDT per eseguire la stampa, id " + string(kst_ddt_stampa[k_riga_ddt].id_sped))
				throw kguo_exception
			end if
...
public function string get_form_di_stampa_libero_current () throws uo_exception;//
//----------------------------------------------------------------------------------------------------
//--- Torna il FORM_DI_STAMPA correntemente attivo
//--- 
//--- 
//--- Inp: st_sped_free.id_sped
//--- Out: 
//--- Ritorna: il nome del form di stampa
//---           		
//--- Lancia EXCEPTION x errore
//--- 
//-----------------------------------------------------------------------------------------------------
...
if trim(ki_dw_stampa_ddt_libero) > " " then
	
	return ki_dw_stampa_ddt_libero

else
	kst_esito.esito = kkg_esito.bug
	kst_esito.SQLErrText = "Errore, manca il nome del 'form di stampa' correntemente in uso." 
	kguo_exception.set_esito(kst_esito)
	throw kguo_exception
end if
...
//--- Ritorna: TRUE = form di stampa già registrato in tab
//---           		
//--- Lancia EXCEPTION x errore
//--- 
//-----------------------------------------------------------------------------------------------------
...
if trim(kst_tab_sped.form_di_stampa) > " " then
	k_return = true
else
	kst_tab_sped.form_di_stampa = get_form_di_stampa_current()
end if
...
kst_tab_sped_free.form_di_stampa = ""
if kst_tab_sped_free.id_sped_free > 0 then
...
if trim(kst_tab_sped_free.form_di_stampa) > " " then
	k_return = true
else
	kst_tab_sped_free.form_di_stampa = get_form_di_stampa_libero_current( )
end if
...
if trim(ki_dw_stampa_ddt) > " " then
	
	return ki_dw_stampa_ddt
...
try
	...
	kds_ddt_libero = create uo_ds_std_1
	kds_ddt_libero.dataobject = kids_stampa_ddt.dataobject
	kds_ddt_libero.settransobject(kguo_sqlca_db_magazzino)
	k_row = kds_ddt_libero.retrieve(ast_ddt_stampa.id_sped)  // legge il DDT da mettere in stampa
	if k_row < 0 then
		kguo_exception.set_st_esito_err_ds(kds_ddt_libero, "Errore in lettura DDT 'libero' per eseguire la stampa, id " + string(ast_ddt_stampa.id_sped))
		throw kguo_exception
	end if
...
//--- Valuta numero copie da fare x il DDT 
		...
		choose case ast_ddt_stampa.tipo_num_copie
			...
			case kki_tipo_num_copie_doppia_c
				if ads_ddt_stampa.rowcount( ) > 0 then
					if trim(ads_ddt_stampa.object.vett_1[1]) > " " then  // se ddt con Vettore 
						k_num_copie_max = 3  
					else
						k_num_copie_max = 2  // fatte le 2 copie esco
					end if
...
//--- Righe DDT		
	...
	for k_ddt_riga_arsp = 1 to ads_ddt_stampa.rowcount( ) 
	
		k_boolean = produci_ddt_riga(ads_ddt_stampa, k_ddt_riga_arsp)
		if not k_boolean then
			kst_esito.sqlcode = 0
			kst_esito.sqlerrtext = "D.D.T.: stampa Riga nr. " + string(k_ddt_riga_arsp) + " art. " + trim(ads_ddt_stampa.object.armo_art[k_ddt_riga_arsp])
			kst_esito.esito = kkg_esito.no_esecuzione
			kguo_exception.inizializza( )
			kguo_exception.set_esito( kst_esito )
			throw kguo_exception
		...
		produci_ddt_riga_note_qtna(ads_ddt_stampa, k_ddt_riga_arsp)  	// stampa eventuale dicitura di Spedizione in Quarantena
...
//--- cerca il cod.pag sulla fattura	
	kst_tab_arfa.id_armo = ads_ddt_stampa.object.arsp_id_armo[a_riga_ds]
...
//--- se sulla fattura non c'e' lo cerca sul cliente	
	if kst_tab_pagam.codice = 0 then	
		...
		kst_tab_clienti.codice = ads_ddt_stampa.object.sped_clie_3[a_riga_ds]
...
//--- aggiungo Riga e controllo Fine Pagina	
			...
			if ads_ddt_stampa.object.armo_dose[a_riga_ds] > 0 then
...
//---
boolean k_return=true
constant integer kk_row_len_max = 71  // su una riga ci stanno 71 char max (Courier size 10)
string k_stampante, k_note_totali, k_txt
long k_pagina, k_note_totali_len, k_ctr
integer k_nrbarcode_trattati
double k_differernza
kuf_base kuf1_base		
st_esito kst_esito
st_tab_clienti kst_tab_clienti
...
//--- Qta'	
kids_stampa_ddt.setitem(k_pagina, "qta_" + string(ki_ddt_riga_corpo), ads_ddt_stampa.object.arsp_colli[a_riga_ds])
...
//--- get del nr barcode trattati
	kst_tab_barcode.id_meca =  ads_ddt_stampa.object.meca_id_meca[a_riga_ds]
...
//---  Kgy (dose) -- formatta senza far vedere i decimali se non necessario
		k_differernza = (ads_ddt_stampa.object.armo_dose[a_riga_ds] -  int(ads_ddt_stampa.object.armo_dose[a_riga_ds])) 
		if k_differernza <> 0 then
			kids_stampa_ddt.setitem(k_pagina, "kgy_" + string(ki_ddt_riga_corpo), string(ads_ddt_stampa.object.armo_dose[a_riga_ds], "###0.00") )
		else
			kids_stampa_ddt.setitem(k_pagina, "kgy_" + string(ki_ddt_riga_corpo), string(ads_ddt_stampa.object.armo_dose[a_riga_ds], "######") )
		end if
	end if
end if

if isnull(ads_ddt_stampa.object.arsp_note_1[a_riga_ds] ) then ads_ddt_stampa.object.arsp_note_1[a_riga_ds] = ""
if isnull(ads_ddt_stampa.object.arsp_note_2[a_riga_ds] ) then ads_ddt_stampa.object.arsp_note_2[a_riga_ds] = ""
if isnull(ads_ddt_stampa.object.arsp_note_3[a_riga_ds] ) then ads_ddt_stampa.object.arsp_note_3[a_riga_ds] = ""
//--- se caricate descrizioni uguali, una viene cancellata
if trim(ads_ddt_stampa.object.arsp_note_1[a_riga_ds] ) = trim(ads_ddt_stampa.object.arsp_note_2[a_riga_ds]) then
	ads_ddt_stampa.object.arsp_note_2[a_riga_ds] = ""
end if
if trim(ads_ddt_stampa.object.arsp_note_2[a_riga_ds] ) = trim(ads_ddt_stampa.object.arsp_note_3[a_riga_ds]) then
	ads_ddt_stampa.object.arsp_note_3[a_riga_ds] = ""
end if

//--- descrizione NOTE
k_note_totali = ""
if len(trim(ads_ddt_stampa.object.arsp_note_1[a_riga_ds] ) ) > 0 then
	k_note_totali = RightTrim(ads_ddt_stampa.object.arsp_note_1[a_riga_ds]) + " " 
end if
if len(trim(ads_ddt_stampa.object.arsp_note_2[a_riga_ds] ) ) > 0 then
	k_note_totali += RightTrim(ads_ddt_stampa.object.arsp_note_2[a_riga_ds]) + " "
end if
if len(trim(ads_ddt_stampa.object.arsp_note_3[a_riga_ds] )) > 0 then
	k_note_totali += RightTrim(ads_ddt_stampa.object.arsp_note_3[a_riga_ds])
end if
		
//--- stampa le NOTE caricate a mano dall'operatore
if len(trim(k_note_totali)) > 0 then
	k_note_totali = trim(k_note_totali)
	k_note_totali_len = len(k_note_totali)
	k_note_totali += space(kk_row_len_max)   // aggiunge spazi in coda per evitare che l'ultima riga sia troppo corta
	k_ctr = 1
	do while k_ctr <= k_note_totali_len 
		
//--- aggiungo riga e controllo Fine Pagina	
		k_pagina = produci_ddt_riga_add() 
		
		kids_stampa_ddt.setitem(k_pagina, "descr_" + string(ki_ddt_riga_corpo), mid(k_note_totali, k_ctr, kk_row_len_max ))
		
		k_ctr += kk_row_len_max 
	loop
end if

//--- stampa le NOTE del cliente (caricate nel pklist)
k_note_totali = trim(ads_ddt_stampa.getitemstring(a_riga_ds, "armo_nt_note_1"))
if k_note_totali > " " then
	k_note_totali_len = len(k_note_totali)
	k_note_totali += space(kk_row_len_max)   // aggiunge spazi in coda per evitare che l'ultima riga sia troppo corta
	k_ctr = 1
	do while k_ctr <= k_note_totali_len 
...
//--- aggiungo riga e controllo Fine Pagina	
 ...
 if  ads_ddt_stampa.object.armo_dose[a_riga_ds] = 0 and trim(ads_ddt_stampa.object.prodotti_des[a_riga_ds] ) > " " then
//--- aggiungo riga e controllo Fine Pagina	
	k_pagina = produci_ddt_riga_add() 
		
	kids_stampa_ddt.setitem(k_pagina, "descr_" + string(ki_ddt_riga_corpo), trim(ads_ddt_stampa.object.prodotti_des[a_riga_ds] ))
end if	

//--- Bolla Mandante da non stampare se indicato in CAUSALE di SPED
if trim(ads_ddt_stampa.object.sped_ddt_st_num_data_in[a_riga_ds]) = kuf1_sped.kki_ddt_st_num_data_in_NO then
else
	if len(trim(ads_ddt_stampa.object.meca_num_bolla_in[a_riga_ds])) > 0 then
	
	//--- aggiungo riga e controllo Fine Pagina	
		k_pagina = produci_ddt_riga_add() 
		//--- stampo il numero bolla di entrata e WO
		k_txt = ""
		k_txt = "D.d.t. n.  " + upper(trim(ads_ddt_stampa.object.meca_num_bolla_in[a_riga_ds])) + "  del  " &
											+ string(ads_ddt_stampa.object.meca_data_bolla_in[a_riga_ds])
		if ads_ddt_stampa.object.e1doco[a_riga_ds] > 0 then
			k_txt += "    (wo: " +  string(ads_ddt_stampa.object.e1doco[a_riga_ds]) + ") "
		end if
		kids_stampa_ddt.setitem(k_pagina, "descr_" + string(ki_ddt_riga_corpo), k_txt)
	end if 
end if 

//--- Non stampa Cliente Mandante tra le righe di dettaglio se indicato in CAUSALE di SPED
if trim(ads_ddt_stampa.object.sped_ddt_st_num_data_in[a_riga_ds]) = kuf1_sped.kki_ddt_st_num_data_in_NO or isnull(ads_ddt_stampa.object.sped_ddt_st_num_data_in[a_riga_ds]) then
else
	//--- Se cliente Mandante diverso da Ricevente stampo Rag. soc. del Mandante}
	if ads_ddt_stampa.object.meca_clie_1[a_riga_ds] <> ads_ddt_stampa.object.sped_clie_2[a_riga_ds] then
		
	//--- aggiungo riga e controllo Fine Pagina	
		k_pagina = produci_ddt_riga_add() 
	//--- stampa dati mittente
		kids_stampa_ddt.setitem(k_pagina, "descr_" + string(ki_ddt_riga_corpo), "di: " + trim(ads_ddt_stampa.object.mittente_rag_soc_10[a_riga_ds] ) + " " &
											+ trim(ads_ddt_stampa.object.mittente_indi_1[a_riga_ds]) + " " + trim(ads_ddt_stampa.object.mittente_cap_1[a_riga_ds]) + " " &
											+ trim(ads_ddt_stampa.object.mittente_loc_1[a_riga_ds]) + " " + trim(ads_ddt_stampa.object.mittente_prov_1[a_riga_ds]))
...
try
	...
	if ads_ddt_stampa.object.meca_id_meca[a_riga_ds] > 0 then
		
		kuf1_meca_qtna = create kuf_meca_qtna
		kst_tab_meca_qtna.id_meca = ads_ddt_stampa.object.meca_id_meca[a_riga_ds]
...
st_esito kst_esito
	...
	if not isvalid(adw_ok) and not isvalid(ads_ok) then
		kst_esito.SQLErrText = "Operazione interrotta: " + kkg.acapo + "tipo oggetto non riconosciuto, errore interno del software "
		kst_esito.esito = kkg_esito.err_logico
	end if	
...
//--- p_img_3
		...
		if isvalid(adw_ok) then
			k_rcx= trim(adw_ok.Describe("txt_p_img.text"))
			k_rcx=adw_ok.Modify("p_img.Filename='" + ki_path_risorse + "\" + k_rcx + "'")  // "logo_orig_blu.JPG" + "'")
			k_rcx= trim(adw_ok.Describe("txt_p_img_1.text"))
			k_rcx=adw_ok.Modify("p_img_1.Filename='"  + ki_path_risorse + "\" + k_rcx + "'")  // "logo_iso_blu.JPG"  + "'")
			k_rcx= trim(adw_ok.Describe("txt_p_img_2.text"))
			k_rcx=adw_ok.Modify("p_img_2.Filename='" + ki_path_risorse + "\" + k_rcx + "'")  // "logo_iso_blu.JPG"  + "'")
			k_rcx= trim(adw_ok.Describe("txt_p_img_3.text"))
			k_rcx=adw_ok.Modify("p_img_3.Filename='" + ki_path_risorse + "\" + k_rcx + "'")  // "logo_iso_blu.JPG"  + "'")
		else
			if isvalid(ads_ok) then
				k_rcx= trim(ads_ok.Describe("txt_p_img.text"))
				k_rcx=ads_ok.Modify("p_img.Filename='" + ki_path_risorse + "\" + k_rcx + "'")  // "logo_orig_blu.JPG" + "'")
				k_rcx= trim(ads_ok.Describe("txt_p_img_1.text"))
				k_rcx=ads_ok.Modify("p_img_1.Filename='" + ki_path_risorse + "\" + k_rcx + "' ")  // "logo_iso_blu.JPG"  + "'")
				k_rcx= trim(ads_ok.Describe("txt_p_img_2.text"))
				k_rcx=ads_ok.Modify("p_img_2.Filename='" + ki_path_risorse + "\" +  k_rcx + "' ")  // "logo_iso_blu.JPG"  + "'")
				k_rcx= trim(ads_ok.Describe("txt_p_img_3.text"))
				k_rcx=ads_ok.Modify("p_img_3.Filename='" + ki_path_risorse + "\" +  k_rcx + "' ")  // "logo_iso_blu.JPG"  + "'")
			end if
		end if
...
//--- Imposta indirzzo DEPOSITO di partenza del DDT se esiste il campo
	k_deposito = trim(kids_stampa_ddt.describe("depo_rag_soc.name")) 
	if k_deposito = "depo_rag_soc" then
		kids_stampa_ddt.object.depo_rag_soc[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_rag_soc[a_riga_dati_ddt] 
		kids_stampa_ddt.object.depo_indi[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_indi[a_riga_dati_ddt] 
		kids_stampa_ddt.object.depo_caplocprovnaz[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_caplocprovnaz[a_riga_dati_ddt] 
		kids_stampa_ddt.object.depo_contatto1[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_contatto1[a_riga_dati_ddt] 
		kids_stampa_ddt.object.depo_contatto2[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_contatto2[a_riga_dati_ddt] 
		kids_stampa_ddt.object.depo_contatto3[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_contatto3[a_riga_dati_ddt] 
		kids_stampa_ddt.object.depo_contatto4[k_riga_stampa_ddt] = ads_ddt_stampa.object.depo_contatto4[a_riga_dati_ddt] 
	end if

//--- Se non impostato indirizzo di spedizione sul Documento allora metto quello presente in Anagrafe x le spedizioni
	if len(trim(ads_ddt_stampa.object.sped_rag_soc_1[a_riga_dati_ddt])) = 0  then
					
		ads_ddt_stampa.object.sped_rag_soc_1[a_riga_dati_ddt] = ads_ddt_stampa.object.clienti_rag_soc_20[a_riga_dati_ddt] 
		ads_ddt_stampa.object.sped_rag_soc_2[a_riga_dati_ddt]  = ads_ddt_stampa.object.clienti_rag_soc_21[a_riga_dati_ddt]  
		ads_ddt_stampa.object.sped_indi[a_riga_dati_ddt]  = ads_ddt_stampa.object.clienti_indi_2[a_riga_dati_ddt]   
		ads_ddt_stampa.object.sped_cap[a_riga_dati_ddt]  = ads_ddt_stampa.object.clienti_cap_2[a_riga_dati_ddt]  
		ads_ddt_stampa.object.sped_loc[a_riga_dati_ddt] = ads_ddt_stampa.object.clienti_loc_2[a_riga_dati_ddt]  
		ads_ddt_stampa.object.sped_prov[a_riga_dati_ddt] = ads_ddt_stampa.object.clienti_prov_2[a_riga_dati_ddt]   
		ads_ddt_stampa.object.sped_id_nazione[a_riga_dati_ddt] = ads_ddt_stampa.object.clienti_id_nazione_2[a_riga_dati_ddt]   
		ads_ddt_stampa.object.sped_nazioni_nome[a_riga_dati_ddt] = ads_ddt_stampa.object.clienti_nazioni_nome_2[a_riga_dati_ddt]  
		
	end if
	
	
//--- riga intestazione ddt - controllo se indirizzio destinazione diverso da sede	
	if trim(ads_ddt_stampa.object.sped_rag_soc_1[a_riga_dati_ddt]) <> trim(ads_ddt_stampa.object.clienti_rag_soc_10[a_riga_dati_ddt]) &
				or trim(ads_ddt_stampa.object.sped_rag_soc_2[a_riga_dati_ddt])  <> trim(ads_ddt_stampa.object.clienti_rag_soc_11[a_riga_dati_ddt])   & 
				or trim(ads_ddt_stampa.object.sped_indi[a_riga_dati_ddt])  <> trim(ads_ddt_stampa.object.clienti_indi_1[a_riga_dati_ddt])  & 
				or trim(ads_ddt_stampa.object.sped_cap[a_riga_dati_ddt])  <> trim(ads_ddt_stampa.object.clienti_cap_1[a_riga_dati_ddt]) &
				or trim(ads_ddt_stampa.object.sped_loc[a_riga_dati_ddt])  <> trim(ads_ddt_stampa.object.clienti_loc_1[a_riga_dati_ddt])  & 
				or trim(ads_ddt_stampa.object.sped_prov[a_riga_dati_ddt])  <> trim(ads_ddt_stampa.object.clienti_prov_1[a_riga_dati_ddt]) then 
		...
		k_rc=kids_stampa_ddt.setitem(k_riga_stampa_ddt, "intestazione",  ads_ddt_stampa.object.clienti_RAG_SOC_10[a_riga_dati_ddt] + " ~n~r " &  
																				+ ads_ddt_stampa.object.clienti_RAG_SOC_11[a_riga_dati_ddt] + "  ~n~r"  )
 
		k_rc=kids_stampa_ddt.setitem(k_riga_stampa_ddt, "intestazione_ind",  ads_ddt_stampa.object.clienti_INDI_1[a_riga_dati_ddt] + " ~n~r" &    
																				+ ads_ddt_stampa.object.clienti_CAP_1[a_riga_dati_ddt] +"  " &
																				+ trim(ads_ddt_stampa.object.clienti_LOC_1[a_riga_dati_ddt]) + "  (" &
																				+ trim(ads_ddt_stampa.object.clienti_PROV_1[a_riga_dati_ddt]) + ")  ~n~r" &  
																				+ trim(ads_ddt_stampa.object.clienti_nazioni_nome_1[a_riga_dati_ddt])  )
...
//--- riga indirizzo	
	k_rc=kids_stampa_ddt.setitem(k_riga_stampa_ddt, "indirizzo_riga_1",  ads_ddt_stampa.object.sped_RAG_SOC_1[a_riga_dati_ddt])  
	k_rc=kids_stampa_ddt.setitem(k_riga_stampa_ddt, "indirizzo_riga_2",  ads_ddt_stampa.object.sped_RAG_SOC_2[a_riga_dati_ddt])  
	k_rc=kids_stampa_ddt.setitem(k_riga_stampa_ddt, "indirizzo_riga_3",  ads_ddt_stampa.object.sped_INDI[a_riga_dati_ddt])  
	kids_stampa_ddt.setitem(k_riga_stampa_ddt, "indirizzo_riga_4",  ads_ddt_stampa.object.sped_CAP[a_riga_dati_ddt] +" " &
																	+ trim(ads_ddt_stampa.object.sped_LOC[a_riga_dati_ddt]) + " " &
																	+ trim(ads_ddt_stampa.object.sped_PROV[a_riga_dati_ddt]) )  

    if len(trim(ads_ddt_stampa.object.sped_id_nazione[a_riga_dati_ddt])) > 0 then
		k_rc=kids_stampa_ddt.setitem(k_riga_stampa_ddt, "indirizzo_riga_5", ads_ddt_stampa.object.sped_nazioni_nome[a_riga_dati_ddt])  
	end if

//--- testata con il numero data ecc...
	if ads_ddt_stampa.object.sped_num_bolla_out[a_riga_dati_ddt] > 9000000 then  // se DDT BIS allora toglie 9000....
		k_num_bolla_out_bis = right(string(ads_ddt_stampa.object.sped_num_bolla_out[a_riga_dati_ddt]),6)
		kids_stampa_ddt.setitem(k_riga_stampa_ddt, "num_bolla_out_1", long(k_num_bolla_out_bis))  
	else
		kids_stampa_ddt.setitem(k_riga_stampa_ddt, "num_bolla_out_1", ads_ddt_stampa.object.sped_num_bolla_out[a_riga_dati_ddt])  
	end if
	kids_stampa_ddt.setitem(k_riga_stampa_ddt, "data_bolla_out",  (ads_ddt_stampa.object.sped_data_bolla_out[a_riga_dati_ddt]))  
	kids_stampa_ddt.setitem(k_riga_stampa_ddt, "id_sped",  (ads_ddt_stampa.object.id_sped[a_riga_dati_ddt]))  

	if isnull(ads_ddt_stampa.object.sped_clie_2[a_riga_dati_ddt]) then ads_ddt_stampa.object.sped_clie_2[a_riga_dati_ddt] = 0 
	kids_stampa_ddt.setitem(k_riga_stampa_ddt, "clie_2",  (ads_ddt_stampa.object.sped_clie_2[a_riga_dati_ddt]))  
	if isnull(ads_ddt_stampa.object.sped_clie_3[a_riga_dati_ddt]) then ads_ddt_stampa.object.sped_clie_3[a_riga_dati_ddt] = 0 
	kids_stampa_ddt.setitem(k_riga_stampa_ddt, "clie_3",  (ads_ddt_stampa.object.sped_clie_3[a_riga_dati_ddt]))  
	
	if isnull(ads_ddt_stampa.object.sped_causale[a_riga_dati_ddt]) then ads_ddt_stampa.object.sped_causale[a_riga_dati_ddt] = " " 
	kids_stampa_ddt.setitem(k_riga_stampa_ddt, "causale",  trim(ads_ddt_stampa.object.sped_causale[a_riga_dati_ddt]))  
...
kids_stampa_ddt = create uo_ds_std_1
