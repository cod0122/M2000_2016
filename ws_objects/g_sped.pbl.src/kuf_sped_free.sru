...
public function integer get_num_bolla_out_next (ref st_tab_sped_free ast_tab_sped_free) throws uo_exception
public subroutine set_num_bolla_out_in_base (ref st_tab_sped_free ast_tab_sped_free) throws uo_exception
end prototypes
...
public function integer get_num_bolla_out_next (ref st_tab_sped_free ast_tab_sped_free) throws uo_exception;//
//------------------------------------------------------------------
//--- Torna l'ultimo numero e anno del DDT Libero registrato sul BASE
//--- 
//---  inp: st_tab_sped_free vuoto 
//---  ret: st_tab_sped_free.num_bolla_out 
//---  ret: Anno DDT
//---                                     
//------------------------------------------------------------------
//
...
try
	...
	k_esito_base = kuf1_base.prendi_dato_base( "ddt_out_n_free")
	if left(k_esito_base,1) <> "0" then
		kst_esito.esito = kkg_esito.db_ko  
		kst_esito.SQLErrText = "Errore in lettura da Proprietà di 'ultimo N. DDT libero' " &
									 + "~n~r"  + trim(mid(k_esito_base,2))
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	else
		if isnumber(mid(k_esito_base,2)) then
			ast_tab_sped_free.num_bolla_out = &
					string(integer(trim(mid(k_esito_base,2))) + 1)
		end if
	end if
	
	if ast_tab_sped_free.num_bolla_out > " " then
		k_esito_base = kuf1_base.prendi_dato_base( "ddt_out_year_free")
		if left(k_esito_base,1) <> "0" then
			kst_esito.esito = kkg_esito.db_ko  
			kst_esito.SQLErrText = "Errore in lettura da Proprietà di 'Anno del DDT libero' " &
										 + "~n~r"  + trim(mid(k_esito_base,2))
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
			...
			if isnumber(mid(k_esito_base,2)) then
				k_return = integer(mid(k_esito_base,2))
			end if
		end if
...
public subroutine set_num_bolla_out_in_base (ref st_tab_sped_free ast_tab_sped_free) throws uo_exception;//
//------------------------------------------------------------------
//--- Registra numero ddt nel BASE
//--- 
//---  input: ast_tab_sped_free.num_bolla_out + data_bolla_out
//---  ret: 
//---                                     
//------------------------------------------------------------------
//
...
try
	...
	if isnumber(trim(ast_tab_sped_free.num_bolla_out)) then	
		kuf1_base = create kuf_base
		
		k_anno = this.get_num_bolla_out_next(kst_tab_sped_free)
		
	//--- aggiorna solo anno uguale e numero non minore 
		if k_anno = year(ast_tab_sped_free.data_bolla_out) &
					and integer(trim(ast_tab_sped_free.num_bolla_out)) >= integer(trim(kst_tab_sped_free.num_bolla_out)) then
			kst_tab_base.st_tab_g_0.esegui_commit = "S"
			kst_tab_base.key = "ddt_out_n_free"
			kst_tab_base.key1 = string(kst_tab_sped_free.num_bolla_out)
			kst_esito  = kuf1_base.metti_dato_base(kst_tab_base)
			if kst_esito.esito = kkg_esito.db_ko then
				kst_esito.SQLErrText = "Errore in aggiornamento in Proprietà di ultimo N. 'DDT Libero' a '" &
											+ string(kst_tab_sped_free.num_bolla_out) + "'" &
