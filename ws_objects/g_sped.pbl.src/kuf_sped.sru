...
//--- costanti nomi DW 
public constant string kki_d_sped_l_indirizzi = "d_sped_l_indirizzi" 
...
private datastore kdsi_d_sped_l_indirizzi
end variables
...
SetPointer(kkg.pointer_attesa)
	...
	if kst_tab_sped.clie_2 > 0 then
		...
		if u_set_ds_d_sped_l_indirizzi(kst_tab_sped) > 0 then
		...
		//=== struttura st_open_w
			...
			kst_open_w.key2 = trim(kdsi_d_sped_l_indirizzi.dataobject)
			kst_open_w.key3 = "0"     //--- viene riempito con il nr di riga selezionata
			kst_open_w.key4 = kGuf_data_base.prendi_win_attiva_titolo()    //--- Titolo della Window di chiamata per riconoscerla
			kst_open_w.key12_any = kdsi_d_sped_l_indirizzi
			kst_open_w.key7 = kuf1_elenco.ki_esci_dopo_scelta  // esce subito dopo la scelta
			kuf1_elenco.u_open(kst_open_w)
...
SetPointer(kkg.pointer_attesa)
	...
	if kst_tab_sped.clie_2 > 0 then
		...
		k_return = u_set_ds_d_sped_l_indirizzi(kst_tab_sped)
...
if kst_tab_sped.id_sped > 0 then
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in lettura dato 'a cura di' del DDT id " + string( kst_tab_sped.id_sped) + " ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			
		end if
	else
		if isnull(kst_tab_sped.cura_trasp) then kst_tab_sped.cura_trasp = kki_cura_trasp_Nessuno
		k_return = kst_tab_sped.cura_trasp
	end if
else
	kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
...
long k_return
	...
	if not isvalid(kdsi_d_sped_l_indirizzi) then
		kdsi_d_sped_l_indirizzi = create datastore
		kdsi_d_sped_l_indirizzi.dataobject = kki_d_sped_l_indirizzi 
		kdsi_d_sped_l_indirizzi.settransobject(sqlca) 
	end if
		
	k_clie_2_x = kdsi_d_sped_l_indirizzi.describe("evaluate('arg1', 1)") 
	if isnumber(k_clie_2_x) then
		k_clie_2 = long(k_clie_2_x) 
	end if

	if kst_tab_sped.clie_2 <> k_clie_2 then
		k_return = kdsi_d_sped_l_indirizzi.retrieve(kst_tab_sped.clie_2)
	else
		k_return = kdsi_d_sped_l_indirizzi.rowcount()
	end if
...
if isvalid(kdsi_d_sped_l_indirizzi) then destroy kdsi_d_sped_l_indirizzi
