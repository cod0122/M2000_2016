...
protected int ki_tabtext_sizemax_onleft = 1030   // DA PERSONALIZZARE: lunghezza massima del texttab per contenere bene i dw
														// 1     2     3     4     5     6     7     8     9  
protected boolean ki_tabpage_enabled[9] = {true, true, true, true, true, true, true, true, true}   // indicare sui figli i tabpage abilitati
...
public subroutine u_tab_1_tabpage_dw_reset ()
public subroutine u_tab_1_tabpage_dw_resetupdate ()
public subroutine u_set_tab_1_visible ()
...
if ki_st_open_w.flag_primo_giro <> 'S' then
	...
	//		if dati_modif_1() then
				...
				m_main.m_finestra.m_gestione.m_fin_conferma.enabled = false
			end if
		else
			m_main.m_finestra.m_gestione.m_fin_conferma.enabled = false
		end if
...
//
string k_return = "", k_rc_inizializza
...
//		tab_1.visible = true
		...
		choose case a_tab_da_attivare 
			case 1
				...
				if k_rc_inizializza = "E" then
					ki_exit_si = true
				end if
				tab_1.tabpage_1.dw_1.event post u_get_focus_change_tab( )
			case 2
				inizializza_1()
				tab_1.tabpage_2.dw_2.event post u_get_focus_change_tab( )
			case 3
				inizializza_2()
				tab_1.tabpage_3.dw_3.event post u_get_focus_change_tab( )
			case 4
				inizializza_3()
				tab_1.tabpage_4.dw_4.event post u_get_focus_change_tab( )
			case 5
				inizializza_4()
				tab_1.tabpage_5.dw_5.event post u_get_focus_change_tab( )
			case 6
				inizializza_5()
				tab_1.tabpage_6.dw_6.event post u_get_focus_change_tab( )
			case 7
				inizializza_6()
				tab_1.tabpage_7.dw_7.event post u_get_focus_change_tab( )
			case 8
				inizializza_7()
				tab_1.tabpage_8.dw_8.event post u_get_focus_change_tab( )
			case 9
				inizializza_8()
				tab_1.tabpage_9.dw_9.event post u_get_focus_change_tab( )
		end choose	
		
		if not ki_exit_si then //E=Uscita Immediata
...
boolean k_tabpage_multilane
	...
	k_tabpage_counted = upperbound(tab_1.control[])

	for k_i = 1 to k_tabpage_counted
		if pos(tab_1.control[k_i].text, "~n") > 0 then
			k_tabpage_multilane = true  // almeno un testo con un 'a capo'
			exit
		end if
	next

	if tab_1.tabposition = tabsonleft! then
		k_height = 10
		if tab_1.showtext then 
			k_width = ki_tabtext_sizemax_onleft 
		else
			k_width = 150
		end if
	else
		k_width = 40
		if k_tabpage_multilane then 
			k_height = 190
		else
			k_height = 110
		end if
...
public subroutine u_tab_1_tabpage_dw_resetupdate ();//---- azzera il flag delle modifiche
			...
			tab_1.tabpage_9.dw_9.ResetUpdate ( ) 
...
tab_1.move(30000, 30000)  // nesconde tab
...
//===              : 999=premuto ANNULLA OPERAZIONE
	k_errore = u_update_if_dw_modified(this.title)
...
for k_ind = 1 to 9
	if ki_tabpage_enabled[k_ind] then
		kpage = this.control[k_ind]
		kpage.CreatePage()
	end if
next
...
event u_get_focus_change_tab();//	
if this.enabled or this.visible then
