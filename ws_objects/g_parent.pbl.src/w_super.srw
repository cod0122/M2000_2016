...
int k_return = 0
int k_ctr
w_super kw_super, kw_super_prev
...
//--- Tolgo la window dalle aperte se è l'ultima resetto il menu
	if kguo_g.window_aperta_rimuove(kw_super) = 0 then
		if isvalid(w_main) then
			//ki_menu.post reset_menu_all( )
			//ki_menu.post u_imposta_window_menu( )
			//w_main.post ChangeMenu(m_main)  // non so xchè ma bisogna resettare il menu per far funzionare i shortcut!!!????!!!
			w_main.post u_menu_init()
		end if
//		
//	else	
//		kw_super_prev = kguo_g.window_aperta_get_last()
//		if isvalid(kw_super_prev) and not isnull(kw_super_prev) then
//			kw_super_prev.post event u_activate_base( )
//		end if
	end if
...
//---
//---
integer k_x, k_y, k_w, k_h, k_num_win, k_ctr, k_n_controls
...
//	k_num_win = kGuf_data_base.prendi_num_win_uguale(trim(kw_super.ClassName()))	
	...
	if ki_st_open_w.st_tab_menu_window.salva_controlli = "S" then //and k_num_win = 1 &
		...
		k_n_controls = upperbound( kw_super.control[] )
		for k_ctr = 1 to k_n_controls
		
			if isvalid(kw_super.control[k_ctr]) then

				k_control_typeof = kw_super.control[k_ctr].typeof()
...
protected subroutine u_win_show ();//---
//--- Trucco per mostrare in Apertura la Windows
//---

// la DOCK non ne tiene conto: this.move(0, 0)   // mostra la window
...
protected subroutine u_win_hide ();//---
//--- Trucco per nascondere in Apertura la Windows
//---

// la DOCK non ne tiene conto: this.move(29998, 30000)   // nasconde subito la window
...
//
//boolean k_risize = false
boolean k_predef = false


	if ki_personalizza_pos_controlli then
		if not ki_controlli_ripristinati then
//--- posiziona e dimensiona gli oggetti personalizzati nella window	
			k_predef = u_window_control_restore()
			if k_predef then
	...
	post u_win_show() // mostra la window
...
event open;//
	u_win_hide( )  // nasconde subito la window
