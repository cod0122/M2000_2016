...
type dw_print_0 from uo_d_std_1 within w_g_tab
end type
...
protected st_stampe kist_stampe

protected string ki_msg_updated_ok
...
string k_errore_check
	...
	ki_msg_updated_ok = "Dati aggiornati correttamente"  // msg di default x dati aggiornati correttamente
...
choose case k_errore_check 
   ...
   case "1" //errore grave: incongruenze dati
		messagebox("Digitati dati incongruenti, operazione non eseguita",  Mid(k_errore_dati, 2), stopsign!)
		k_return="1" + Mid(k_errore_dati, 2)
		
	case "2" //errore grave: digitazione dati 
		messagebox("Inseriti dati non validi, operazione non eseguita", Mid(k_errore_dati, 2), stopsign!)
		k_return="1" + Mid(k_errore_dati, 2)
		
	case "3" //errore grave: mancanza dati
		messagebox("Dati insufficienti, operazione non eseguita", Mid(k_errore_dati, 2), stopsign!)
		k_return="1" + Mid(k_errore_dati, 2)
	...
	case "4" //avvertenza DB: dati da rivedere
		if ki_aggiorna_richiesta_conferma then
			if messagebox("Rilevate alcune anomalie", Mid(k_errore_dati, 2) + "~n~rEseguire comunque l'aggiornamento ?", question!, yesno!, 2) = 2 then
				k_return = "1" + "Dati non aggiornati. Operazione interrotta dall'utente per dati anomali: " + trim(Mid(k_errore_dati, 2))
			else
				k_return = "0" + Mid(k_errore_dati, 2)
			end if
		else
	...
	case "5" //avvertenza dati: dati da rivedere
		if ki_aggiorna_richiesta_conferma then
			if messagebox("Rilevate alcune incongruenze", Mid(k_errore_dati, 2) + "~n~rEseguire comunque l'aggiornamento ?", question!, yesno!, 2) = 2 then
				k_return = "1" + "Dati non aggiornati. Operazione interrotta dall'utente per dati incongruenti: " + trim(Mid(k_errore_dati, 2))
			else
				k_return = "0" + Mid(k_errore_dati, 2)
			end if
		else
	...
	case "6" //avvertenza dati: operazione non eseguita
		messagebox("Operazione non eseguita",  Mid(k_errore_dati, 2), stopsign!)
		k_return="1" + Mid(k_errore_dati, 2)
	...
	case else //avvertenze: altro
		if ki_aggiorna_richiesta_conferma then
			if messagebox("Richiesto Aggiornamento Archivio", Mid(k_errore_dati, 2) + "~n~rProseguire con l'aggiornamento ?", question!, yesno!, 2) = 2 then
				k_return = "1" + "Dati non aggiornati. Operazione interrotta dall'utente per 'avvertenze sui dati': " + trim(Mid(k_errore_dati, 2))
			else
				k_return = "0" + Mid(k_errore_dati, 2)
			end if		
		else
...
//--- se x caso l'oggetto non è buono lo riassegno.... (ma dovrebbe esserlo sempre!)
	//if not isvalid(m_main) then	m_main = this.menuid
	...
	if ki_filtra_attivo then
		...
		m_main.m_filtro.enabled = false
	end if
	m_main.m_filtro.toolbaritemvisible = m_main.m_filtro.enabled 
	...
	if m_main.m_finestra.m_layout_predefinito.enabled <> ki_salva_controlli then
		m_main.m_finestra.m_layout_predefinito.enabled = ki_salva_controlli
	end if
	if not m_main.m_finestra.m_layout_predefinito.enabled then
		if ki_windowpredef then
			m_main.m_finestra.m_layout_predefinito.enabled = ki_windowpredef
		end if
	end if

////--- Appena entra "resetto" la barra degli strumenti
//	if ki_toolbar_programmi_primo_giro then
//		m_main.reset_menu_strumenti( )
//	end if
	
//	if ki_toolbar_window_presente then
//		m_main.m_strumenti.visible = true
//	else
//		m_main.m_strumenti.visible = false
//	end if
	
	m_main.post u_imposta_window_menu()
...
//--- fisso l'elenco di ricerca 	
	...
	if isvalid(kdw_1) then
		if kdw_1.rowcount() > 1 then
			...
			kuf1_sort = create kuf_sort
			kuf1_sort.u_open_window(kdw_1)
			destroy kuf1_sort
//			setnull(k_x)
//			kdw_1.setsort(k_x)
...
protected subroutine attiva_tasti_pers ();
//--- 
//--- script di personalizzazione TASTI riservata x gli eredi 
//---
...
//	setpointer(kkg.pointer_attesa)
	u_win_hide( )  //nasconde la window in apertura
...
//---- oggetto generico, se riesce lo crea dall'oggetto referenziato con la funzione es. 'kuf_cliente'
	if ki_st_open_w.nome_oggetto > " " then
		kiuf_parent = create using ki_st_open_w.nome_oggetto
	end if
	if not isvalid(kiuf_parent) then
...
int iCurrent
call super::create
this.dw_print_0=create dw_print_0
...
this.Control[iCurrent+1]=this.dw_print_0
this.Control[iCurrent+2]=this.st_ritorna
this.Control[iCurrent+3]=this.st_ordina_lista
this.Control[iCurrent+4]=this.st_aggiorna_lista
this.Control[iCurrent+5]=this.cb_ritorna
this.Control[iCurrent+6]=this.st_stampa
end on

on w_g_tab.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.dw_print_0)
