...
//=== Legge le righe del dw salvate l'ultima volta (importfile)
	...
	if k_importa <= 0 then // Nessuna importazione eseguita
		if dw_lista_0.retrieve(ki_data_ini, ki_id_cliente) < 1 then
			k_return = "1Nessun Documento scaduto oltre il " + string(ki_data_ini) //+ " - " + string(ki_data_fin)

			SetPointer(oldpointer)
			messagebox("Elenco Documenti", mid(k_return,2))
...
//--- identifico il contratto x chiamare l'applicazione giusta	
	choose case dw_lista_0.getitemstring( k_riga, "k_tipo_contratto") 
		...
		case "6" // Quotazioni
			...
			K_st_open_w.id_programma = kuf1_contratti_doc.get_id_programma( kkg_flag_modalita.visualizzazione )
			kst_tab_contratti_doc.id_cliente = dw_lista_0.getitemnumber( k_riga, "id_cliente") 
			kst_tab_contratti_doc.id_contratto_doc = long(dw_lista_0.getitemstring( k_riga, "nr_contratto") )
...
type st_duplica from w_g_tab0`st_duplica within w_contratti_generali_l
end type
