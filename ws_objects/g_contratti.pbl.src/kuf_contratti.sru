...
//--- Anteprima nome dw
constant string kki_anteprima_contratti_l = "d_contratti_l_attivi"
end variables
...
choose case a_campo_link
	...
	case "contratto", "mc_co", "contratti_codice", "id_contratto"
		kst_tab_contratti.codice = 0
		if a_campo_link = "contratto" or a_campo_link = "id_contratto" then
			...
			if trim(adw_1.Describe("contratti_codice.x")) <> "!" then 
				...
				if trim(adw_1.Describe("id_contratto.x")) <> "!" then 
					kst_tab_contratti.codice = adw_1.getitemnumber(adw_1.getrow(), "id_contratto")
				else
					if trim(adw_1.Describe("contratto.x")) <> "!" then 
	...
	case "sc_cf", "cf"
		...
		if len(trim(kst_tab_contratti.sc_cf)) > 0 then
			...
			kst_open_w.key1 = "Capitolato: " + trim(kst_tab_contratti.sc_cf)
		else
			k_return = false
		end if
	...
	case "contratti_l"
		if trim(adw_1.Describe("cod_cli.x")) <> "!" then 
			kst_tab_contratti.cod_cli = adw_1.getitemnumber(adw_1.getrow(), "cod_cli")
		else
			if trim(adw_1.Describe("clie_3.x")) <> "!" then 
				kst_tab_contratti.cod_cli = adw_1.getitemnumber(adw_1.getrow(), "clie_3")
			else
				if trim(adw_1.Describe("id_cliente.x")) <> "!" then 
					kst_tab_contratti.cod_cli = adw_1.getitemnumber(adw_1.getrow(), "id_cliente")
				else
					kst_tab_contratti.cod_cli = 0
				end if
			end if
		...
		kst_tab_contratti.mc_co = "*"
		kst_esito = this.anteprima_l( kdsi_elenco_output, kst_tab_contratti )
		if kst_esito.esito <> kkg_esito.ok then
			kguo_exception.inizializza( )
			kguo_exception.set_esito( kst_esito)
			throw kguo_exception
		...
		if kst_tab_contratti.cod_cli > 0 then
			kst_open_w.key1 = "Elenco Contratti Attivi del Cliente " + string(kst_tab_contratti.cod_cli)
		else
			kst_open_w.key1 = "Elenco Tutti i Contratti Attivi"
		end if
...
if k_return then
	...
	//--- struttura st_open_w
		...
		kguo_exception.inizializza( )
		if kst_open_w.key1 > " " then
			kst_open_w.key1 = "(" + kst_open_w.key1 + ")"
		end if
		kguo_exception.setmessage( "Nessun valore disponibile (" + this.get_id_programma(kst_open_w.flag_modalita) + "). " + kst_open_w.key1 )
		throw kguo_exception
...
st_esito kst_esito
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in lettura periodo validità del Contratto '" + string(ast_tab_contratti.codice) + "'" &
...
kuf_utility kuf1_utility 
	...
	if isvalid(kdw_anteprima)  then
		if kdw_anteprima.dataobject = kki_anteprima_contratti_l then 
		else
			kdw_anteprima.dataobject = kki_anteprima_contratti_l
			kdw_anteprima.dataobject = kki_anteprima_contratti_l		
			kdw_anteprima.settransobject(sqlca)
		end if
	end if
	...
	kdw_anteprima.reset()	
//--- retrive dell'attestato 
	k_rc=kdw_anteprima.retrieve(kst_tab_contratti.cod_cli, kst_tab_contratti.mc_co)
...
public function string get_sl_pt (ref st_tab_contratti ast_tab_contratti) throws uo_exception;/*
  ...
  Inp: st_tab_contratti.codice
  out: st_tab_contratti.sl_pt
  rit: st_tab_contratti.sl_pt
*/
string k_return 
...
if ast_tab_contratti.codice > 0 then
	...
	select distinct
			 sl_pt
	 into :ast_tab_contratti.sl_pt
		from contratti
		where codice =  :ast_tab_contratti.codice
		using kguo_sqlca_db_magazzino;

	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		kguo_exception.set_st_esito_err_db(kguo_sqlca_db_magazzino, "Errore in Lettura Piano di Treattamento (PT) in tab.Contratti, codice=" + string(ast_tab_contratti.codice))
		throw kguo_exception
	end if
	
	if kguo_sqlca_db_magazzino.sqlcode = 0 then
		if ast_tab_contratti.sl_pt > " " then
			k_return = ast_tab_contratti.sl_pt
		end if
	end if
