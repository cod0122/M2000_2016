...
forward
global type kuf_sv_skedula from kuf_parent
end type
end forward
...
public function st_esito anteprima_elenco (datastore kdw_anteprima, st_sv_skedula kst_sv_skedula)
end prototypes
...
kist_esito.nome_oggetto = this.classname()
	...
	if sqlca.sqlcode <> 0 then
		...
		kist_esito.SQLErrText = trim(This.ClassName()) + "~n~r" &
...
//--- Get nome oggetto da chiamare 		
			...
			if trim(kst_open_w.nome_oggetto) > " " then
				kuf1_parent0 = create using kst_open_w.nome_oggetto   //"kuf_prodotti"

				kst_esito = kuf1_parent0.u_batch_run( )  // LANCIA LA FUNZIONE
...
if k_riga > 0 then
	...
	choose case a_campo_link
		...
		case "sv_eventi_sked"
			k_rcx = adw_link.describe("sv_skedula_id.visible") 
			if k_rcx <> "!" and k_rcx <> "?" then
				kst_sv_skedula.id = adw_link.getitemnumber(k_riga, "sv_skedula_id")
				if kst_sv_skedula.id > 0 then
					kst_esito = this.anteprima_elenco( kdsi_elenco_output, kst_sv_skedula )
					if kst_esito.esito <> kkg_esito.ok then
						kguo_exception.inizializza()
						kguo_exception.set_esito( kst_esito)
						throw kguo_exception
					end if
					k_titolo = "Evento '" + trim(string(kst_sv_skedula.id)) + "' " 
				else
					k_return = false
				end if
			else
...
st_esito kst_esito
	...
	if kst_sv_skedula.id > 0 then
		...
		kdw_anteprima.dataobject = "d_sv_eventi_sked_l"		
		kdw_anteprima.settransobject(sqlca)

		kdw_anteprima.reset()	
		k_rc=kdw_anteprima.retrieve(kst_sv_skedula.id)

	else
		kst_esito.sqlcode = 0
		kst_esito.SQLErrText = "Nessun Evento da visualizzare: ~n~r" + "nessun codice indicato"
		kst_esito.esito = "1"
