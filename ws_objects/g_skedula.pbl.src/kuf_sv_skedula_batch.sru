...
private function st_esito tb_insert_schedula_eventi ()
end prototypes
...
int k_giorno, k_num_gg, k_ora, k_month, k_year
...
//--- inserisco nuovo evento						
						kist_esito = tb_insert_schedula_eventi()
			...
			case "M", "N"   // M = primo del mese successivo; N = Ultimo giorno di questo mese
				k_giorno = integer((kist_sv_skedula.rilancia_dopo_mm / 60) / 24)	
				k_data = date(k_dataoggi )
				
				k_month = Month(k_data)
				k_year = year(k_data)
				...
				if kist_sv_skedula.flag_run_giorni = "M" then   //primo del mese successivo
					k_data = date(k_year, k_month, 01)   
				else	// Ultimo giorno del mese (questo)
					k_data = relativedate(date(k_year, k_month, 01), -1)   
				end if
...
//--- aggiorna lo stato della tab schedulati
									...
									kist_esito = tb_aggiorna_stato_sv_skedula()
								end if
							end if
						end if
...
//--- aggiorna lo stato della tab schedulati
		if kist_esito.esito = kkg_esito.ok or kist_esito.esito = kkg_esito.db_wrn then
			kist_esito = tb_aggiorna_stato_sv_skedula_1('1', '5')
...
//--- aggiorna lo stato Operazioni da Schedulare								
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		...
		kst_esito.SQLErrText = trim(This.ClassName()) + "~n~r" + &
...
st_esito kst_esito
  ...
  VALUES ( 
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Tab.Schedulazioni Eventi (sv_eventi_sked):" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		if kguo_sqlca_db_magazzino.sqlcode = 100 then
			kst_esito.esito = kkg_esito.not_fnd
		else
			if kguo_sqlca_db_magazzino.sqlcode > 0 then
