...
type st_cb_prototipi from statictext within tabpage_9
end type
type cb_prototipi from commandbutton within tabpage_9
end type
type st_31 from statictext within tabpage_9
end type
type st_30 from statictext within tabpage_9
end type
type cb_placeholder_l from commandbutton within tabpage_9
end type
type st_cb_placeholder_l from statictext within tabpage_9
end type
...
type st_smart from statictext within tabpage_10
end type
...
//--- controllo cartella x esportazione dati Ritiro Lotti x SMART
			if trim(tab_1.tabpage_4.dw_4.getitemstring( k_riga, "smart_pickup_lots_dir")) > " " then
				if trim(tab_1.tabpage_4.dw_4.getitemstring( k_riga, "smart_pickup_lots_file")) > " " then
					// ok
				else
					k_testo = trim(tab_1.tabpage_4.dw_4.object.smart_pickup_lots_file_t.text)
					k_return += tab_1.tabpage_4.text + ": manca il nome del file nel campo '" + k_testo + "'~n~r"
					k_errore = "2"
					k_nr_errori++
				end if
			end if
...
//--- controllo codice barcode Dosimetri
			...
			if trim(kst_tab_base.dosimetro_ult_barcode) > " " then
				if left(kst_tab_base.dosimetro_ult_barcode, 2) <= "AA" and left(kst_tab_base.dosimetro_ult_barcode, 2) >= "ZZ" then
					k_testo = trim(tab_1.tabpage_5.dw_5.object.dosimetro_ult_barcode_t.text)
				...
				if k_errore = "0" then
					...
					tab_1.tabpage_5.dw_5.setitem( k_riga, "dosimetro_ult_barcode", k_rx) 
				end if
			end if	
...
if tab_1.tabpage_1.dw_1.rowcount( ) > 0 then
	...
	if tab_1.tabpage_1.dw_1.getitemstatus(1, 0, primary!) = datamodified! or &
		...
		k_text = ""
		k_text = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "memo_window_open")
		if isnull(k_text) then
			k_text="0"
		end if
	...
	if tab_1.SelectedTab = 5 then
		if tab_1.tabpage_5.dw_5.getitemstatus(1, 0, primary!) = datamodified! or &
			...
			if tab_1.tabpage_5.dw_5.update() < 0 then
				...
				try
//--- dati VARI
					kst_tab_base.id_base = tab_1.tabpage_5.dw_5.object.id[1]
					kst_tab_base.ddt_out_n_free = tab_1.tabpage_5.dw_5.getitemnumber(1, "ddt_out_n_free")
...
//	kpbdom_el_node11111 = create PBDOM_Element
	...
	if tab_1.tabpage_12.dw_12.rowcount() = 0 then
		...
		tab_1.tabpage_12.dw_12.object.file_errorlog.Expression = "trim('" + kguo_path.get_nome_path_file_errori_xml() + "')"
...
try
	...
	kst_tab_base.dosimetro_barcode_mask = trim(tab_1.tabpage_5.dw_5.getitemstring(1, "dosimetro_barcode_mask"))
	...
	tab_1.tabpage_5.dw_5.setitem(1, "dosimetro_ult_barcode", trim(kst_tab_base.dosimetro_ult_barcode))
...
k_path = tab_1.tabpage_4.dw_4.getitemstring (1, "smart_pickup_lots_dir")
if len(trim(k_path)) > 0 then
else
	k_path=".."
end if

k_ret = GetFolder ( "Scegliere la Cartella dove caricare i dati di Ritiro Lotti (SMART)", k_path )

if k_ret = 1 then
	tab_1.tabpage_4.dw_4.setitem(1, "smart_pickup_lots_dir", trim(k_path))
else
	if k_ret < 0 then
//--- ERRORE	
	end if
...
//tab_1.tabpage_personale.dw_dett_0.accepttext()
	...
	k_rcn = tab_1.tabpage_4.dw_4.update()

	if k_rcn < 0 then
		kst_esito = tab_1.tabpage_4.dw_4.kist_esito
		//messagebox("Archivio 'Proprietà Procedura' Non Aggiornato:", trim(kst_esito.SQLErrText), &
		//				stopsign!, ok!) 
...
//--- dati Cartelle
	...
	kst_tab_base.smart_pickup_lots_dir = tab_1.tabpage_4.dw_4.getitemstring(1, "smart_pickup_lots_dir")
	kst_tab_base.smart_pickup_lots_file = tab_1.tabpage_4.dw_4.getitemstring(1, "smart_pickup_lots_file")
	kst_tab_base.st_tab_g_0.esegui_commit = "N" 
...
//--- UPDATE!
	kst_tab_base.st_tab_g_0.esegui_commit = "N"
	kuf1_base.tb_update_other(kst_tab_base) 

	kguo_sqlca_db_magazzino.db_commit( )
		
	tab_1.tabpage_4.dw_4.resetupdate( ) // setitemstatus(1, 0, primary!, NotModified!)	
...
//--- Reimposta le Variabli Globali dei Dati di Ora legale e UTC 	
	...
	kuo_exception.messaggio_utente( "Proprietà Non Aggiornate", trim(kst_esito.SQLErrText))
...
//--- Scheda da visualizzare (se nulla visualizza tutto)
	...
	if k_scheda = "utente" then
		...
		tab_1.tabpage_5.dw_5.modify("background.color='" + string(rgb(255,0,0))+"'")
...
elseif dwo.name = "b_fgrp_out_path" then
	get_fgrp_out_path()
elseif dwo.name = "b_folder_doc_root" then
	get_path_doc_root()
elseif dwo.name = "b_folder_centrale" then
	get_path_centrale()
elseif dwo.name = "b_folder_upd" then
	get_path_pgm_upd()
...
elseif dwo.name = "b_folder_esolver_fidi" then
...
if dwo.name = "b_dosimetro_ult_barcode" then
	get_dosimetro_ult_barcode()
end if
...
long picturemaskcolor = 553648127
st_cb_protemail st_cb_protemail
cb_protemail cb_protemail
st_cb_prototipi st_cb_prototipi
cb_prototipi cb_prototipi
st_31 st_31
st_30 st_30
cb_placeholder_l cb_placeholder_l
st_cb_placeholder_l st_cb_placeholder_l
end type

on tabpage_9.create
this.st_cb_protemail=create st_cb_protemail
this.cb_protemail=create cb_protemail
this.st_cb_prototipi=create st_cb_prototipi
this.cb_prototipi=create cb_prototipi
this.st_31=create st_31
this.st_30=create st_30
this.cb_placeholder_l=create cb_placeholder_l
this.st_cb_placeholder_l=create st_cb_placeholder_l
int iCurrent
call super::create
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.st_cb_protemail
this.Control[iCurrent+2]=this.cb_protemail
this.Control[iCurrent+3]=this.st_cb_prototipi
this.Control[iCurrent+4]=this.cb_prototipi
this.Control[iCurrent+5]=this.st_31
this.Control[iCurrent+6]=this.st_30
this.Control[iCurrent+7]=this.cb_placeholder_l
this.Control[iCurrent+8]=this.st_cb_placeholder_l
end on

on tabpage_9.destroy
call super::destroy
destroy(this.st_cb_protemail)
destroy(this.cb_protemail)
destroy(this.st_cb_prototipi)
destroy(this.cb_prototipi)
destroy(this.st_31)
destroy(this.st_30)
destroy(this.cb_placeholder_l)
destroy(this.st_cb_placeholder_l)
end on
...
long backcolor = 16777215
string text = "Archivi generali di supporto"
boolean focusrectangle = false
end type
...
long textcolor = 8388608
long backcolor = 16777215
string text = "Gestione Prototipi e-mail. Definire il messaggio in formato testo o HTML da usare per l~'invio:"
...
long textcolor = 8388608
long backcolor = 16777215
string text = "Associa il Prototipo a una specifica Funzione:"
alignment alignment = right!
boolean focusrectangle = false
end type
...
//=== Si potrebbero passare:
//=== key1=codice sl_pt;
kuf_placeholder kuf1_placeholder


kuf1_placeholder = create kuf_placeholder
kuf1_placeholder.u_open(kkg_flag_modalita.elenco )

if isvalid(kuf1_placeholder) then destroy kuf1_placeholder
...
long textcolor = 8388608
long backcolor = 16777215
string text = "Elenco segnaposti per la comunicazione email:"
alignment alignment = right!
boolean focusrectangle = false
end type
