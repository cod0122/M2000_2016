...
forward
global type kuf_base from kuf_parent0
end type
end forward
...
public function boolean if_thema_attivo ()
public function decimal get_version () throws uo_exception
private function boolean tb_update_base_dir (st_tab_base kst_tab_base) throws uo_exception
private function boolean tb_update_base_fatt (st_tab_base ast_tab_base) throws uo_exception
...
try
	...
	if kst_tab_base.st_tab_base_personale.memo_window_open > " " then
	else
		kst_profilestring_ini.file = "base_personale"
		kst_profilestring_ini.titolo = "conf_personale"
		kst_profilestring_ini.nome = "memo_window_open"
		...
		kst_tab_base.st_tab_base_personale.memo_window_open = trim(kst_profilestring_ini.valore)
	end if
...
//--- Progetti: numero progressivo del Modulo di Laboratorio
		case "ptasks_valid_modaccompn"
			k_long = long(trim(k_key_1))
			update base_vari
				set ptasks_valid_modaccompn = :k_long
				using kguo_sqlca_db_magazzino;
			if kguo_sqlca_db_magazzino.sqlcode = 0 then
			else
				kst_esito.esito = kkg_esito.db_ko 
...
//--- Progetti: numero progressivo della Fattura
		case "ptasks_cs_invoicen"
			k_long = long(trim(k_key_1))
			update base_vari
				set ptasks_cs_invoicen = :k_long
				using kguo_sqlca_db_magazzino;
			if kguo_sqlca_db_magazzino.sqlcode = 0 then
			else
				kst_esito.esito = kkg_esito.db_ko 
...
//--- DDT LIBERO: numero del DDT manuale di OUT 
		case "ddt_out_n_free"
			k_long = long(trim(k_key_1))
			update base_vari
				set ddt_out_n_free = :k_long
				using kguo_sqlca_db_magazzino;
			if kguo_sqlca_db_magazzino.sqlcode = 0 then
			else
				kst_esito.esito = kkg_esito.db_ko 
...
//--- DDT LIBERO: anno del DDT manuale di OUT 
		case "ddt_out_year_free"
			k_long = long(trim(k_key_1))
			update base_vari
				set ddt_out_year_free = :k_long
				using kguo_sqlca_db_magazzino;
			if kguo_sqlca_db_magazzino.sqlcode = 0 then
			else
				kst_esito.esito = kkg_esito.db_ko 
...
public function string prendi_dato_base (string k_key);//
//----------------------------------------------------------------------------------------------------------------------------------
//--- Legge dati BASE (arch.ini o tabella) restituendo un valore 
//---
...
//			kst_esito = kguo_exception.inizializza(this.classname())
//			kuf1_base = create kuf_base
//			k_esito_base = kuf1_base.prendi_dato_base( "num_int")
...
//--- prendo il nome utente windows
		case "utente_login"
			k_ulong = 255
			k_record = space(k_longptr)
			k_boolean = GetUserNameA ( k_record, k_longptr )
			if k_boolean then
...
//				kst_esito.SQLErrText = 
		...
		case "memo_window_open" 
			...
			kst_profilestring_ini.titolo = "conf_personale"
			kst_profilestring_ini.nome = "memo_window_open_on"
			kst_profilestring_ini.operazione = kGuf_data_base.ki_profilestring_operazione_leggi_iniz
			kst_profilestring_ini.valore = "0"
			k_rcx = kGuf_data_base.profilestring_ini(kst_profilestring_ini)
			if kst_profilestring_ini.esito <> "0" then
				kst_profilestring_ini.valore = " "
			end if
			k_record = trim(kst_profilestring_ini.valore)  // 1=ok apri MEMO
			k_pos_ini = 1
			k_lungo = len(k_record)
		...
		case else
			...
			if kguo_sqlca_db_magazzino.if_connesso( ) then
				...
				k_return = prendi_dato_base_db(k_key)       // cerca nel DB se connesso
				

				if left(k_return, 1) = "9" then
	//---- Ultima spiaggia cerco dentro ai dati personali
					...
					k_stato = left(k_return, 1) 
					k_errore = mid(k_return, 2) + " "
					kguo_sqlca_db_magazzino.sqlcode = 0
					k_pos_ini = 0
				...
				k_stato = "1"
            k_errore = "Database non connesso, dato di configurazione '" + k_key + "' non recuperato."
				kguo_sqlca_db_magazzino.sqlcode = 0
			end if
...
//--- se ok restituisco il dato salvato nella stringa K_RECORD			
	...
	k_return = "1" + + "Esito '" + kuo_exception.kist_esito.esito + "' " + trim(kuo_exception.kist_esito.sqlerrtext) + " (" + string(kuo_exception.kist_esito.sqlcode) + ")"
...
st_tab_base kst_tab_base
	...
	if kst_tab_base.update_last_vers = "S" then
		...
		kst_tab_base.last_version = get_version( )

		if kst_tab_base.last_version > kkG.versione then
			k_return=true
		end if
	end if
...
public function boolean if_thema_attivo ();//---
//---  Verifica se applicare il thema 
//---
//---  Out: TRUE = attivo
//---
boolean k_return=false
st_tab_base_personale kst_tab_base_personale
	...
	try
		kst_tab_base_personale.thema_attivo = trim(Mid(prendi_dato_base("thema_enabled"), 2)) 
		if kst_tab_base_personale.thema_attivo > ' ' then
			if kst_tab_base_personale.thema_attivo <> '0' then
				kst_tab_base_personale.thema_attivo = '1'
			end if
		else
			kst_tab_base_personale.thema_attivo = '0'
		end if
		
		if kst_tab_base_personale.thema_attivo = '1' then
			k_return = true
		end if
	catch (uo_exception kuo_exception)
...
public function decimal get_version () throws uo_exception;//---
//--- Get numero della Versione 'Ufficiale' della Procedura 
//---
//---  Out: torna TRUE solo se Procedura da aggiornare
//---
uo_exception kuo_exception 
st_tab_base kst_tab_base
		...
		kst_tab_base.last_version = dec(trim(Mid(prendi_dato_base("last_version"), 2)))
		if isnull(kst_tab_base.last_version) then
			kst_tab_base.last_version = 00.0000
		end if
		
		if isnull(kkG.versione) then
			kuo_exception = create uo_exception
...
return kst_tab_base.last_version

end function

private function boolean tb_update_base_dir (st_tab_base kst_tab_base) throws uo_exception;//
//====================================================================
//=== Aggiorna rek nella tabella BASE per dati riferimenti PATH
//=== 
//=== Input: st_tab_base 
...
if len(trim(kst_tab_base.id_base)) > 0 then
	...
	if isnull(kst_tab_base.smart_pickup_lots_dir) then
...
//--- tento l'insert se manca in arch.
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in aggiornamento dati 'Proprietà Procedura' (base_dir):" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	else
	...
	kst_esito.SQLErrText = "Carico dati 'Proprietà Procedura' (base_dir): operazione non eseguita, manca il codice Azienda. "
...
try
	...
	//--- tento l'insert se manca in arch.
		...
		if k_rcn < 0 then
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in aggiornamento 'Proprietà Procedura' (base_fatt): " + trim(kguo_sqlca_db_magazzino.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		else
		...
		kst_esito.SQLErrText = "Carico dati 'Proprietà Procedura' (base_fatt): operazione non eseguita, manca il codice Azienda. "
		kst_esito.esito = kkg_esito.no_esecuzione
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
if len(trim(ast_tab_base.id_base)) > 0 then
	...
	kst_esito.SQLErrText = "Carico dati 'Proprietà Procedura' (base_gmt): operazione non eseguita, manca il codice Azienda. "
	kst_esito.esito = kkg_esito.no_esecuzione
	kguo_exception.set_esito(kst_esito)
	throw kguo_exception
end if
...
//--- tento l'insert se manca in arch.
	...
	kst_esito.SQLErrText = "Carico dati 'Proprietà Procedura' (base_SMTP): operazione non eseguita, manca il codice Azienda. "
	kst_esito.esito = kkg_esito.no_esecuzione
	kguo_exception.set_esito(kst_esito)
	throw kguo_exception
end if
...
public function boolean tb_update_other (st_tab_base ast_tab_base) throws uo_exception;//
//====================================================================
//=== Aggiorna rek nella tabella BASE per altri dati 
//=== 
//=== Input: st_tab_base 
//=== Ritorna tab. ST_ESITO, Esiti:  STANDARD; 
//=== 
...
try
	...
	if tb_update_base_fatt(ast_tab_base) then
		k_return = true
	end if
	if tb_update_base_smtp(ast_tab_base) then
		k_return = true
	end if
	if tb_update_base_dir(ast_tab_base) then
		k_return = true
	end if
	if tb_update_base_vari(ast_tab_base) then
		k_return = true
	end if
	if tb_update_base_gmt(ast_tab_base) then
		k_return = true
	end if
...
//=== Rit.: % Occupazione Pedana
//=== Lancia EXCEPTION
//=== 
//====================================================================
decimal{2} k_return
decimal{2} k_dimensioni_tot=0.0, k_dimensioni_passate=0.0, k_dimensioni_occup=0.0
		...
		if kst_tab_base_orig.mis_x > 0 then
			...
			if k_dimensioni_tot > 0 then
				...
				if k_dimensioni_passate > 0 then
					...
					if k_return = 0.00 then k_return = 1
				end if
			end if
		end if
...
st_profilestring_ini kst_profilestring_ini
		...
		kst_profilestring_ini.operazione = "2"
		kst_profilestring_ini.valore = a_path_centrale
		k_rcx = kGuf_data_base.profilestring_ini(kst_profilestring_ini)
		if kst_profilestring_ini.esito <> "0" then
			k_return = "1Dato '" + kst_profilestring_ini.nome + "' non aggiornato. Errore: " &
										  + trim(kst_profilestring_ini.esito)
		else
			k_return = "0" + kst_profilestring_ini.valore
		end if
...
st_profilestring_ini kst_profilestring_ini
		...
		k_rcx = kGuf_data_base.profilestring_ini(kst_profilestring_ini)
		if kst_profilestring_ini.esito <> "0" then
			k_return = "1Dato '" + kst_profilestring_ini.nome + "' non trovato. Errore: " &
													  + trim(kst_profilestring_ini.esito)
		else
			k_return = "0" + kst_profilestring_ini.valore
		end if
...
private function string prendi_dato_base_db (string k_key);/*
----------------------------------------------------------------------------------------------------------------------------------
 Legge dati BASE dal DB 
 ritorna char 1 : 0=ok; 1=errore; 9=k_key non trovata
              2-...: valore  

----------------------------------------------------------------------------------------------------------------------------------
*/
string k_record, k_return = " ", k_stato, k_errore = ""
...
double k_num1, k_num2, k_num3, k_double
ulong k_ulong 
boolean k_boolean, k_trim_no
string k_Version, k_Level, k_MajorVersion, k_MinorVersion, k_ProductBuild, k_Edition, k_EngineEdition 
string k_name, k_server_id, k_provider
datastore kds
st_stampante kst_stampante 
kuf_stampe kuf1_stampe 
...
//				end if
		...
		case "path_centrale"
			...
			k_record = trim(k_record)
			if k_record > " " then
				set_confdb_path_centrale(k_record)
			else
				k_record = " "
			end if
			k_pos_ini = 1
...
//--- BASE_DIR ---------------------------------------------------------------------------------------------------------------------------------------------			
		...
		case "smart_pickup_lots" // Cartella+nome file X dati SMART di Ritiro Lotti
			k_record = kkg.path_sep
			select trim(smart_pickup_lots_dir) + :k_record + trim(smart_pickup_lots_file)
				 into :k_record
				 from base_dir;
			k_record = trim(k_record)
			if isnull(k_record) or k_record = kkg.path_sep then
				k_record = ""
			end if
			k_pos_ini = 1
			k_lungo = len(k_record)
		case "smart_pickup_lots_dir" // Cartella X dati SMART di Ritiro Lotti
			select smart_pickup_lots_dir
				 into :k_record
				 from base_dir;
			k_record = trim(k_record)
			if isnull(k_record) then
			...
			k_pos_ini = 1
			k_lungo = len(k_record)
		case "smart_pickup_lots_file" // Nome file X dati SMART di Ritiro Lotti
			select smart_pickup_lots_file
				 into :k_record
				 from base_dir;
			k_record = trim(k_record)
			if isnull(k_record) then
...
//--- prefisso URL fornito da SMART per fare link di conferma della Prenotazione Ritiro Lotti
		case "smart_pickup_lots_url_xbook"
			select coalesce(smart_pickup_lots_url_xbook, '')
				 into :k_record
				 from base_vari;
			k_record = trim(k_record)
			k_pos_ini = 1
			k_lungo = len(k_record)
//--- Chiave per calcolare il codice hash da attaccare al URL di cui sopra
		case "smart_pickup_lots_sha_key"
			select coalesce(smart_pickup_lots_sha_key, '')
				 into :k_record
				 from base_vari;
			k_record = trim(k_record)
			k_pos_ini = 1
...
//--- Progetti: numero progressivo del Modulo di Laboratorio
		case "ptasks_valid_modaccompn"
			select coalesce(ptasks_valid_modaccompn, 0)
				 into :k_long
				 from base_vari;
			k_record = string(k_long)
			k_pos_ini = 1
...
//--- Progetti: numero progressivo della Fattura
		case "ptasks_cs_invoicen"
			select coalesce(ptasks_cs_invoicen, 0)
				 into :k_long
				 from base_vari;
			k_record = string(k_long)
			k_pos_ini = 1
...
//--- DDT LIBERO: numero del DDT manuale di OUT 
		case "ddt_out_n_free"
			select coalesce(ddt_out_n_free, 0)
				 into :k_long
				 from base_vari;
			k_record = string(k_long)
			k_pos_ini = 1
...
//--- DDT LIBERO: anno del DDT manuale di OUT 
		case "ddt_out_year_free"
			select coalesce(ddt_out_year_free, 0)
				 into :k_long
				 from base_vari;
			k_record = string(k_long)
			k_pos_ini = 1
