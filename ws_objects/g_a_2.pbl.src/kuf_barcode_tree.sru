...
public function integer u_tree_riempi_listview_x_rifer_da_stamp (ref kuf_treeview kuf1_treeview, string k_tipo_oggetto)
end prototypes
		...
		choose case k_tipo_oggetto
			...
			case kuf1_treeview.kist_treeview_oggetto.barcode_no_st_dett
				k_query_where = " where " &
					...
					+ " and (meca.stato = " + string(kuf1_armo.ki_meca_stato_blk_controllato) + " "  &
					+       " or meca.stato = " + string(kuf1_armo.ki_meca_stato_blk_da_controllare) + ") "
...
//					+ " and (barcode.data_sosp <= ?  or  barcode.data_sosp is null) " 
			...
			case kuf1_treeview.kist_treeview_oggetto.barcode_sosp_dett
				k_query_where = " where " &
					...
					+ " or meca.stato = '" + string(kuf1_armo.ki_meca_stato_blk_controllato) + "' "  & 
					+ " or meca.stato = '" + string(kuf1_armo.ki_meca_stato_blk_da_controllare) + "' "  & 
...
integer k_return = 0, k_rc
long k_handle_item = 0, k_handle_item_padre = 0, k_handle_item_figlio = 0
integer k_ctr, k_mese, k_anno, k_pic_list, k_row, k_rows
string k_tipo_oggetto_padre, k_tipo_oggetto_figlio
string k_query_select, k_query_where, k_query_order, k_barcode_periodo
date k_save_data_int, k_data_da, k_data_a, k_data_0
treeviewitem ktvi_treeviewitem
...
//--- ricavo i dati dalla struttura gia' letta nel padre
		...
		kds_tree.dataobject = "ds_meca_barcode_da_stamp"
		kds_tree.settransobject(kguo_sqlca_db_magazzino)
		k_rows = kds_tree.retrieve(k_data_da, k_data_a, kuf1_armo.ki_meca_stato_OK)
...
//			if kguo_sqlca_db_magazzino.sqlcode = 0 then
		...
		for k_row = 1 to k_rows
			...
			kst_tab_meca.clie_2 = kds_tree.getitemnumber(k_row, "clie_2")
			kst_tab_meca.clie_3 = kds_tree.getitemnumber(k_row, "clie_3")
			kst_tab_meca.num_bolla_in = kds_tree.getitemstring(k_row, "num_bolla_in")
			kst_tab_meca.data_bolla_in = kds_tree.getitemdate(k_row, "data_bolla_in")
			kst_tab_meca.area_mag = kds_tree.getitemstring(k_row, "area_mag")
			kst_tab_meca.id_wm_pklist = kds_tree.getitemnumber(k_row, "id_wm_pklist")
			kst_tab_contratti.codice = kds_tree.getitemnumber(k_row, "contratto")
			kst_tab_contratti.mc_co = kds_tree.getitemstring(k_row, "mc_co")
			kst_tab_contratti.sc_cf = kds_tree.getitemstring(k_row, "sc_cf")
			kst_tab_clienti.rag_soc_10 = kds_tree.getitemstring(k_row, "rag_soc_10") 
			kst_tab_clienti.rag_soc_11 = kds_tree.getitemstring(k_row, "rag_soc_10_1") 
			kst_tab_clienti.rag_soc_20 = kds_tree.getitemstring(k_row, "rag_soc_10_2") 
			kst_tab_armo.magazzino = kds_tree.getitemnumber(k_row, "magazzino")
			kst_tab_armo.dose = kds_tree.getitemnumber(k_row, "dose") 
			kst_tab_armo.larg_2 = kds_tree.getitemnumber(k_row, "largh_2")
			kst_tab_armo.lung_2 = kds_tree.getitemnumber(k_row, "lung_2")
			kst_tab_armo.alt_2 = kds_tree.getitemnumber(k_row, "alt_2")
			kst_tab_armo.peso_kg = kds_tree.getitemnumber(k_row, "peso_kg_unit")
			...
			kst_treeview_data_any.contati = kds_tree.getitemnumber(k_row, "n_barcode")
			...
			kst_treeview_data.label = &
										 string(kst_tab_barcode.num_int, "####0") &
									  + " - " + string(kst_tab_barcode.data_int, "dd.mm.yyyy") &
									  + " (" + string(kst_treeview_data_any.contati) + " barcode)" &
									  + " " + string(kst_tab_meca.clie_1, "#####") + " -> " &
									  + string(kst_tab_meca.clie_3, "#####") + ""
			kst_treeview_data.oggetto = k_tipo_oggetto_figlio 
			kst_treeview_data.oggetto_padre = k_tipo_oggetto
	...
	choose case a_campo_link
		...
		case "barcode", "barcode_t", "barcode_lav", "barcode_lav_t"
			...
			if len(kst_tab_barcode.barcode) > 0 then
				...
				k_title = "Dettaglio Barcode : " + trim(string(kst_tab_barcode.barcode,"@@@ @@@@@@@@@")) 
			else
				k_return = false
			end if
		...
		case "barcode_figli", "barcode_figli_t"
			...
			if len(kst_tab_barcode.barcode) > 0 then
				...
				k_title = "Elenco 'Figli' del Barcode : " + trim(string(kst_tab_barcode.barcode,"@@@ @@@@@@@@@")) 
			else
				k_return = false
			end if
		...
		case "barcode_lotto" 
			...
			if kst_tab_barcode.id_meca > 0 then
				...
				k_title = "Elenco Barcode Lotto  (id=" + trim(string(kst_tab_barcode.id_meca)) + ") " 
			else
				k_return = false
			end if
		...
		case "grp" 
			...
			if kst_tab_barcode.id_meca > 0 then
				...
				k_title = "Groupage Barcode Lotto  (id=" + trim(string(kst_tab_barcode.id_meca)) + ") " 
			else
				k_return = false
			end if
...
//--- Costruisce e Dimensiona le colonne all'interno della listview
		kuf1_treeview.kilv_lv1.getColumn(1, k_label, k_align_1, k_ctr) 
		if k_label <> "Lotti (da stamp)" then 
			kuf1_treeview.kilv_lv1.DeleteColumns ( )
			k_ind=1
			k_campo[k_ind] = "Lotti (da stamp)"
			k_align[k_ind] = left!
			k_ind++
			k_campo[k_ind] = "Area"
			k_align[k_ind] = left!
