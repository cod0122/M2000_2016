...
kuf_ptasks_types kuf1_ptasks_types
...
//		end if
	...
	case 9
		k_record = " Tipo Attività Progetto "
		k_riga = tab_1.tabpage_9.dw_9.getrow()	
		if k_riga > 0 then
			k_key_2 = tab_1.tabpage_9.dw_9.getitemnumber(k_riga, "id_ptasks_type")
			k_desc = tab_1.tabpage_9.dw_9.getitemstring(k_riga, "descr")
			k_key = string(k_key_2)
		end if
end choose	
...
//=== Cancella la riga dal data windows di lista
		choose case tab_1.selectedtab 
			...
			case 9
				try
					kuf1_ptasks_types = create kuf_ptasks_types
					kst_tab_ptasks_types.id_ptasks_type = k_key_2
					kuf1_ptasks_types.tb_delete(kst_tab_ptasks_types)
				catch (uo_exception kuo_exception)
					kst_esito = kuo_exception.get_st_esito()
				finally
					if isvalid(kuf1_ptasks_types) then destroy kuf1_ptasks_types
				end try
		end choose	
...
choose case tab_1.selectedtab 
	...
	case 9
		...
		tab_1.tabpage_9.dw_9.setcolumn("task")
end choose	
...
int k_riga, k_riga_find, k_riga_find1
...
//=== Controllo altro tab
	...
	do while k_riga > 0  and k_nr_errori < 10
		...
		if isnull(tab_1.tabpage_2.dw_2.getitemdate ( k_riga, "data")) &
			...
			tab_1.tabpage_2.dw_2.setitem ( k_riga, "data", kguo_g.get_dataoggi())
		end if
...
//=== Controllo altro tab
	...
	do while k_riga > 0  and k_nr_errori < 10
		if tab_1.tabpage_9.dw_9.getitemstatus(k_riga, 0, primary!) = newmodified! then
			k_key = tab_1.tabpage_9.dw_9.getitemstring(k_riga, "task") 
			select count(task)
				into :k_nr_righe
				from ptasks_types
				where task = :k_key;
			if k_nr_righe > 0 then
				k_return = tab_1.tabpage_9.text + ": Attività '" + trim(k_key) &
							+ "' già caricata alla riga " + string(k_riga) + ".~n~r" 
				k_errore = "4"
				k_nr_errori++
			end if
		end if
...
for k_ctr = k_riga to 1 step -1
	if tab_1.tabpage_9.dw_9.getitemstatus(k_ctr, 0, primary!) = newmodified! & 
		...
		if trim(tab_1.tabpage_9.dw_9.getitemstring(k_ctr, "task")) > " " &  
				and trim(tab_1.tabpage_9.dw_9.getitemstring(k_ctr, "descr")) > " " then
		else
...
//		loop
	...
	case 9
		k_riga = tab_1.tabpage_9.dw_9.getnextmodified(0, primary!)
		do while k_riga > 0  
			tab_1.tabpage_9.dw_9.setitem(k_riga, "x_datins", kGuf_data_base.prendi_x_datins())
			tab_1.tabpage_9.dw_9.setitem(k_riga, "x_utente", kGuf_data_base.prendi_x_utente())
			k_riga = tab_1.tabpage_9.dw_9.getnextmodified(k_riga, primary!)
		loop
...
public subroutine u_dosimetrie_storicizza ();//
st_tab_dosimetrie kst_tab_dosimetrie
kuf_dosimetrie kuf1_dosimetrie
...
try
	...
	if messagebox("Storicizza Lotti Dosimetrici" &
			...
			+ string(kst_tab_dosimetrie.data, "dd mmm yy") + ". Procedere ?" &
			, Question!, yesno!, 2) = 1 then
			
		setpointer(kkg.pointer_attesa)
	
		kuf1_dosimetrie.tb_move_to_history(kst_tab_dosimetrie)
	...
	if isvalid(kuf1_dosimetrie) then destroy kuf1_dosimetrie
...
type st_duplica from w_ausiliari`st_duplica within w_ausiliari_1
end type
