...
st_esito kst_esito
	...
	if k_rek_ok = 1 then
		...
		if kguo_sqlca_db_magazzino.sqlcode <> 0 then
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Tab.Gruppi Articoli:" + trim(kguo_sqlca_db_magazzino.SQLErrText)
			if kguo_sqlca_db_magazzino.sqlcode = 100 then
...
//===
int k_file=0, k_rc 
int k_bytes, k_ctr, k_ctr_1, k_bytes_f, k_righe, k_righe_insert, k_riga_dett
string k_rcx
string k_record, k_numero_x
string k_path, k_nome_file, k_ext

st_esito kst_esito, kst_esito_insert
st_tab_dosimetrie kst_tab_dosimetrie, kst_tab_dosimetrie_test
...
//--- piglio il primo record di dettaglio
			...
			do while k_bytes > 0 and kst_esito.esito = kkg_esito.OK
				...
				k_riga_dett++
//--- ricopre la virgola decimale con il punto
				//k_record = kuf1_utility.u_replace(k_record, ",", ".")
						
//--- record di dettaglio piglio il coefficiente
				k_ctr = PosA (k_record, ';')
				if k_ctr > 0 then
					k_numero_x = trim(mid(k_record,1, k_ctr - 1)) 
					if isnumber(k_numero_x) then
						kst_tab_dosimetrie.coeff_a_s = double(trim(k_numero_x))
						if (k_ctr + 1) < k_bytes then
//--- record di dettaglio piglio la Dose
							k_ctr_1 = PosA (k_record, ';', k_ctr + 1)
							if k_ctr_1 > 0 then
								k_numero_x = trim(Mid(k_record,k_ctr+1, k_ctr_1 - k_ctr - 1))
								if isnumber(trim(k_numero_x)) then
									kst_tab_dosimetrie.dose = double(trim(k_numero_x))
								else
									kst_esito.esito = kkg_esito.bug
									kst_esito.sqlcode = 0
									kst_esito.SQLErrText = "Dose " + k_numero_x + " non numerica vedi riga " + string(k_righe) + ", valore: " + trim(MidA(k_record,k_ctr+1, k_ctr_1 - k_ctr - 1))
								end if
							else
								k_numero_x = trim(Mid(k_record, k_bytes - k_ctr)) 
								if isnumber(k_numero_x) then
									kst_tab_dosimetrie.dose = double(trim(k_numero_x))
									...
									kst_esito.esito = kkg_esito.bug
									kst_esito.sqlcode = 0
									kst_esito.SQLErrText = "Dose " + k_numero_x + " non numerica vedi riga " + string(k_righe) + ", valore: " + trim(MidA(k_record, k_ctr+1, k_bytes - k_ctr))
								end if
							end if
...
//--- inserisci nuovo record								
								if k_elaborazione_di_test then 
									if k_riga_dett = 1 then
										kst_tab_dosimetrie_test = kst_tab_dosimetrie
									end if
								else
						...
						kst_esito.esito = kkg_esito.bug
						kst_esito.sqlcode = 0
						kst_esito.SQLErrText = "Coefficiente " + k_numero_x + " non numerico vedi riga " + string(k_righe) + ", valore: " + trim(MidA(k_record, 1, k_ctr - 1))
					end if
				end if
	...
	if kst_esito.esito <> kkg_esito.ok then
		...
		if k_elaborazione_di_test then 
			kst_esito.SQLErrText = "Il File sembra corretto, controllare la prima riga qui sotto~n~r~n~r"  &
			                     + "Coeff A/S: " + string(kst_tab_dosimetrie_test.coeff_a_s ) + "~n~r" &
			                     + "dose: " + string(kst_tab_dosimetrie_test.dose ) + "~n~r" &
			                     + "data: " + string(kst_tab_dosimetrie_test.data ) + "~n~r" &
			                     + "tipo: " + string(kst_tab_dosimetrie_test.dosim_tipo ) + "~n~r" &
			                     + "note: " + trim(kst_tab_dosimetrie_test.note) + "~n~r~n~r" &
...
if trim(k_st_tab_nazioni.id_nazione) > " " then
	select count(*) 
	   into :k_contati
	   from nazioni
		where id_nazione = trim(:k_st_tab_nazioni.id_nazione)
...
public function boolean tb_pagam_if_pag_anticipato (ref st_tab_pagam ast_tab_pagam) throws uo_exception;//--
//---  Torna TRUE = pagamento ANTICIPATO
//--- 
//---  input: st_tab_pagam.codice
//---  otput: true/false
//---
...
if ast_tab_pagam.codice > 0 then 
  ...
  SELECT  
			  ...
			  gg_int,
           gg_ulteriori
	   into :ast_tab_pagam.scad
		    ,:ast_tab_pagam.rate
		    ,:ast_tab_pagam.gg_1_rata
		    ,:ast_tab_pagam.gg_int
		    ,:ast_tab_pagam.gg_ulteriori
	   from  pagam
       where codice   =  :ast_tab_pagam.codice 
       using kguo_sqlca_db_magazzino;
	...
	if sqlca.sqlcode < 0 then
		...
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.SQLErrText = "Errore in verifica se Pagamento cod. " + string(ast_tab_pagam.codice ) + " è 'Anticipato' ~n~r" &
	...
	if_isnull_tb(ast_tab_pagam)

	if ast_tab_pagam.scad <> "0" &
			and ast_tab_pagam.rate < 2  &
			and ast_tab_pagam.gg_1_rata = 0 &
			and ast_tab_pagam.gg_int = 0 &
			and ast_tab_pagam.gg_ulteriori = 0 then
		k_return = true
	end if
end if
