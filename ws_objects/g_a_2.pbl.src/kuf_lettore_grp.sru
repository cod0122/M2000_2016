...
private function st_esito popola_ds_file_grp (ref string k_file)
public function st_esito tb_truncate () throws uo_exception
public function boolean tb_delete_all (st_tab_lettore_grp kst_tab_lettore_grp) throws uo_exception
public function boolean tb_delete (st_tab_lettore_grp kst_tab_lettore_grp) throws uo_exception
end prototypes
...
//--- legge un file Groupage e lo importa nel DataStore
		if right(kst_file_lettore_grp[k_ind].path_file,1) <> "\" and right(kst_file_lettore_grp[k_ind].path_file,1) <> "/" then 
			k_file = kst_file_lettore_grp[k_ind].path_file + kkg.path_sep + kst_file_lettore_grp[k_ind].nome_file
		else
...
//--- controllo se GRP già caricato dallo stesso file
		...
		if not k_doppi then  // OK, Nessun DOPPIONE carico!!!
...
if len(trim(k_file)) > 0 then
	...
	if FileExists (k_file ) then
		...
		if k_errore < 0 then
			...
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = &
				"Errore in lettura File Groupage da palmare " + string(kst_tab_lettore_grp.id) + " (err= "+ string(k_errore) + ")~n~r" &
								+ "file: " + string(k_file) 
			kst_esito.esito = kkg_esito.db_ko
		end if
	else
...
public function st_esito tb_truncate () throws uo_exception;//
//====================================================================
//=== Drop e Ricreate tabella LETTORE_GRP
//=== 
//=== Input: 
//=== Ritorna ST_ESITO
//=== 
...
try
	kst_esito = kguo_exception.inizializza(this.classname())
	if if_sicurezza(kkg_flag_modalita.cancellazione) then
	
	//--- ne approfitta cancellando anche tutti i groupage con PADRE=NULL
		k_truncate = "truncate table lettore_grp"
		EXECUTE IMMEDIATE :k_truncate
				using kguo_sqlca_db_magazzino;
	
		if kguo_sqlca_db_magazzino.sqlcode < 0 then 
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = &
	"Errore durante la rigenerazione della tabella dei Groupage da Lettore ~n~r" &
					+ " ~n~rErrore-tab.LETTORE_GRP:"	+ trim(kguo_sqlca_db_magazzino.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		end if
...
public function boolean tb_delete_all (st_tab_lettore_grp kst_tab_lettore_grp) throws uo_exception;/*
 ...
 Input: st_tab_lettore_grp.padre (ozpionale) e timestamp_inizio 
 Rit: cancellato=true
	...
	//--- ne approfitta cancellando anche tutti i groupage con PADRE=NULL
		delete from lettore_grp
			where timestamp_inizio = :kst_tab_lettore_grp.timestamp_inizio
					  and (:kst_tab_lettore_grp.padre = '' or padre = :kst_tab_lettore_grp.padre)
					  or padre is null
				using kguo_sqlca_db_magazzino;
		...
		if kguo_sqlca_db_magazzino.sqlcode < 0 then 
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = &
	"Errore in cancellazione del Groupage scaricati dal Lettore: " &
					+ kkg.acapo + " padre: " + string(kst_tab_lettore_grp.padre) + " data: " + string(kst_tab_lettore_grp.timestamp_inizio) &
					+ kkg.acapo + " Errore (LETTORE_GRP): " + trim(kguo_sqlca_db_magazzino.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		end if
...
try
	...
	this.if_sicurezza(kkg_flag_modalita.cancellazione)
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then 
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = &
	"Errore in cancellazione Barcode dal Groupage scaricato dal Lettore" &
					+ kkg.acapo + " id: " + string(kst_tab_lettore_grp.id) &
					+ kkg.acapo + "Errore (LETTORE_GRP): "	+ trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
