...
//--- nr colli caricati da trattare
		kst_tab_armo.id_armo = ast_tab_barcode.id_armo
		kst_tab_armo.colli_2 = kuf1_armo.get_colli_entrati_riga_datrattare(kst_tab_armo) +  kuf1_armo.get_colli_entrati_conto_deposito(kst_tab_armo)
...
try
	...
	if ast_tab_barcode.id_meca > 0 or ast_tab_barcode.id_armo > 0 then
		...
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.sqlerrtext = "Errore in importazione barcode da E1. Manca ID Lotto"
		kguo_exception.inizializza()
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
...
//		end if
			...
			kst_esito.esito = kkg_esito.no_esecuzione
			kst_esito.SQLErrText = "Errore in importazione codici Barcode da E1, codice Barcode Cliente Vuoto su tabella E1. " &
