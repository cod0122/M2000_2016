...
string themepath = "C:\Sterigenics\app\theme"
...
string appicon = "C:\Sterigenics\app\MAIN.ico"
...
function boolean GetUserNameA (ref string userID, ref longptr len) library "ADVAPI32.DLL" alias for "GetUserNameA;Ansi" 
...
//	kuf1_conf_access = create kuf_conf_access
//	kuf1_conf_access.u_set_st_conf_access( )
	kguo_utente.Ki_DOMAIN = kguo_sqlca_db_magazzino.kiuf_conf_access.kist_conf_access.domain
...
//29.01.2020				kguf_memo_allarme.set_visualizza_allarme( )
				end if
			else
...
try
	...
	kst_esito.sqlerrtext = "L'applicazione non riesce a gestire il tipo di errore interno che è capitato, pertanto il sistema potrebbe diventare instabile. Si consiglia di riavviare il programma. " &
			  + kkg.acapo + "Errore: " + string (Error.number) + " " + trim(error.Text) & 
