...
uo_exception kuo_exception
	...
	k_data_int_a = relativedate(kst_report_regart50.k_data_a, 120)
...
int krc = 0, k_ctr
string k_rcx=""
long kind=0, kriga=0, k_rows, k_id_meca_prev
...
try
	...
	kst_report_regart50.k_nrprotocollo --    //tolgo 1 al n. prot. così da ripartire bene 
	
	//--- Riempie il Datastore REPORT da restituire
	//--- Riferimenti entrati nel periodo indicato con i dati di uscita
	k_rows = kids_report_entrate_uscite.rowcount( ) 
	for kind = 1 to k_rows
		
		kriga=kids_report_regart50.insertrow(0)
		
		kids_report_regart50.setitem( kriga, "nrprot", 1 )  // più avanti ci metto il numero VERO!!
	
		kids_report_regart50.setitem( kriga,"nrpag", kst_report_regart50.k_nrpagina -1)  //--- numero di pagina di partenza
		
		kids_report_regart50.setitem( kriga, "data_sort", date(kids_report_entrate_uscite.getitemdatetime(kind, "data_ent")) )
		kids_report_regart50.setitem( kriga, "num_sort", kids_report_entrate_uscite.getitemnumber(kind, "num_int") )
		
		if kind > 1 then
			k_id_meca_prev = kids_report_entrate_uscite.getitemnumber(kind - 1, "id_meca")
		else
			k_id_meca_prev = 0
		end if
	//--- non ripete i dati se Lotto già indicato	
		if k_id_meca_prev <> kids_report_entrate_uscite.getitemnumber(kind, "id_meca") then
			kids_report_regart50.setitem( kriga, "num_sort_sub", 0 )
			...
			kids_report_regart50.setitem( kriga, "num_sort_sub", 1 )
			kids_report_regart50.setitem( kriga, "merce", "Vedasi Riferimento Lotto nr. " &
									+ string(kids_report_entrate_uscite.getitemnumber(kind -1, "num_int")) + " del " &
									+ string(kids_report_entrate_uscite.getitemdate(kind -1, "data_int")) 	)
			...
			kids_report_regart50.setitem( kriga, "mittente", "")
		end if
	...
	k_rows = kids_report_solo_uscite.rowcount( ) 
	for kind = 1 to k_rows
		kriga=kids_report_regart50.insertrow(0)
		...
		kids_report_regart50.setitem( kriga, "num_sort", kids_report_solo_uscite.getitemnumber(kind, "num_int") )
		kids_report_regart50.setitem( kriga, "num_sort_sub", 0 )
	...
	kids_report_regart50.setsort( "data_sort asc, num_sort asc, num_sort_sub asc " )
	kids_report_regart50.sort( )
	
	//--- carico il nr. protocollo progressivo
	k_rows = kids_report_regart50.rowcount( ) 
	for kriga = 1 to k_rows
		
		if kids_report_regart50.getitemnumber( kriga, "nrprot" ) = 1 then
			kst_report_regart50.k_nrprotocollo++
			kids_report_regart50.setitem( kriga, "nrprot", kst_report_regart50.k_nrprotocollo )  //--- numero di Protocollo 
		end if
...
//		end if
//	end if
	if isnull(kst_tab_meca_da.id) then kst_tab_meca_da.id = 0
	if isnull(kst_tab_meca_a.id) then kst_tab_meca_a.id = 0

	if sqlca.sqlcode < 0 then
		kst_esito = kguo_exception.inizializza(this.classname())
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.SQLErrText = "Lettura range id Lotti (MECA), da " &
						+ string(kst_tab_meca_da.data_int) + " a " &
						+ string(kst_tab_meca_a.data_int) + ": " + trim(sqlca.SQLErrText)
...
//	end if
	...
	if isnull(kst_tab_sped_da.id_sped) then kst_tab_sped_da.id_sped = 0
	if isnull(kst_tab_sped_a.id_sped) then kst_tab_sped_a.id_sped = 0

	if sqlca.sqlcode < 0 then
		kst_esito = kguo_exception.inizializza(this.classname())
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.SQLErrText = "Lettura range id DDT di spedizione (SPED), da " &
						+ string(kst_tab_sped_da.data_bolla_out) + " a " &
						+ string(kst_tab_sped_a.data_bolla_out) + ": " + trim(sqlca.SQLErrText)
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
