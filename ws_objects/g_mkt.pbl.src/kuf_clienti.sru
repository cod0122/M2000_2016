...
//--- dw Anteprima
constant string kki_anteprima_clienti_l = "d_clienti_l_rag_soc"
...
if isnull(kst_tab_clienti.kst_tab_clienti_web.email_send_certif_off) then kst_tab_clienti.kst_tab_clienti_web.email_send_certif_off = 0
...
if k_riga > 0 then
	...
	choose case a_campo_link
		...
		case "clie_1", "clie_2", "clie_3", "clie", "cod_cli", "id_cliente", "id_cliente_1" , "id_cliente_link" 
...
//--- 
//--- Inp: ragione sociale   
//--- Out: prima ragione sociale completa trovata
//--- rit: codice
...
st_tab_clienti kst_tab_clienti
	...
	kst_esito = kguo_exception.inizializza(this.classname())
  	...
  	if trim(a_rag_soc_10) > " " then
		...
		SELECT top (1) codice, rag_soc_10
			 into :kst_tab_clienti.codice
			 	,  :kst_tab_clienti.rag_soc_10
			 FROM clienti
...
//		if kst_esito.esito = kkg_esito.ok then
	//--- scelta del e-mail ta tornare			
		choose case a_nr_email
			case 1
				k_return = get_email1(ast_tab_clienti_web)
				//k_return = ast_tab_clienti_web.email
			case 2
				k_return = get_email2(ast_tab_clienti_web)
				//k_return = ast_tab_clienti_web.email1
			case 3
				k_return = get_email3(ast_tab_clienti_web)
				//k_return = ast_tab_clienti_web.email2
			case 4
				k_return = get_email4(ast_tab_clienti_web)
...
//		end if
		...
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.sqlcode = 0
		kst_esito.SQLErrText = "Manca codice cliente in Lettura indirizzo email"
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
	...
	kst_esito.SQLErrText = "Errore in Lettura Clienti per ricerca indirizzo email (Cliente: " + string(ast_tab_clienti_web.id_cliente) + ") ~n~r" + trim(kst_esito1.SQLErrText)
	kguo_exception.set_esito(kst_esito)
	throw kguo_exception
...
if kguo_sqlca_db_magazzino.sqlcode <> 0 then
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.SQLErrText = "Errore in Lettura Cliente per codice Nazione. Codice '" +string(ast_tab_clienti.codice) + "'" //~n~r" + trim(sqlca.SQLErrText) 
		kguo_exception.set_esito( kst_esito)
		throw kguo_exception
	end if
else
	if kguo_sqlca_db_magazzino.sqlcode = 0 then
		if trim(ast_tab_clienti.id_nazione_1) > " " then
			...
			ast_tab_clienti.id_nazione_1 = "IT"
		end if
...
st_esito kst_esito
  ...
  SELECT 
	...
	if sqlca.sqlcode = 0 then
		...
		if sqlca.sqlcode < 0 then
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = "Errore in lettura email 1 in Tab.Anagrafiche cliente " &
						+ string(kst_tab_clienti_web.id_cliente) + ". Errore: " + trim(sqlca.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		else
...
return kst_tab_clienti_web.email
...
st_esito kst_esito
  ...
  SELECT 
	...
	if sqlca.sqlcode = 0 then
		...
		if sqlca.sqlcode < 0 then
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = "Errore in lettura email 2 in Tab.Anagrafiche cliente " &
						+ string(kst_tab_clienti_web.id_cliente) + ". Errore: " + trim(sqlca.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
...
return kst_tab_clienti_web.email1
...
st_esito kst_esito
  ...
  SELECT 
	...
	if sqlca.sqlcode = 0 then
		...
		if sqlca.sqlcode < 0 then
			kst_esito.sqlcode = sqlca.sqlcode
			kst_esito.SQLErrText = "Errore in lettura email 3 in Tab.Anagrafiche cliente " &
						+ string(kst_tab_clienti_web.id_cliente) + ". Errore: " + trim(sqlca.SQLErrText)
			kst_esito.esito = kkg_esito.db_ko
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
...
return kst_tab_clienti_web.email2
...
return kst_tab_clienti_web.email3

end function

public function integer get_email_send_certif_off (ref st_tab_clienti_web kst_tab_clienti_web) throws uo_exception;//
//-----------------------------------------------------------------------------
//--- Torna se fare email del Certificato 
//--- 
//--- Input: st_tab_clienti_web.id_cliente     
//--- Output: st_tab_clienti_web.get_email_send_certif_off 
//--- Ritorna get_email_send_certif_off (1=non fare) 
//--- 
//-----------------------------------------------------------------------------
int k_return = 0
st_esito kst_esito
	...
	kst_tab_clienti_web.email_send_certif_off = 0

   if kst_tab_clienti_web.id_cliente > 0 then
	else
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.SQLErrText = "Manca codice cliente in Lettura se fare email dell'Attestato."
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
	
//--- get prima del numero email da usare 
	SELECT clienti_web.email_send_certif_off
		 into :kst_tab_clienti_web.email_send_certif_off
		 FROM clienti_web
		 where clienti_web.id_cliente = :kst_tab_clienti_web.id_cliente
			using kguo_sqlca_db_magazzino;
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in Lettura Clienti per ricerca se fare email dell'Attestato (Cliente: " + string(kst_tab_clienti_web.id_cliente) + ") ~n~r:" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if

//--- da numero email da usare piglio l'indirizzo
	if not isnull(kst_tab_clienti_web.email_send_certif_off) then
		k_return = kst_tab_clienti_web.email_send_certif_off
	end if 
...
public function long get_clie_3_da_rag_soc (ref string a_rag_soc_10) throws uo_exception;//
//-----------------------------------------------------------------------------------------------
//--- Torna CODICE CLIENTE da Ragione sociale (prima parte)
//--- 
//--- Inp: ragione sociale   
//--- Out: prima ragione sociale completa trovata
//--- rit: codice
	...
	kst_esito = kguo_exception.inizializza(this.classname())
  	...
  	if trim(a_rag_soc_10) > " " then
		...
		SELECT top (1) codice, rag_soc_10
			 into :kst_tab_clienti.codice
			 	,  :kst_tab_clienti.rag_soc_10
			 FROM clienti inner join m_r_f on clienti.codice = m_r_f.clie_3
			 where clienti.rag_soc_10 like :kst_tab_clienti.rag_soc_10 
			      and clienti.stato <> :kst_tab_clienti.stato
			 group by codice, rag_soc_10
	   using kguo_sqlca_db_magazzino ;
	
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in Lettura Mandante (ricerca codice da nome: " + trim(a_rag_soc_10) + "). Errore: " + trim(kguo_sqlca_db_magazzino.SQLErrText) //~n~r
			kguo_exception.set_esito(kst_esito)
			throw kguo_exception
		else
			if kguo_sqlca_db_magazzino.sqlcode = 100 or isnull(kst_tab_clienti.codice) then
