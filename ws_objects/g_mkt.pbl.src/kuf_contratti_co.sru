...
st_esito kst_esito
   ...
   SELECT offerta_data
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		...
		kst_esito.SQLErrText = "Errore in Lettura Contratto Commerciale (cercato Ultimo Documento) ~n~r:" + trim(kguo_sqlca_db_magazzino.SQLErrText)
	end if
...
ds_contratti_co_select kds_contratti_co_select
	...
	if kst_tab_contratti_co.id_contratto_co > 0 then
		...
		if kds_contratti_co_select.kist_esito.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kds_contratti_co_select.kist_esito.sqlcode
			kst_esito.SQLErrText = "Errore in Lettura Contratto Commerciale ~n~r:" + trim(kds_contratti_co_select.kist_esito.SQLErrText)
		end if
