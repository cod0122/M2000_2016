...
st_esito kst_esito
   ...
   SELECT  offerta_data, max(id_contratto_rd)  
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		...
		kst_esito.SQLErrText = "Errore in Lettura Contratto di Studio&Sviluppo (cercato Ultimo Documento) ~n~r:" + trim(kguo_sqlca_db_magazzino.SQLErrText)
	end if
...
ds_contratti_rd_select kds_contratti_rd_select
	...
	if kst_tab_contratti_rd.id_contratto_rd > 0 then
		...
		if kds_contratti_rd_select.kist_esito.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kds_contratti_rd_select.kist_esito.sqlcode
			kst_esito.SQLErrText = "Errore in Lettura Contratto Studio Sviluppo ~n~r:" + trim(kds_contratti_rd_select.kist_esito.SQLErrText)
		end if
