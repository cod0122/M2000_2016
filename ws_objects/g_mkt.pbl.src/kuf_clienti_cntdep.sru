...
public constant int kki_flag_note_aco_NO = 0
public constant int kki_flag_note_aco_SI = 1
...
public function integer get_flag_note_aco (ref st_tab_clienti_cntdep_cfg kst_tab_clienti_cntdep_cfg) throws uo_exception
end prototypes
...
long k_ornumvia_progr, k_row_max, k_row
int k_ornumvia_x_len, k_rc
string k_orrifdes_packinglist, k_ornumvia_x, k_file, k_ordocarr_packinglist
kuf_file_explorer kuf1_file_explorer
...
//				k_orrifdes_packinglist = kids_clienti_cntdep_l_xpklist.getitemstring(k_row, "orrifdes")
			if k_ordocarr_packinglist <> kids_clienti_cntdep_l_xpklist.getitemstring(k_row, "ordocarr") then
				k_ordocarr_packinglist = kids_clienti_cntdep_l_xpklist.getitemstring(k_row, "ordocarr")
				k_ornumvia_progr ++
			end if
...
public function integer get_flag_note_aco (ref st_tab_clienti_cntdep_cfg kst_tab_clienti_cntdep_cfg) throws uo_exception;//
//====================================================================
//=== Legge tabella CLIENTI_CNTDEP_CFG per flag NOTE per ACO
//=== 
//=== Input: st_tab_clienti_cntdep_cfg.id_cliente
//=== Out: st_tab_clienti_cntdep_cfg.flag_note_aco       
//=== Ritorna: flag_note_aco  
//=== 
//====================================================================
st_esito kst_esito
int k_return = 0
	...
	kst_esito = kguo_exception.inizializza(this.classname())
  	...
  	SELECT
		  flag_note_aco
    INTO 
	 	  :kst_tab_clienti_cntdep_cfg.flag_note_aco
        FROM clienti_cntdep_cfg
        WHERE ( id_cliente = :kst_tab_clienti_cntdep_cfg.id_cliente)   
 ...
 if kguo_sqlca_db_magazzino.sqlcode <> 0 then
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		...
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.SQLErrText = "Errore in Lettura dati Conto Deposito (ACO) Cliente codice=" +string(kst_tab_clienti_cntdep_cfg.id_cliente) + "~n~r" + trim(sqlca.SQLErrText) 
		kguo_exception.inizializza( )
		kguo_exception.set_esito( kst_esito)
		throw kguo_exception
	end if
	...
	if kguo_sqlca_db_magazzino.sqlcode = 0 then
		if kst_tab_clienti_cntdep_cfg.flag_note_aco > 0 then
			k_return =  kst_tab_clienti_cntdep_cfg.flag_note_aco
		end if
	end if
end if
