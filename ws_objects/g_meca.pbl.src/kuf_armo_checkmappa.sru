...
private function st_esito u_check_dati_riga (ref datastore ads_inp_righe) throws uo_exception
end prototypes
...
//--- controllo se esistono documenti con data maggiore ma numero minore		
	if kst_tab_meca.id > 0 then
		...
		if kst_tab_meca_data.id > 0  then
			...
			kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Data in conflitto con i Documenti già emessi in precedenza, ad esempio per il Lotto con ID/ASN = " + string(kst_tab_meca_data.id)) 
			k_nr_errori++
		end if
	end if
...
//--- Controllo dati LOTTO
	try
		...
		if not kguo_exception.if_esito_ok_wrn(kst_esito1.esito) then  // se ho preso ERR GRAVE allora aggiorna errore
			kst_esito.esito = kst_esito1.esito
		end if
		kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline(kst_esito1.sqlerrtext)
	end try

//--- AREA MAG
	if kguo_exception.if_esito_ok_wrn(kst_esito.esito) then  
		if NOT if_carico_nodose() then  // area magazzino deve esserci x forza?
			if trim(kst_tab_meca.area_mag) > " " then
				k_area_mag_alpha = kuf1_utility.u_stringa_alfanum(kst_tab_meca.area_mag)
				if trim(kst_tab_meca.area_mag) <> k_area_mag_alpha then
					kst_esito.esito = kkg_esito.ko
					kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("'Area Magazzino' non Corretta, verificare il codice caricato!" )
					kguo_exception.set_esito( kst_esito ) 	
				end if
			else
				kst_esito.esito = kkg_esito.DATI_WRN
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Manca 'Area Magazzino'")
				kguo_exception.set_esito( kst_esito ) 	
			end if
		end if
	end if
	...
	if kguo_exception.if_esito_ok_wrn(kst_esito.esito) then  
		if ads_inp_testa.getitemnumber(k_riga_testa, "meca_blk_id_meca_causale") > 0 then
			...
			if trim(ads_inp_testa.getitemstring(k_riga_testa, "meca_blk_descrizione")) > " " then 
				...
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Attenzione, in presenza della descrizione del BLOCCO è necessario caricare anche il codice nel campo '" + trim(ads_inp_testa.describe("meca_blk_id_meca_causale_t.text")) + "' ")
				k_nr_errori++
			end if
		end if
	end if
		...
		try
			...
			if kst_tab_meca.contratto > 0 then
				...
				if not if_articoli_nocontratto( ) then
					...
					kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Righe Articolo caricate senza il codice Contratto!")
					k_nr_errori++
				end if
			end if
...
//--- Controlla DATA ENTRATA LOTTO
			if kst_esito.esito = kkg_esito.ok or kst_esito.esito = kkg_esito.DB_WRN or kst_esito.esito = kkg_esito.DATI_WRN then
				if date(kst_tab_meca.data_ent) > kkg.data_zero and date(kst_tab_meca.data_ent) < kst_tab_meca.data_int then
					kst_esito.esito = kkg_esito.DATI_WRN
					kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Prego controllare, data di Entrata " + string(kst_tab_meca.data_ent, "dd mmm yy") + " minore della data Lotto " + string(kst_tab_meca.data_int, "dd mmm yy"))
					k_nr_errori++
				end if
			end if
...
//--- check dettaglio righe ------------------------------------------------------------------------------------------------------------------------------------------------------------
	kst_esito = u_check_dati_riga(ads_inp_righe)


catch (uo_exception kuo3_exception)
	kst_esito1 = kuo3_exception.get_st_esito()
	if not kguo_exception.if_esito_ok_wrn(kst_esito1.esito) then  // se ho preso ERR GRAVE allora aggiorna errore
		kst_esito.esito = kst_esito1.esito
	end if
	kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline(kst_esito1.sqlerrtext)
...
//--- se non ci sono righe di dettaglio (neanche tra le cancellate) allora legge la fattura
	...
	k_riga = 1  
	do while kguo_exception.if_esito_ok_wrn(kst_esito.esito) and k_riga <= k_nr_righe and k_nr_errori < 10
...
//--- Verifica Mt CUBI
		...
		if kst_tab_armo.m_cubi > 0 then
			if kst_tab_armo.alt_2 > 0 then
				...
				kst_esito.esito = kkg_esito.DATI_WRN
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Riga Lotto n. " + string(k_riga) + ": Altezza pallet a ZERO ma Metri Cubi indicati maggiore di zero!")
				k_nr_errori++
			end if
		else
			if kst_tab_armo.alt_2 > 0 then // m_cubi a ZERO ma altezza valorizzata
				kst_esito.esito = kkg_esito.err_logico
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Riga Lotto n. " + string(k_riga) + ": Indicati Metri Cubi a ZERO ma il Pallet ha una altezza maggiore di zero!")
				k_nr_errori++
			end if
		end if
...
//--- Verifica PESO
		...
		if kst_tab_armo.peso_kg > 0 then
			if kst_tab_armo.alt_2 > 0 then
				...
				kst_esito.esito = kkg_esito.DATI_WRN
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Riga Lotto n. " + string(k_riga) + ": Altezza pallet a ZERO ma PESO indicato maggiore di zero!")
				k_nr_errori++
			end if
		else
			if kst_tab_armo.alt_2 > 0 then // m_cubi a ZERO ma altezza valorizzata
				kst_esito.esito = kkg_esito.err_logico
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Riga Lotto n. " + string(k_riga) + ": Indicato PESO a ZERO ma il Pallet ha una altezza maggiore di zero!")
				k_nr_errori++
			end if
		end if
		...
		if kst_tab_armo.larg_2 > 0 and kst_tab_armo.lung_2 > 0 and kst_tab_armo.alt_2 > 0 and kst_tab_armo.peso_kg > 0 then
			...
			if kuf1_armo.if_magazzino_da_trattare(kst_tab_armo) then
				...
				kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Riga Lotto n. " + string(k_riga) &
								+ "(mag. " + string(kst_tab_armo.magazzino) + "): Pallet da Trattare, Dimensioni (larg-lung-alt) e Peso devono essere maggiori di ZERO!")
				k_nr_errori++
			end if
		end if

//--- E1 richiede il codice contratto del loro Sistema, eccetto per i Listini di servizio
		if kst_tab_armo.magazzino <> 3 and kst_tab_armo.magazzino <> 9 then
			if kguo_g.if_e1_enabled( ) then  
				...
				if kst_tab_listino.id > 0 then
					...
					if trim(kst_tab_listino.e1litm) > " " then
						...
						kst_esito.esito = kkg_esito.dati_insuff
						kst_esito.sqlerrtext = kguo_exception.u_add_esito_and_nwline("Manca il codice di E-ONE su Listino. E' necessario censirlo in archivio!")
						k_nr_errori++
					end if
				end if
			end if
