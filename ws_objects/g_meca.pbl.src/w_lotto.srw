...
private boolean ki_e1_richiesta_importa_eseguita=false
...
try
	...
	if tab_1.tabpage_1.dw_1.rowcount( ) > 0 then
		...
		choose case ki_tab_1_index_new
			case 1 
				...
				if k_riga > 0 then
					if tab_1.tabpage_1.dw_1.getitemstatus(k_riga, 0, primary!) <> new!  &
						...
						k_record_1 = &
							"Sei sicuro di voler eliminare definitivamente il Lotto~n~r" + &
							"Riferimento n. " + string(kst_tab_meca.num_int, "#######") +  &
							" del " + string(kst_tab_meca.data_int) + " (ASN id=" + string(kst_tab_meca.id) + ") ?"
					else
				...
				if k_riga > 0 then
					...
					if kuf1_barcode.if_barcode_in_pl(kst_tab_barcode) then
						...
						k_record_1 = "il dosimetro " + trim(kst_tab_meca_dosim.barcode) + " non può essere rimosso, "  &
									+ " barcode " + trim(kst_tab_barcode.barcode) + " pianificato o già trattato."
					else
			...
			case 9
				k_record = " MEMO "
				k_riga = tab_1.tabpage_9.dw_9.getselectedrow(0)	
				if k_riga > 0 then
					kst_tab_memo.id_memo = tab_1.tabpage_9.dw_9.getitemnumber(k_riga, "id_memo")
					kst_tab_memo.titolo = tab_1.tabpage_9.dw_9.getitemstring(k_riga, "titolo")
					if trim(kst_tab_memo.titolo) > " " then
					else
						kst_tab_memo.titolo = "*SENZA TITOLO*"
					end if
					k_riga1 = tab_1.tabpage_5.dw_5.getselectedrow(k_riga)	
					if k_riga1 > 0 then
						k_record_1 = "Sei sicuro di voler eliminare i MEMO selezionati di cui il primo è " + string(kst_tab_memo.id_memo) + ". Titolo: '"  &
										+ trim(kst_tab_memo.titolo) + "' ?"
					else
						k_record_1 = "Sei sicuro di voler eliminare il MEMO " + string(kst_tab_memo.id_memo) + ". Titolo: '"  &
									+ trim(kst_tab_memo.titolo) + "' ?"
					end if
				end if
...
//=== Se righe in lista
	if k_riga > 0  then
		...
		if messagebox("Elimina" + k_record, k_record_1, question!, yesno!, 2) = 2 then
			kguo_exception.setmessage("Elimina" + k_record, "Operazione Annullata !!")
	...
	//=== Richiesta di conferma della eliminazione del rek
				...
				kst_esito = kiuf_armo.tb_delete_riferimento( kst_tab_meca )   // CANCELLA LOTTO!
			...
			case 9
				...
				do while k_riga > 0
					kst_tab_memo.id_memo = tab_1.tabpage_9.dw_9.getitemnumber(k_riga, "id_memo")
					kuf1_memo.tb_delete( kst_tab_memo )   // CANCELLA MEMO
					k_riga = tab_1.tabpage_9.dw_9.getselectedrow(k_riga)	
				loop 
				kst_esito.esito = kkg_esito.ok
		end choose	
		...
		if kst_esito.esito <> kkg_esito.ok then
			...
			kguo_exception.setmessage("Elimina" + k_record, k_errore)
		...
		choose case tab_1.selectedtab 
			...
			case 9
				...
				if tab_1.tabpage_9.dw_9.rowcount() > 0 then
					tab_1.tabpage_9.dw_9.selectrow(0, false)
		...
		choose case k_rc
			...
			case 0
				...
				if ki_st_open_w.flag_modalita = kkg_flag_modalita.modifica then
					...
					kguo_exception.setmessage(  &
						"Lotto cercato non e' stato trovato in archivio ~n~r" + &
						"(ID Documento cercato: " + string(kist_tab_meca_orig.id) + ") " )
					tab_1.tabpage_1.dw_1.insertrow(0)
					tab_1.tabpage_1.dw_1.setitem(1, "k_avvertenze", kguo_exception.getmessage( ))
					tab_1.tabpage_1.dw_1.SetItemStatus(1, "k_avvertenze", primary!, NotModified!) 
...
//--- legge gli archivi di dettaglio				
					u_inizializza_3( )
					if u_inizializza_4( ) = 0 then
						u_inizializza_6( )  // legge ASN solo se non ci sono BARCODE x velocizzare un po'
					end if
					u_inizializza_5( )
					u_inizializza_8( )
...
//--- se non ho caricato nessuna riga e sono in inserimento nessuna aggiornamento!
	if tab_1.tabpage_4.dw_4.rowcount() = 0 then
		if ki_st_open_w.flag_modalita = kkg_flag_modalita.inserimento then
			u_tab_1_tabpage_dw_resetupdate()
		end if
...
//=== 
	if tab_1.tabpage_1.dw_1.rowcount() > 0 then	
		...
		if m_main.m_strumenti.m_fin_gest_libero1.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then 
			m_main.m_strumenti.m_fin_gest_libero1.text = "Elenco articoli del Contratto " 
			m_main.m_strumenti.m_fin_gest_libero1.microhelp = "Elenco articoli del Contratto indicato "
			m_main.m_strumenti.m_fin_gest_libero1.visible = true

			m_main.m_strumenti.m_fin_gest_libero1.toolbaritemVisible = true
			m_main.m_strumenti.m_fin_gest_libero1.toolbaritemText = 	"Art., Articoli con Contratto"
			m_main.m_strumenti.m_fin_gest_libero1.toolbaritembarindex=2
			
			m_main.m_strumenti.m_fin_gest_libero1.enabled = k_attiva
			m_main.m_strumenti.m_fin_gest_libero1.toolbaritemname = "contrBianco32.png"  //kGuo_path.get_risorse() + kkg.path_sep +
		end if
		if m_main.m_strumenti.m_fin_gest_libero2.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then 
			m_main.m_strumenti.m_fin_gest_libero2.text = "Elenco articoli senza Contratto " 
			m_main.m_strumenti.m_fin_gest_libero2.microhelp = "Elenco articoli senza Contratto "
			m_main.m_strumenti.m_fin_gest_libero2.visible = true

			m_main.m_strumenti.m_fin_gest_libero2.toolbaritemVisible = true
			m_main.m_strumenti.m_fin_gest_libero2.toolbaritemText = 	"Vari, Articoli senza Contratto"
			m_main.m_strumenti.m_fin_gest_libero2.toolbaritembarindex=2
			
			m_main.m_strumenti.m_fin_gest_libero2.enabled = k_attiva
			m_main.m_strumenti.m_fin_gest_libero2.toolbaritemname = "contrNero32.png"
		end if
		...
		if m_main.m_strumenti.m_fin_gest_libero6.enabled <> k_attiva_bcode or ki_st_open_w.flag_primo_giro = "S" then 
			if ki_e1_enabled then  // Se interfccia E1 è abilitata
				m_main.m_strumenti.m_fin_gest_libero6.text = "Genera E1-ASN" 
				m_main.m_strumenti.m_fin_gest_libero6.microhelp = "Genera ASN "
				m_main.m_strumenti.m_fin_gest_libero6.visible = true
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritemVisible = true
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritemText = "ASN, Genera E1-ASN"
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritembarindex=2
				m_main.m_strumenti.m_fin_gest_libero6.enabled = k_attiva_bcode
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritemname = "e1Lotto.png"
			else
				m_main.m_strumenti.m_fin_gest_libero6.text = "Genera Barcode " 
				m_main.m_strumenti.m_fin_gest_libero6.microhelp = "Genera Barcode "
				m_main.m_strumenti.m_fin_gest_libero6.visible = true
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritemVisible = true
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritemText = "Barcode, Genera Barcode"
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritembarindex=2
				m_main.m_strumenti.m_fin_gest_libero6.enabled = k_attiva_bcode
				m_main.m_strumenti.m_fin_gest_libero6.toolbaritemname = "barcode.ico"
			end if
		end if

		if ki_e1_enabled then
			if m_main.m_strumenti.m_fin_gest_libero9.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then 
				m_main.m_strumenti.m_fin_gest_libero9.text = "Importa dati E1" 
				m_main.m_strumenti.m_fin_gest_libero9.microhelp = "Importa E1"
				m_main.m_strumenti.m_fin_gest_libero9.visible = true
				m_main.m_strumenti.m_fin_gest_libero9.toolbaritemVisible = true
				m_main.m_strumenti.m_fin_gest_libero9.toolbaritemText = "E1, Importa solo dati necessari da E1"
				m_main.m_strumenti.m_fin_gest_libero9.toolbaritembarindex=2
				m_main.m_strumenti.m_fin_gest_libero9.enabled = k_attiva
				m_main.m_strumenti.m_fin_gest_libero9.toolbaritemname = "Database1!"
			end if
		end if
...
//		this.settoolbar( 2, false)
		...
		m_main.m_strumenti.m_fin_gest_libero9.visible = false
...
choose case LeftA(k_par_in, 2) 
	...
	case kkg_flag_richiesta.libero6 		//Genera BARCODE o i dati per e-ONE (ASN)
		u_genera_barcode_e1asn()
		
	case kkg_flag_richiesta.libero9 		//Importa dati da e-ONE 
		try
			k_msg = u_e1_importa_all(true)
			u_write_avvertenze(k_msg)
		catch (uo_exception kuo_exception)
			a_avvertenze = kuo_exception.get_errtext( )
			u_write_avvertenze(a_avvertenze)
			kuo_exception.messaggio_utente()
		finally 
		end try
...
try
	...
	if kst_tab_meca.contratto > 0 then
		...
		kds_1.settransobject(sqlca) 
		k_righe_articoli = kds_1.retrieve(kst_tab_meca.contratto, kst_tab_meca.clie_3, kst_tab_meca.data_int)
	else
		kguo_exception.inizializza( )
		kguo_exception.set_tipo(kguo_exception.KK_st_uo_exception_tipo_non_eseguito )
...
SetPointer(kkg.pointer_attesa )
	...
	kds_1.settransobject(sqlca) 
	kds_1.retrieve(kst_tab_meca.clie_3, kst_tab_meca.data_int)
	if kds_1.rowcount() > 0 then
		tab_1.selectedtab = 4
		kuf1_elenco = create kuf_elenco
		kst_open_w.flag_primo_giro = "S"
...
//--- Aggiorna le NOTE 
		kst_tab_armo_nt.note[1] = trim(tab_1.tabpage_4.dw_4.getitemstring(k_riga, "armo_nt_note_1"))
...
//--- se STATO NON BLOCCATO ok altrimenti barcode NON generabili
		if kst_tab_meca.stato = kiuf_armo.ki_meca_stato_ok then //<> kiuf_armo.ki_meca_stato_blk_con_controllo and kst_tab_meca.stato <> kiuf_armo.ki_meca_stato_gen_da_completare then
			...
			k_nr_barcode = u_get_nr_colli_xbcode( ) - u_get_nr_bcode_presenti() // Ottiene la differenza dei barcode tra eventuale prima e attuale
			if k_nr_barcode > 0 then
				ki_genera_barcode = true // Barcode generabili
			else
				if k_nr_barcode < 0 then
					if NOT ki_e1_enabled then // se E1 NON Attivo
						ki_genera_barcode = true // Barcode generabili
					end if
				end if
			end if
...
public subroutine set_stato_ok ();//
//---- Imposta lo STATO a NORMALE se è SBLOCCATO o DA-COMPLETARE
//
st_tab_meca kst_tab_meca

if tab_1.tabpage_1.dw_1.rowcount() > 0 then
	kst_tab_meca.stato = tab_1.tabpage_1.dw_1.getitemnumber(1, "stato")
	if kst_tab_meca.stato = kiuf_armo.ki_meca_stato_sblk &
	          or kst_tab_meca.stato = kiuf_armo.ki_meca_stato_gen_da_completare then
		tab_1.tabpage_1.dw_1.setitem(1, "stato", kiuf_armo.ki_meca_stato_ok)
	end if
end if
...
//--- salvo i parametri cosi come sono stati immessi
	...
	tab_1.tabpage_9.st_9_retrieve.Text =  string(tab_1.tabpage_1.dw_1.getitemnumber(1, "id_meca")) 
...
//--- Info x DRAG&DROP
	if tab_1.tabpage_9.dw_9.rowcount() <= 0 and (ki_st_open_w.flag_modalita = kkg_flag_modalita.modifica or ki_st_open_w.flag_modalita = kkg_flag_modalita.visualizzazione) then
		...
		tab_1.tabpage_9.st_9_retrieve.Text = "" // azzero per essere sicuro che al prx tentativo torna a fare la retrieve
	end if
...
//--- se tutto OK ESCE
	if k_nr_elab > 0 or ki_exit_si then
...
protected subroutine inizializza_lista ();//
ki_e1_richiesta_importa_eseguita =false
...
try
	...
	if ki_e1_enabled then
		if not u_if_esiste_e1litm( ) then // verifica la presenza di almeno un Contratto-E1 attivo
			...
			k_nr_elaborati = u_e1_crea_asn_esegui() 						// GENERA ASN SU E1
		end if
	else
		kst_tab_barcode.id_meca = tab_1.tabpage_1.dw_1.getitemnumber(1, "id_meca")
		k_nr_elaborati = u_genera_barcode_esegui(kst_tab_barcode)  	// GENERA I BARCODE
	...
	if k_nr_elaborati > 0 then
		if k_nr_elaborati = 1 then
			if ki_e1_enabled then
				k_esito = "Trasmessi dati a E-ONE, è stato generato " + string(k_nr_elaborati) + " riga del ASN"
			else
				k_esito = "Generazione Barcode corretta, è stato generato " + string(k_nr_elaborati) + " Barcode"
			end if
		else
			if ki_e1_enabled then
				k_esito = "Trasmessi dati a E-ONE, sono state generate " + string(k_nr_elaborati) + " righe del ASN"
			else
...
//		end if
	...
	k_nr_elaborati = u_e1_crea_asn_esegui( )  // ADD ASN SU E-ONE
...
private function integer u_get_nr_bcode_presenti () throws uo_exception;//---
//--- Calcola il nr barcode generati/importati meno le righe rimosse dall'elenco
//--- 
//--- out: nr colli
//
int k_return = 0
...
//
int k_return
int k_rc, k_ctr, k_row_deleted, k_rows
st_tab_meca kst_tab_meca, kst_tab_meca_prec
...
//--- importa barcode
	k_n_bcode = u_e1_importa_barcode()
	if k_n_bcode > 0 then
		k_msg = "Sono stati importati " + string(k_n_bcode) + " barcode"
	end if

//--- importa WO + SO
	k_wo = u_e1_importa_wo_so()
	if k_wo > 0 then
		if k_msg > " " then
			k_msg += " e "
		else
		...
		k_msg += "il WO n. " + string(k_wo) 
	end if

//--- importa data di entrata
	k_data_ent = u_e1_importa_data_ent()
	if date(k_data_ent) > kkg.data_zero then
		if k_msg > " " then
			k_msg += " e "
		else
			k_msg = "E' stata importata "
		end if
	...
	if k_msg > " " then
		...
		k_msgErr = "Non sono stati trovati dati da importare da E1."
	end if

else

	k_msgErr = "Elaborazione con E1 non Abilitata. Nessun dato può essere importato."
...
if k_msg_show then
	...
	if k_msg > " " then
		messagebox("Importazione Dati da E1 Terminata", k_msg)
	else
		messagebox("Nessun Dato Importato da E1", k_msgErr)
	end if
...
protected function string aggiorna_dati ();//=== Ritorna 1 char: 0=Tutto OK; 1=Errore grave; 
//===	              : 2=Errore Non grave dati aggiornati
//===               : 3=
//
string k_esito, k_return="0"
string k_msg
int k_nr_barcode = 0, k_nr_elaborati=0
boolean k_asn_presente=false
//st_tab_barcode kst_tab_barcode
st_tab_meca kst_tab_meca
...
try
	...
	k_return = super::aggiorna_dati( )    // Aggiorna LOTTO
...
type dw_print_0 from w_g_tab_3`dw_print_0 within w_lotto
end type
...
event cb_aggiorna::clicked;call super::clicked;//
//----
//
//st_esito kst_esito
//
...
event tab_1::u_constructor;//
							// 1     2     3     4     5     6     7     8     9   
ki_tabpage_enabled = {true, false, false, true, true, true, true, true, true} // disabilita alcune tabpage
...
try
	...
	kuf1_link_zoom.link_standard_call_p (kdw_link, "id_memo") // attiva i tasti con il LINK
...
//	if isvalid(kdw_link) then destroy kdw_link
