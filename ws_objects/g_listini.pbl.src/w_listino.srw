							...
							if kst_tab_listino_parent.id > 0 then
								try
									...
									if kst_tab_listino_parent.attivo = kiuf_listino.kki_attivo_si then
										...
										kst_tab_listino.dt_start = tab_1.tabpage_1.dw_1.getitemdate(1, "dt_start")
										if kst_tab_listino.dt_start <= kguo_g.get_dataoggi() then
											...
											kst_tab_listino.id_parent = kst_tab_listino_parent.id
											kst_tab_listino_parent.dt_end = kiuf_listino.set_dt_end_parent(kst_tab_listino)
											if kst_tab_listino_parent.dt_end > date(0) then  // aggiusta la data di fine validità nel Listino originale
												kguo_exception.inizializza( )
												kguo_exception.set_tipo(kguo_exception.kk_st_uo_exception_tipo_ok )
												kguo_exception.messaggio_utente( "Listino Aggiornato", &
													"Attenzione, modificata la data di Validità del Listino di duplica '" + string(kst_tab_listino.id_parent) + "' a " &
													+ string(kst_tab_listino_parent.dt_end, "dd mmm yyyy"))
											end if
										end if
									end if
...
//--- attiva DWC
	...
	if string(k_id_listino) <> string(trim(tab_1.tabpage_2.st_2_retrieve.Text), "[general];' '") then
		tab_1.tabpage_2.st_2_retrieve.text = string(k_id_listino)
		tab_1.tabpage_2.dw_2.retrieve(k_id_listino)
	end if

//--- metto ID in testata
	if tab_1.tabpage_2.dw_2.rowcount( )  > 0 then
		tab_1.tabpage_2.dw_2.modify("id_listino_testa.Expression='~"" + string(tab_1.tabpage_1.dw_1.getitemnumber(1,"id") ) + " ~" ' ")
	end if  
...
//=== 
	...
	m_main.m_strumenti.m_fin_gest_libero3.text = "Mandanti"
	m_main.m_strumenti.m_fin_gest_libero3.microhelp = "Elenco Mandanti per questo Cliente    "
...
//		   if trim(ki_st_open_w.flag_modalita) = kkg_flag_modalita.inserimento then
	...
	m_main.m_strumenti.m_fin_gest_libero3.toolbaritemText = 	"Elenco Mandanti per questo Cliente    "
	m_main.m_strumenti.m_fin_gest_libero3.toolbaritemName = "Insert!"
	m_main.m_strumenti.m_fin_gest_libero3.toolbaritembarindex=2
	
	m_main.m_strumenti.m_fin_gest_libero1.text = "Cambia il periodo di estrazione scheda elenco Movimenti (data Lotto)"
	m_main.m_strumenti.m_fin_gest_libero1.microhelp =  "Cambia periodo di estrazione Movimenti"
...
event tab_1::u_constructor; //
							// 1     2     3     4     5     6     7     8     9   
ki_tabpage_enabled = {true, true, true, false, true, false, false, false, false} // disabilita alcune tabpage
...
choose case upper(dwo.name)
	...
	case "ATTIVO"
		if kist_tab_listino_open.attivo <> kiuf_listino.kki_attivo_si and this.getitemnumber(row, "listino_id_parent") > 0 then
			if data = "S" then

				if tab_1.tabpage_1.dw_1.getitemdate(row, "dt_start") > kguo_g.get_dataoggi() then
				else

					post messagebox("Allerta", "L'attivazione di questo Listino potrebbe comportare la DISATTIVAZIONE del Listino '" &
				          + string(this.getitemnumber(row, "listino_id_parent")) + "' da cui e' stato duplicato " &
							, information!) 
...
//			k_errore = kuf1_listino.tb_gia_fatturato( kst_tab_listino )
			kst_tab_meca.id = kuf1_listino.if_key_modif( kst_tab_listino )
			if kst_tab_meca.id = 0 then
				...
				if date(kst_tab_meca.data_ent) > kkg.data_zero then
					...
					k_errore +=  " il materiale non risulta entrato"  
				end if
