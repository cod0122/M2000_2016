...
public function date get_dt_end (ref st_tab_listino ast_tab_listino) throws uo_exception
public function long get_id_listini_invigore (ref st_tab_listino kst_tab_listino[]) throws uo_exception
public function long get_id_listini_attivinodata (ref st_tab_listino kst_tab_listino[]) throws uo_exception
public function date get_dt_start (ref st_tab_listino ast_tab_listino) throws uo_exception
public function long get_id_parent (ref st_tab_listino ast_tab_listino) throws uo_exception
public function boolean set_dt_end (st_tab_listino kst_tab_listino) throws uo_exception
public function date set_dt_end_parent (ref st_tab_listino kst_tab_listino) throws uo_exception
public function long if_e1litm_gia_presente (st_tab_listino ast_tab_listino) throws uo_exception
public function string get_e1litm (ref st_tab_listino ast_tab_listino) throws uo_exception
public function long if_key_modif (st_tab_listino kst_tab_listino) throws uo_exception
public function decimal get_occup_ped (st_tab_listino ast_tab_listino) throws uo_exception
end prototypes
...
if isnull(kst_tab_listino.occup_ped ) then kst_tab_listino.occup_ped = 0.00
...
//=== key=vedi sotto
		...
		K_st_open_w.key3 = " " // 
		K_st_open_w.key4 = string(kst_tab_listino.contratto)  // cod contratto CO
		K_st_open_w.key5 = " " //
		K_st_open_w.key6 = " " //
...
st_tab_listino kst_tab_listino, kst_tab_listino_altro
...
//--- data VALIDITA'
		kst_tab_listino.dt_end = ads_inp.getitemdate (k_riga, "dt_end")
		if kst_tab_listino.dt_end > kkg.data_no then
			if ads_inp.getitemdate (k_riga, "dt_start") > kst_tab_listino.dt_end then
				k_errori ++
				k_tipo_errore = kkg_esito.err_formale
				ads_inp.modify("dt_start.tag = '" + k_tipo_errore + "' ")
				kst_esito.esito = kkg_esito.err_formale
				kst_esito.sqlerrtext += "Periodo in Vigore indicato (dal " + string(ads_inp.getitemdate (k_riga, "dt_start"), "dd mmm yy") + " al " &
																					+ string(kst_tab_listino.dt_end, "dd mmm yy") &
																					+ ") non congruente~n~r" 
			end if
		end if
...
//--- codice PRODOTTO 
		...
		kst_tab_contratti.attivo = ads_inp.getitemstring( k_riga, "attivo")
		if kst_tab_contratti.attivo <> kki_attivo_no then
	...
	//--- codice CONTRATTI 
			...
			//--- verifica data VALIDITA'
					if kst_tab_listino.dt_end > kkg.data_no then
						kuf1_contratti.get_data_scad(kst_tab_contratti)
						if kst_tab_contratti.data_scad  > kkg.data_no then
							if kst_tab_listino.dt_end > kst_tab_contratti.data_scad then
								kst_esito.sqlerrtext += "Listino in vigore fino al '" +  string(kst_tab_listino.dt_end, "dd mmm yyyy") &
											+ "' maggiore della scadenza in CO del '" + string(kst_tab_contratti.data_scad, "dd mmm yyyy") + "'~n~r" 
							end if
						end if
					end if
				else
	...
	//--- codice E1LITM preferibile se tipo=STANDARD
			if k_tipo_errore = "0" or k_tipo_errore = "4" or k_tipo_errore = "5"  then
	...
	//--- codice E1LITM verifica se già presente
			if k_tipo_errore = "0" or k_tipo_errore = "4" or k_tipo_errore = "5"  then
				if kst_tab_listino.dt_end > kkg.data_no then
					kst_tab_listino.id = ads_inp.getitemnumber ( k_riga, "id")
					kst_tab_listino.e1litm = trim(ads_inp.getitemstring ( k_riga, "e1litm"))
					if kst_tab_listino.e1litm > " " then
						kst_tab_listino_altro.id = if_e1litm_gia_presente(kst_tab_listino)
						if kst_tab_listino_altro.id> 0 then 
							k_errori ++
							k_tipo_errore = kkg_esito.DATI_WRN //"3"
							ads_inp.modify("e1litm.tag = '" + k_tipo_errore + "' ")
							kst_esito.esito = kkg_esito.DATI_WRN
							kst_esito.sqlerrtext += "Codice E1 già presente nel Listino Attivo (id=" +string(kst_tab_listino_altro.id)+")~n~r" 
						end if
					end if
				end if
			end if
...
//--- toglie valori NULL
	...
	if ast_tab_listino.occup_ped > 0.00 then 
		kds_listino.setitem(1, "occup_ped", ast_tab_listino.occup_ped)
	end if
	...
	if ast_tab_listino.dt_start > date(0) then 
		kds_listino.setitem(1, "dt_start", ast_tab_listino.dt_start)
		kds_listino.setitem(1, "dt_end", ast_tab_listino.dt_start)
	end if
	if ast_tab_listino.dt_end > date(0) then 
		kds_listino.setitem(1, "dt_end", ast_tab_listino.dt_end)
	end if
...
//--- Se nuova riga Imposto il campo Contatore ID (SERIAL)				
		...
		kst_esito.sqlcode = -1
		kst_esito.SQLErrText = "Errore in Duplica del Listino '" + string(ast_tab_listino.id_parent) + "'" 
		kst_esito.esito = KKG_ESITO.db_ko
...
public function long get_id_listini (ref st_tab_listino kst_tab_listino[]) throws uo_exception;//
//------------------------------------------------------------------------------------------------
//--- Torna  ID_ LISTINI in Vigore x Cliente e Contratto
//--- 
//--- Input: st_tab_listino[1]  COD_CLI, CONTRATTO, DATA_END (data di verifica, es. data oggi)
//--- Out: st_tab_listino[].id,  st_tab_listino[1].st_tab_g_0.contati il numero listini trovati 
//---                                      ordinata x  MIS_Y da grande a piccola 
...
//---- Legge i listini In Vigore nel periodo
	k_return = get_id_listini_invigore(kst_tab_listino[])
	
	if k_return = 0 then
//---- Legge i listini In Vigore senza periodo
		k_return = get_id_listini_attivinodata(kst_tab_listino[])
	end if
...
public function date get_dt_end (ref st_tab_listino ast_tab_listino) throws uo_exception;//
//====================================================================
//=== Torna il Data di fine Valididità 
//=== 
//=== Input: st_tab_listino id     
//=== Output: dt_end = data di scadenza          
//=== Ritorna: data di scadenza
//===           		  
//====================================================================
date k_return 
st_esito kst_esito
	...
	SELECT listino.dt_end
	 	 into
	  			:ast_tab_listino.dt_end
   	 FROM listino
		 where id = :ast_tab_listino.id
			  using kguo_sqlca_db_magazzino;		
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in lettura Data Fine Validità, id= " + string(ast_tab_listino.id) +  "~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText) + " - " + trim(kst_esito.nome_oggetto)
			kguo_exception.inizializza( )
			kguo_exception.set_esito(kst_esito)		
			throw kguo_exception
		end if
	...
	if ast_tab_listino.dt_end > date(0) then
		k_return = ast_tab_listino.dt_end
	end if
...
//--- Torna  ID_ LISTINI in Vigore x Cliente e Contratto
//--- 
//--- Input: st_tab_listino[1]  COD_CLI, CONTRATTO, dt_end (data di verifica, es. data oggi)   
//--- Out: st_tab_listino[].id,  st_tab_listino[1].st_tab_g_0.contati il numero listini trovati 
//---                                      ordinata x  MIS_Y da grande a piccola 
//--- Ritorna id del listino (occurs 1)
//---           		  
...
try
	...
	kds_1 = create datastore
	kds_1.dataobject = "ds_listino_l_invigore"
	kds_1.settransobject(kguo_sqlca_db_magazzino)
	kst_tab_listino[1].st_tab_g_0.contati = kds_1.retrieve(kst_tab_listino[1].COD_CLI, kst_tab_listino[1].CONTRATTO, kst_tab_listino[1].dt_end )
	
	if kst_tab_listino[1].st_tab_g_0.contati > 0 then
		for k_ctr = 1 to kst_tab_listino[1].st_tab_g_0.contati
				
			kst_tab_listino[k_ctr].id = kds_1.getitemnumber(k_ctr, "id_listino")
		...
		if kst_tab_listino[1].st_tab_g_0.contati < 0 then
			...
			kguo_exception.kist_esito.sqlcode = kst_tab_listino[1].st_tab_g_0.contati
			kguo_exception.kist_esito.SQLErrText = "Errore in Lettura Listini In Vigore alla data " + string(kst_tab_listino[1].dt_end, "dd mmm yyyy") &
								+ "~n~rCliente: " &
								+ string(kst_tab_listino[1].COD_CLI) + "- Contratto: " +  string(kst_tab_listino[1].contratto) 
			throw kguo_exception
		end if
	end if
...
public function long get_id_listini_attivinodata (ref st_tab_listino kst_tab_listino[]) throws uo_exception;//
//------------------------------------------------------------------------------------------------
//--- Torna  ID_ LISTINI in Vigore senza indicazione della data x Cliente e Contratto
//--- 
//--- Input: st_tab_listino[1]  COD_CLI, CONTRATTO, dt_end (data di verifica, es. data oggi)   
//--- Out: st_tab_listino[].id,  st_tab_listino[1].st_tab_g_0.contati il numero listini trovati 
//---                                      ordinata x  MIS_Y da grande a piccola 
...
try
	...
	kds_1 = create datastore
	kds_1.dataobject = "ds_listino_l_attivo_nodata"
	kds_1.settransobject(kguo_sqlca_db_magazzino)
	kst_tab_listino[1].st_tab_g_0.contati = kds_1.retrieve(kst_tab_listino[1].COD_CLI, kst_tab_listino[1].CONTRATTO)
	...
	if kst_tab_listino[1].st_tab_g_0.contati > 0 then
		...
		if kst_tab_listino[1].st_tab_g_0.contati < 0 then
			...
			kguo_exception.kist_esito.esito = kkg_esito.db_ko
			kguo_exception.kist_esito.sqlcode = kst_tab_listino[1].st_tab_g_0.contati
			kguo_exception.kist_esito.SQLErrText = "Errore in Lettura Listini In Vigore" &
								+ "~n~rCliente: " &
								+ string(kst_tab_listino[1].COD_CLI) + "- Contratto: " +  string(kst_tab_listino[1].contratto) 
			throw kguo_exception
		end if
...
public function date get_dt_start (ref st_tab_listino ast_tab_listino) throws uo_exception;//
//====================================================================
//=== Torna il Data di inizio Valididità 
//=== 
//=== Input: st_tab_listino id     
//=== Output: dt_start = data di inizio validità          
//=== Ritorna: data di inizio validità  
//===           		  
//====================================================================
date k_return 
st_esito kst_esito
	...
	SELECT listino.dt_start
	 	 into
	  			:ast_tab_listino.dt_start
   	 FROM listino
		 where id = :ast_tab_listino.id
			  using kguo_sqlca_db_magazzino;		
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in lettura Data Inizio Validità Listino, id= " + string(ast_tab_listino.id) +  "~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText) + " - " + trim(kst_esito.nome_oggetto)
			kguo_exception.inizializza( )
			kguo_exception.set_esito(kst_esito)		
			throw kguo_exception
		end if
	...
	if ast_tab_listino.dt_start > date(0) then
		k_return = ast_tab_listino.dt_start
	end if
...
st_esito kst_esito
	...
	SELECT listino.id_parent
	 	 into
	  			:ast_tab_listino.id_parent
   	 FROM listino
		 where id = :ast_tab_listino.id
			  using kguo_sqlca_db_magazzino;		
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore in lettura ID Listino Padre, id= " + string(ast_tab_listino.id) +  "~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText) + " - " + trim(kst_esito.nome_oggetto)
			kguo_exception.inizializza( )
			kguo_exception.set_esito(kst_esito)		
			throw kguo_exception
		end if
	...
	if ast_tab_listino.id_parent > 0 then
		k_return = ast_tab_listino.id_parent
	end if
...
public function boolean set_dt_end (st_tab_listino kst_tab_listino) throws uo_exception;//
//-----------------------------------------------------------------------------------------------------
//--- Aggiorna la data di scadenza del LISTINO padre (dal quale è stato duplicato)
//--- 
//--- Inp: st_tab_listino.id, dt_end
//--- Ritorna: true se OK 
//--- Lancia EXCEPTION
//---  
//-----------------------------------------------------------------------------------------------------
...
//--- imposto dati utente e data aggiornamento
	...
	update listino 
			set dt_end = :kst_tab_listino.dt_end
				 ,x_datins = :kst_tab_listino.x_datins 
				 ,x_utente = :kst_tab_listino.x_utente 
		 where 
			  id = :kst_tab_listino.id
		 using kguo_sqlca_db_magazzino ;

	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Errore in Aggiornamento Data fine Validità Listino " + string(kst_tab_listino.id) + ". Errore: " + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kst_esito.esito = kkg_esito.db_ko
		if kst_tab_listino.st_tab_g_0.esegui_commit <> "N" or isnull(kst_tab_listino.st_tab_g_0.esegui_commit) then
			kguo_sqlca_db_magazzino.db_rollback( )
		end if
...
//--- 
//--- Inp: st_tab_listino.id
//--- Ritorna: dt_end del listino parent aggiornato
//--- Lancia EXCEPTION
//---  
//-----------------------------------------------------------------------------------------------------
//
date k_return
st_tab_listino kst_tab_listino_parent
st_esito kst_esito
...
try
	...
	if kst_tab_listino.id_parent > 0 then
		kst_tab_listino_parent.id = kst_tab_listino.id_parent
	else
		kst_tab_listino_parent.id = get_id_parent(kst_tab_listino)
	end if
	if kst_tab_listino_parent.id > 0 then
		
		if kst_tab_listino.dt_start > date(0) then
		else
			get_dt_start(kst_tab_listino) // get data INIZIO del nuovo listino
		end if
		if kst_tab_listino.dt_start > date(0) then
			
			get_dt_end(kst_tab_listino_parent)  // get data FINE Listino Originale
			
			//--- se data FINE listino Originale non è impostata oppure supera data INIZIO del nuovo allora va aggiornata
			if kst_tab_listino.dt_start > kst_tab_listino_parent.dt_end &
			         and kst_tab_listino_parent.dt_end > date(0) then
			else
				kst_tab_listino_parent.dt_end = relativedate(kst_tab_listino.dt_start, -1) // 1 gg indietro alla data INIZIO del nuovo
				kst_tab_listino_parent.st_tab_g_0 = kst_tab_listino.st_tab_g_0
				if set_dt_end(kst_tab_listino_parent) then

					k_return = kst_tab_listino_parent.dt_end
...
public function long if_e1litm_gia_presente (st_tab_listino ast_tab_listino) throws uo_exception;//
//------------------------------------------------------------------------
//--- Verifica presenza di altri Codici Contratto E1 (e1litm) attivi  
//--- oltre al ID passato
//--- 
//--- Input: st_tab_listino.id + e1litm     
//--- Output:            
//--- Ritorna: altro Listino con E1 attivo
//---           		  
//------------------------------------------------------------------------
long k_return
long k_id
...
date k_dataoggi, k_datazero
	...
	if ast_tab_listino.e1litm > ' ' then
		SELECT top 1 a.id
				 into
					:k_id
				 FROM listino a, listino b
			 where a.e1litm = :ast_tab_listino.e1litm
					and a.attivo = 'S'
					and (a.dt_end is null or (a.dt_end > b.dt_start and a.dt_end > b.dt_start))
					and a.id <> :ast_tab_listino.id
					and b.id = :ast_tab_listino.id
					and b.attivo = 'S'
					and (b.dt_end is null or (b.dt_end > :k_datazero and b.dt_end > :k_dataoggi))
				  using kguo_sqlca_db_magazzino;		
		...
		if kguo_sqlca_db_magazzino.sqlcode <> 0 then
			if kguo_sqlca_db_magazzino.sqlcode < 0 then
				kst_esito.esito = kkg_esito.db_ko
				kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
				kst_esito.SQLErrText = "Errore in verifica presenza Listino con più Contratti-E1 (e1litm) attivi, Contratto E1 = " + string(ast_tab_listino.e1litm) &
				     + " (id " + string(ast_tab_listino.id) +  ")~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText) + " - " + trim(kst_esito.nome_oggetto)
				kguo_exception.inizializza( )
				kguo_exception.set_esito(kst_esito)		
				throw kguo_exception
			end if
...
public function long if_key_modif (st_tab_listino kst_tab_listino) throws uo_exception;//
//--------------------------------------------------------------------------------------------------------------
//--- Controlla se Chiave Listino modificabile
//--- 
//--- Ritorna: > 0 ID Lotto trovato aperto e non spedito quindi non modificabile 
//---          0 = ok modificabile
//---           		  
//--------------------------------------------------------------------------------------------------------------
...
try
	...
	if kst_tab_listino.id > 0 then
		...
		kst_esito.esito = kkg_esito.no_esecuzione
		kst_esito.sqlerrtext = "Controllo Fallito se campi chiave del Listino sono modificabili, Id Listino non indicato."
  ...
  SELECT max(armo.id_meca)
       into :k_num
			...
			where 
				 armo.id_listino = :kst_tab_listino.id
				 and (meca.aperto in ('S', 'R', ' ') or meca.aperto is null)
				 and  not exists ( 
			      SELECT arsp.id_arsp
				     FROM arsp
	...
	if kguo_sqlca_db_magazzino.sqlCode < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlerrtext = "Errore in verifica se campi chiave del Listini sono modificabili:~n~r" + kguo_sqlca_db_magazzino.sqlerrtext
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
