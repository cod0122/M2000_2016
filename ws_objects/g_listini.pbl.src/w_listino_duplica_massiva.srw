...
type st_waiting from statictext within w_listino_duplica_massiva
end type
...
protected subroutine attiva_menu ();//	
	...
	if not m_main.m_strumenti.m_fin_gest_libero3.enabled  then
		...
		m_main.m_strumenti.m_fin_gest_libero3.toolbaritemtext = "Deseleziona,"+ &
												 m_main.m_strumenti.m_fin_gest_libero3.text
...
//	
	if not m_main.m_strumenti.m_fin_gest_libero4.enabled  then
		...
		m_main.m_strumenti.m_fin_gest_libero4.toolbaritemtext = "Seleziona,"+ &
	...
	if k_soglia_min > 0 then
		...
		if k_soglia_max > 0 and k_soglia_max < k_soglia_min then
			if kst_esito.sqlerrtext > " " then
				kst_esito.sqlerrtext += "~n~rSoglia Minima superiore alla Massima"
				kst_esito.esito = kkg_esito.no_esecuzione
			else
...
//--- verifica periodo IN VIGORE
	if dw_box.getitemstring(1, "attivo") = "S" then
		if dw_box.getitemdate(1, "dt_start") > dw_box.getitemdate(1, "dt_end") then
			if kst_esito.sqlerrtext > " " then
				kst_esito.sqlerrtext += "~n~rPeriodo in vigore indicato, non congruente"
				kst_esito.esito = kkg_esito.no_esecuzione
			else
				kst_esito.sqlerrtext = "Periodo in vigore indicato, non congruente"
...
//--- imposta periodo 'In Vigore'
		kst_listino_duplica[1].attivo = dw_box.getitemstring(1, "attivo")
		if kst_listino_duplica[1].attivo = kiuf_listino.kki_attivo_si then 
			kst_listino_duplica[1].dt_start = dw_box.getitemdate(1, "dt_start")
			kst_listino_duplica[1].dt_end = dw_box.getitemdate(1, "dt_end")
		else
			kst_listino_duplica[1].attivo = kiuf_listino.kki_attivo_da_fare
		end if
		...
		for k_riga_x_dup = 1 to UpperBound(kist_listino_duplica[])
			if kist_listino_duplica[k_riga_x_dup].id_listino > 0 then
				...
				kst_listino_duplica[k_riga].attivo = kst_listino_duplica[1].attivo
				if kst_listino_duplica[k_riga].attivo = kiuf_listino.kki_attivo_si then
					kst_listino_duplica[k_riga].dt_start = kst_listino_duplica[1].dt_start
					kst_listino_duplica[k_riga].dt_end = kst_listino_duplica[1].dt_end
				end if
				...
				if kst_listino_duplica[k_riga].attivo = kiuf_listino.kki_attivo_si then
					kiuf_esito_operazioni.tb_add_riga("     --------> In VIGORE dal " + string(kst_listino_duplica[k_riga].dt_start, "dd mmm yy") &
												 + " al " + string(kst_listino_duplica[k_riga].dt_end, "dd mmm yy") + " <--------", false)
				else
					kiuf_esito_operazioni.tb_add_riga("     --------> Da Attivare manualmente, nessun periodo 'In VIGORE' indicato  <--------", false)
				end if
...
this.Control[iCurrent+1]=this.gb_emissione
this.Control[iCurrent+2]=this.rb_emissione_tutto
this.Control[iCurrent+3]=this.rb_emissione_selezione
this.Control[iCurrent+4]=this.rb_definitiva_si
this.Control[iCurrent+5]=this.rb_definitiva_no
this.Control[iCurrent+6]=this.pb_ok
this.Control[iCurrent+7]=this.dw_documenti
...
this.Control[iCurrent+12]=this.dw_box
this.Control[iCurrent+13]=this.st_waiting
...
if st_waiting.visible then
	st_waiting.x = dw_documenti.x + ( dw_documenti.width / 2 - st_waiting.width / 2)
	st_waiting.y = dw_documenti.y + ( dw_documenti.height / 2 - st_waiting.height / 2)
end if
...
type dw_print_0 from w_g_tab`dw_print_0 within w_listino_duplica_massiva
end type
...
string text = "Carico dei Listini in corso, prego attendere...."
...
long textcolor = 33554432
long backcolor = 67108864
string text = " Imposta i nuovi dati "
end type
