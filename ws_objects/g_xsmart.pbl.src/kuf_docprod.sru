...
//--- leggo il cliente del documento			
			...
			if kst_esito.esito = kkg_esito.ok then
				...
				if k_nr_errori > 15 then
					...
					k_errore += "DDT di spedizione n. " + string(kst_docprod_esporta.kst_tab_docprod[k_item].doc_num) + " NON Esportato, codice Cliente non Trovato (id doc=" + string(kst_docprod_esporta.kst_tab_docprod[k_item].id_docprod) +")! ~n~r"
				end if
			else	
				try
...
//--- lancia esportazione PDF
		k_righe_esportate = kuf1_certif_print.stampa_digitale_esporta(kst_docprod_esporta_pdf )
