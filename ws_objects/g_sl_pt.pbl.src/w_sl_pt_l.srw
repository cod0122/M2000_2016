...
private string ki_ultimo_rag_soc_cercato="!"
private int ki_ultimo_impianto_cercato=9
...
//
	if not m_main.m_strumenti.m_fin_gest_libero1.visible then
		m_main.m_strumenti.m_fin_gest_libero1.text = "Mostra/Nascondi righe Disattivate nei Listini e/o Contratti"
		...
		m_main.m_strumenti.m_fin_gest_libero1.toolbaritemText = "Nascondi,"+m_main.m_strumenti.m_fin_gest_libero1.text
...
long k_return=0	
	...
	if ki_ultimo_impianto_cercato = 3 then
		dw_lista_0.dataobject = "d_sl_pt_g3_elenco"
	else
		dw_lista_0.dataobject = "d_sl_pt_elenco"
	end if		
	
	dw_lista_0.settransobject( kguo_sqlca_db_magazzino )
	k_return = dw_lista_0.retrieve(ki_ultimo_cod_sl_pt_cercato, ki_ultimo_clie_3_cercato, ki_ultimo_rag_soc_cercato) 
	if k_return < 0 then
		kguo_exception.inizializza(this.classname())
		kguo_exception.set_esito(dw_lista_0.kist_esito)
		kguo_exception.kist_esito.sqlerrtext = "Errore in lettura elenco Piani di Trattamento (" + trim(dw_lista_0.dataobject) + "). " + kkg.acapo + dw_lista_0.kist_esito.sqlerrtext
...
type dw_print_0 from w_g_tab0`dw_print_0 within w_sl_pt_l
end type
...
try
	...
	kst_tab_sl_pt.impianto = dw_guida.getitemnumber(1, "impianto")
	...
	//--- estrazione sul Nominativo del cliente?	
		if upper(left(kst_tab_sl_pt.cod_sl_pt, 2)) = "N=" then
			kst_tab_clienti.rag_soc_10 = trim(mid(kst_tab_sl_pt.cod_sl_pt, 3))
			if kst_tab_clienti.rag_soc_10 > " " then
				kst_tab_clienti.rag_soc_10 = "%" + trim(kst_tab_clienti.rag_soc_10) + "%"
			else
		...
		if kst_tab_clienti.codice = 0 and kst_tab_clienti.rag_soc_10 = "" then
	...
	if kst_tab_sl_pt.cod_sl_pt <> ki_ultimo_cod_sl_pt_cercato or kst_tab_sl_pt.impianto <> ki_ultimo_impianto_cercato then
				
		ki_ultimo_impianto_cercato = kst_tab_sl_pt.impianto
		ki_ultimo_clie_3_cercato = kst_tab_clienti.codice
		ki_ultimo_rag_soc_cercato = kst_tab_clienti.rag_soc_10
...
type st_duplica from w_g_tab0`st_duplica within w_sl_pt_l
end type
