...
try
	...
	if ast_tab_email_invio.lang > " " then
		kst_msg_replace_placeholder.language = ast_tab_email_invio.lang
	end if
...
try
	...
	if ast_tab_email_invio.flg_lettera_html = ki_lettera_html_si then
	...
	if trim(ast_tab_email_invio.email_di_ritorno) > " " then
		...
		if left(k_esito,1) <> "0" then
			kst_esito.esito = kkg_esito.db_ko  
			kst_esito.SQLErrText = mid(k_esito,2)
			ast_tab_email_invio.email_di_ritorno = "email@email.com"   // nel caso la email non si trovi neanche in Proprietà Azienda
		else
			ast_tab_email_invio.email_di_ritorno = trim(mid(k_esito,2))
		end if
	end if
	gn_smtp.of_SetFrom(trim(ast_tab_email_invio.email_di_ritorno), kst_tab_base.rag_soc_1 + " " + kst_tab_base.rag_soc_2 )
	...
	//--- Imposto l'oggetto
	ast_tab_email_invio.oggetto = u_message_replace_placeholder(ast_tab_email_invio, ast_tab_email_invio.oggetto) // sostituisce i segnaposto con i valori
	gn_smtp.of_SetSubject(ast_tab_email_invio.oggetto)
	
	//--- Imposto il corpo della comunicazione da Inviare!!!!!!!
	ls_Emp_Input = get_comunicazione(ast_tab_email_invio) // recupera il messaggio da file o lettera
	ls_Emp_Input = u_message_replace_placeholder(ast_tab_email_invio, ls_Emp_Input) // sostituisce i segnaposto con i valori
	...
	// add any attachments
	if ast_tab_email_invio.flg_allegati = ki_allegati_si then
	...
	//--- piglia l'elenco dei file xml contenuti nella cartella
		...
		//--- estrae il file da allegare
					...
					gn_smtp.of_AddFile(ast_tab_email_invio.allegati_cartella + ls_filename) // aggiunge un file 
				end for
			else
	//--- se la cartella non esiste
				kst_esito.esito = kkg_esito.no_esecuzione  
				kst_esito.SQLErrText = "La cartella Allegati (" + trim(ast_tab_email_invio.allegati_cartella) + ") non esiste. Id e-mail " &
