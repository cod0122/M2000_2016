...
Else
	If ib_messagebox Then
		choose case ai_msglevel
			case iINFO
				MessageBox(	"Informazione di rete-email  (Msg Winsock)", &
							as_msgtext, Information!)
			case iDEBUG
				MessageBox(	"Debug  rete-email  (Msg Winsock)", &
							as_msgtext, Information!)
			case iERROR
				MessageBox(	"Errore di rete-email  (Msg Winsock)", &
							as_msgtext, StopSign!)
			case else
				MessageBox(	"Strato rete-email  (Msg a livello Winsock): " + String(ai_msglevel), &
							as_msgtext, Information!)
		end choose
	End If
End If
...
kst_esito.sqlerrtext = "Invio EMAIL in ERRORE. Livello: " + string(ai_msglevel) + " Errore: " + trim(as_msgtext)
