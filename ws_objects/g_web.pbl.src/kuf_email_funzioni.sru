...
public subroutine if_isnull (ref st_tab_email_funzioni kst_tab_email_funzioni)
end prototypes
...
if kst_tab_email_funzioni.id_email_funzione > 0 then
  ...
  SELECT
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		...
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Fallita lettura tab. di collegamento Prototipo E-mail funzione (id_email) " + string(kst_tab_email_funzioni.id_email_funzione) + " ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
else
	kst_esito.SQLErrText = "Manca id per leggere il prototipo E-mail. (funzione get_id_email) ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
	kst_esito.esito = kkg_esito.no_esecuzione
	kguo_exception.set_esito(kst_esito)
	throw kguo_exception
end if
...
if kst_tab_email_funzioni.id_email > 0 then
  ...
  SELECT
	...
	if kguo_sqlca_db_magazzino.sqlcode < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
		kst_esito.SQLErrText = "Fallita lettura Associazione Prototipi E-mail per codice funzione, id_email " + string(kst_tab_email_funzioni.id_email) + " ~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText)
		kguo_exception.set_esito(kst_esito)
		throw kguo_exception
	end if
else
	kst_esito.SQLErrText = "Manca id email del Prototipo x lettura codice Assciazione Prototipo e-mail."
	kst_esito.esito = kkg_esito.no_esecuzione
	kguo_exception.set_esito(kst_esito)
	throw kguo_exception
end if
