$PBExportHeader$kuf_sped_depo_ddt.sru
forward
global type kuf_sped_depo_ddt from nonvisualobject
end type
end forward

global type kuf_sped_depo_ddt from nonvisualobject
end type
global kuf_sped_depo_ddt kuf_sped_depo_ddt
...
private function boolean oldproduci_ddt_riga_pagamento (ref datastore kds_ddt_stampa, long k_riga_ds) throws uo_exception
public function integer produci_ddt (ref st_ddt_depo_stampa kst_ddt_depo_stampa[]) throws uo_exception
public function integer stampa_ddt (st_ddt_depo_stampa kst_ddt_depo_stampa[]) throws uo_exception
end prototypes
...
//--- Se il flag DUPLICA e' attivo allora diplica solo il file (dal path precedente) senza rifare la Stampa (ovviamente l'ITEM deve essere superiore a 1)
							if  kst_docprod_esporta.duplica[k_riga] and k_riga > 1 then
								...
								kst_ddt_depo_stampa[k_riga].tipo_num_copie = kki_tipo_num_copie_generica // emiss.solo di una copia
			
								kst_ddt_depo_stampa[1].id_sped_depo = kst_tab_docprod.id_doc
								kst_ddt_depo_stampa[1].num_bolla_out = kst_tab_docprod.doc_num
								kst_ddt_depo_stampa[1].data_bolla_out = kst_tab_docprod.doc_data
								produci_ddt(kst_ddt_depo_stampa[])   // PRODUCE IL LAY-OUT 
...
//--- stampo Note 1 e 2 da SPED
	...
	kids_stampa_ddt.setitem(k_riga, "sped_note", trim(kds_ddt_stampa.object.sped_note_1[k_riga_dw]) )
	if kids_stampa_ddt.describe("sped_note_2.x") <> "!" then
		kids_stampa_ddt.setitem(k_riga, "sped_note_2", trim(kds_ddt_stampa.object.sped_note_2[k_riga_dw])  )
	end if
...
if len(trim(kds_ddt_stampa.object.arsp_note_1[k_riga_ds] ) ) > 0 then
	k_note_totali = RightTrim(kds_ddt_stampa.object.arsp_note_1[k_riga_ds]) + " " 
end if
if len(trim(kds_ddt_stampa.object.arsp_note_2[k_riga_ds] ) ) > 0 then
...
k_note_totali = trim(kds_ddt_stampa.getitemstring(k_riga_ds, "armo_nt_note_1"))
...
if trim(kds_ddt_stampa.object.sped_ddt_st_num_data_in[k_riga_ds]) = kuf1_sped.kki_ddt_st_num_data_in_NO then
	...
	//--- aggiungo riga e controllo Fine Pagina	
		...
		k_txt = ""
		k_txt = "D.d.t. n.  " + upper(trim(kds_ddt_stampa.object.meca_num_bolla_in[k_riga_ds])) + "  del  " &
											+ string(kds_ddt_stampa.object.meca_data_bolla_in[k_riga_ds])
		if kds_ddt_stampa.object.e1doco[k_riga_ds] > 0 then
			k_txt += "    (wo: " +  string(kds_ddt_stampa.object.e1doco[k_riga_ds]) + ") "
		end if
...
public function integer produci_ddt (ref st_ddt_depo_stampa kst_ddt_depo_stampa[]) throws uo_exception;//
//--- Costruisce i ddt da stampare  
//--- input:  st_ddt_stampa  valorizzare prefnum_bolla_out + num_bolla_out + data_bolla_out dai ddt
//--- out: true = OK, false = non stampata
//---
long krc=0
...
try
	...
	if kst_ddt_depo_stampa[1].print_ddt_libero then
	...
	k_num_ddt = upperbound(kst_ddt_depo_stampa[])

	if k_num_ddt > 0 then
//qui		produci_ddt_inizializza(kst_ddt_depo_stampa[1])
	end if
	
//--------------------------------------- CICLO PRINCIPALE ------------------------------------------------------------------
	for k_riga_ddt = 1 to k_num_ddt

		if kst_ddt_depo_stampa[k_riga_ddt].id_sped_depo > 0 then

//--- legge UNA bolla intera
			//k_anno = year(kst_tab_sped.data_bolla_out)
			krc = kds_ddt_stampa.retrieve(kst_ddt_depo_stampa[k_riga_ddt].id_sped_depo)
			//krc = kds_ddt_stampa.retrieve(kst_tab_sped.num_bolla_out, k_anno )
			if krc <= 0 then 
			else
				
				if kst_ddt_depo_stampa[1].print_ddt_libero then
//qui					k_ddt_stampato = produci_ddt_1_libero(kst_ddt_depo_stampa[k_riga_ddt],kds_ddt_stampa)
				else
//qui					k_ddt_stampato = produci_ddt_1(kst_ddt_depo_stampa[k_riga_ddt],kds_ddt_stampa)
				end if
...
//	kiuf_sped.if_sicurezza(kkg_flag_modalita.stampa)
	...
	if upperbound(kst_ddt_depo_stampa[]) > 0 then

//--- produci stampa
		k_n_ddt_stampati = produci_ddt(kst_ddt_depo_stampa[])
	
		if k_n_ddt_stampati > 0 then

//=== stampa dw
//qui			k_stampato=stampa_ddt_emissione(kst_ddt_stampa[1])
...
on kuf_sped_depo_ddt.create
call super::create
TriggerEvent( this, "constructor" )
end on

on kuf_sped_depo_ddt.destroy
